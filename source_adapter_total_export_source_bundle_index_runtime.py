...
RUNTIME_TITLE = 'Total Export Source Bundle Index'
RUNTIME_STAGE = 'total_export_source_bundle_index'
...
class TotalExportSourceBundleIndexRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> TotalExportSourceBundleIndexRecord:
    ...
    return TotalExportSourceBundleIndexRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> TotalExportSourceBundleIndexRecord:
    return build_record(
        "total_export_source_bundle_index-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "total_export_source_bundle_index"},
    )


def summarise_record(record: TotalExportSourceBundleIndexRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: TotalExportSourceBundleIndexRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> TotalExportSourceBundleIndexRecord:
    return build_record(
...
def write_record(path: str | Path, record: TotalExportSourceBundleIndexRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> TotalExportSourceBundleIndexRecord:
