...
SCHEMA_VERSION = "source_adapter_capture_artifact_normalizer_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_capture_artifact_normalizer_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_CAPTURE_ARTIFACT_NORMALIZER_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['html', 'screenshot', 'snapshot', 'comment_thread', 'transcript']


@dataclass(frozen=True)
class CaptureArtifactNormalizerRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class CaptureArtifactNormalizerRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = CaptureArtifactNormalizerRuntimeRow(
            row_id=_stable_id("source_adapter.capture_artifact_normalizer_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="capture artifact normalizer",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "capture artifact normalizer",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.capture_artifact_normalizer_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "capture artifact normalizer",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.capture_artifact_normalizer_runtime.json"
    rows_path = out / f"{package['package_id']}.capture_artifact_normalizer_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.capture_artifact_normalizer_runtime.verification.json"
