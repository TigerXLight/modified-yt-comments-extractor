...
RUNTIME_TITLE = 'Evidence Final Integrity Check'
RUNTIME_STAGE = 'evidence_final_integrity_check'
...
class EvidenceFinalIntegrityCheckRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> EvidenceFinalIntegrityCheckRecord:
    ...
    return EvidenceFinalIntegrityCheckRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> EvidenceFinalIntegrityCheckRecord:
    return build_record(
        "evidence_final_integrity_check-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "evidence_final_integrity_check"},
    )


def summarise_record(record: EvidenceFinalIntegrityCheckRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: EvidenceFinalIntegrityCheckRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> EvidenceFinalIntegrityCheckRecord:
    return build_record(
...
def write_record(path: str | Path, record: EvidenceFinalIntegrityCheckRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> EvidenceFinalIntegrityCheckRecord:
