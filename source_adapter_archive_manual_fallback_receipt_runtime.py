...
RUNTIME_TITLE = 'Archive Manual Fallback Receipt'
RUNTIME_STAGE = 'archive_manual_fallback_receipt'
...
class ArchiveManualFallbackReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ArchiveManualFallbackReceiptRecord:
    ...
    return ArchiveManualFallbackReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ArchiveManualFallbackReceiptRecord:
    return build_record(
        "archive_manual_fallback_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "archive_manual_fallback_receipt"},
    )


def summarise_record(record: ArchiveManualFallbackReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ArchiveManualFallbackReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ArchiveManualFallbackReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: ArchiveManualFallbackReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ArchiveManualFallbackReceiptRecord:
