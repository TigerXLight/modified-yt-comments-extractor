...
RUNTIME_TITLE = 'Release Final Integrity Check'
RUNTIME_STAGE = 'release_final_integrity_check'
...
class ReleaseFinalIntegrityCheckRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ReleaseFinalIntegrityCheckRecord:
    ...
    return ReleaseFinalIntegrityCheckRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ReleaseFinalIntegrityCheckRecord:
    return build_record(
        "release_final_integrity_check-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "release_final_integrity_check"},
    )


def summarise_record(record: ReleaseFinalIntegrityCheckRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ReleaseFinalIntegrityCheckRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ReleaseFinalIntegrityCheckRecord:
    return build_record(
...
def write_record(path: str | Path, record: ReleaseFinalIntegrityCheckRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ReleaseFinalIntegrityCheckRecord:
