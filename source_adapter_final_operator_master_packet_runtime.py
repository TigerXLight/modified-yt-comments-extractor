...
RUNTIME_TITLE = 'Final Operator Master Packet'
RUNTIME_STAGE = 'final_operator_master_packet'
...
class FinalOperatorMasterPacketRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> FinalOperatorMasterPacketRecord:
    ...
    return FinalOperatorMasterPacketRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> FinalOperatorMasterPacketRecord:
    return build_record(
        "final_operator_master_packet-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "final_operator_master_packet"},
    )


def summarise_record(record: FinalOperatorMasterPacketRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: FinalOperatorMasterPacketRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> FinalOperatorMasterPacketRecord:
    return build_record(
...
def write_record(path: str | Path, record: FinalOperatorMasterPacketRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> FinalOperatorMasterPacketRecord:
