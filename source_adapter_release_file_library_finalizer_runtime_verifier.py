...
from source_adapter_release_file_library_finalizer_runtime import RUNTIME_STAGE, ReleaseFileLibraryFinalizerRecord


def verify_record(record: ReleaseFileLibraryFinalizerRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[ReleaseFileLibraryFinalizerRecord]) -> list[str]:
    issues: list[str] = []
