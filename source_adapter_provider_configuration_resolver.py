...
SCHEMA_VERSION = "source_adapter_provider_configuration_resolver_v1"
STATUS = "SOURCE_ADAPTER_PROVIDER_CONFIGURATION_RESOLVER_BUILT"
HANDOFF_STATUS = "SOURCE_ADAPTER_PROVIDER_CONFIGURATION_READY_FOR_DRIVER_BINDING"

def _config_rows() -> list[dict[str, Any]]:
    rows=[]
    for index, action in enumerate(PROVIDER_ACTIONS):
        payload={"provider_action": action, "profile": f"operator.{action}.default"}
        rows.append({"schema_version":"source_adapter_provider_configuration_row_v1","row_index":index,"provider_configuration_row_id":stable_id("source_adapter.provider_configuration",payload),"provider_action":action,"provider_profile_id":f"source_adapter.profile.{action}","backend_module":f"source_adapter_{action}_backend".replace("credential_lookup","keys_accounts_lookup"),"configuration_status":"RESOLVED_FOR_OPERATOR_PROFILE","supports_live_execution":True,"supports_local_receipts":True,"operator_input_required": action in {"browser_capture","archive_submit"},"secrets_stored_in_row":False,"credential_reference_required": action == "credential_lookup","keys_accounts_label":KEYS_ACCOUNTS_LABEL})
    return rows

def build_source_adapter_provider_configuration_resolver() -> dict[str, Any]:
    upstream=example_full_execution_integration_bundle_closeout_package()
    rows=_config_rows()
    handoff={"schema_version":"source_adapter_provider_configuration_handoff_v1","handoff_status":HANDOFF_STATUS,"provider_configuration_row_count":len(rows),"upstream_handoff_status":upstream["handoff"]["handoff_status"],"keys_accounts_label":KEYS_ACCOUNTS_LABEL,"required_next_stage":"browser_driver_archive_release_and_library_backend_binding"}
    return {"schema_version":SCHEMA_VERSION,"id":stable_id("source_adapter.provider_configuration_resolver",rows),"status":STATUS,"provider_configuration_row_count":len(rows),"provider_configuration_rows":rows,"handoff":handoff,"operator_summary":{"schema_version":"source_adapter_provider_configuration_operator_summary_v1","status":STATUS,"provider_configuration_row_count":len(rows),"live_capable_count":sum(1 for r in rows if r["supports_live_execution"]),"keys_accounts_label":KEYS_ACCOUNTS_LABEL},"issue_count":0,"issues":[]}

def example_source_adapter_provider_configuration_resolver_package() -> dict[str, Any]:
    return build_source_adapter_provider_configuration_resolver()
