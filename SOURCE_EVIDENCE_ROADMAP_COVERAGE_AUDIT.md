...
| API/cloud ASR provider credentials | Rows 2A, 2B, 2C1, and 2C2 implemented within bounded boundaries | `ACCESS_KEYS_MANAGER_SPEC.md`; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; focused tests; Access & Keys metadata/dialog; narrow `main.py` wiring | Stable credential IDs and security policy are implemented together with read-only runtime status overlays, a secure credential-store abstraction for catalogued cloud-ASR credential IDs, and masked cloud-ASR Save/Clear controls. YouTube reports only boolean presence plus safe existing-storage provenance and remains excluded from the new secure-store path. Row 2C2 invokes the secure store only for explicit cloud-ASR Save/Clear and safe presence probing; no provider consumes stored credentials. The credential entry is masked from widget creation onward, does not preload stored values, clears after Save/Clear paths, and uses fixed non-secret result/status messages. Environment-variable presence reporting remains supported, keyring/environment precedence is deterministic, and clearing secure-store state does not hide a still-configured environment credential. The initial plaintext cloud-ASR field defect was corrected before commit and the corrected manual retest passed. | Remaining credential work is unresolved and separately approval-gated: existing YouTube migration/legacy plaintext cleanup, any provider credential consumption, connection testing, OAuth/browser flows, provider/API calls, cloud ASR execution, uploads, and network behavior | Yes; extend the row 2A/2B/2C1/2C2 architecture rather than bypassing it |
...
| Most important next milestone | Gap remains after row 2C2 | Remaining row 2C credential lifecycle boundary; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py` | Rows 2A, 2B, 2C1, and 2C2 are implemented: non-secret policy, read-only local status/provenance overlays, secure-store infrastructure, and masked cloud-ASR Save/Clear UI with safe presence refresh. Stored cloud-ASR credentials are not consumed by providers, and the existing YouTube workflow is still excluded from the new secure-store path. | Obtain separate explicit security approval before YouTube credential migration/legacy plaintext cleanup, provider credential consumption, connection testing, OAuth/browser flows, provider/API calls, cloud ASR execution, uploads, or network behavior | Do not treat row 2C2 as approval for later credential layers or skip to later rows |
...
## Ordered Audit Progress At `97de48d`
...
- Remaining credential work after row 2C2 is separately approval-gated: existing YouTube migration/legacy plaintext cleanup, provider credential consumption, connection testing, OAuth/browser flows, provider/API calls, cloud ASR execution, uploads, and network behavior.
...
- Existing YouTube credential migration, legacy plaintext cleanup, and any cleanup/removal of the legacy YouTube plaintext fallback.
...
Row 1 bounded GUI presentation, row 2A non-secret credential architecture/audit, row 2B read-only local credential status integration, row 2C1 backend-only secure credential-store infrastructure, and row 2C2 masked cloud-ASR Save/Clear UI are complete.

The next ordered credential boundary is the remaining row 2C credential lifecycle work: existing YouTube credential migration, legacy plaintext cleanup, and any cleanup/removal of the legacy YouTube plaintext fallback. The current roadmap documents do not define a precise `row 2C3` label, so this remains the next separately approved credential boundary rather than an implicitly numbered milestone.

That boundary must not silently expand into provider credential consumption, connection tests, provider calls, OAuth, cloud ASR execution, browser access, uploads, or network behavior. Those remain later separate approval boundaries. Do not skip to later roadmap rows.
