...
| API/cloud ASR provider credentials | Rows 2A, 2B, 2C1, 2C2, the approved YouTube credential-migration boundary, and the local-only cloud-ASR credential-consumption prerequisite are implemented within bounded boundaries | `ACCESS_KEYS_MANAGER_SPEC.md`; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; `credential_consumption.py`; `youtube_credential_migration.py`; focused tests; Access & Keys metadata/dialog; narrow `main.py` wiring | Stable credential IDs and security policy are implemented together with read-only runtime status overlays, a secure credential-store abstraction for catalogued cloud-ASR credential IDs, masked cloud-ASR Save/Clear controls, explicit secure YouTube credential migration/legacy cleanup, and local-only action-time cloud-ASR credential resolution through a trusted internal callback. Cloud-ASR consumption uses the same secure-store-then-environment precedence as safe status reporting: a present non-empty secure value wins; a missing secure value may fall back to a non-empty environment value; backend unavailable/error may still use the environment fallback already supported by the status layer; an empty or whitespace-only secure value is an invalid secure state, does not invoke the callback, and does not fall back to environment. Public consumption results contain only fixed statuses/provenance/diagnostics and never return credential values or callback returns. The callback is trusted internal action code, not a security sandbox. YouTube new/updated saves are secure-only with no plaintext fallback; legacy plaintext migration is explicit/user-controlled, uses preserve-before-delete ordering, and removes legacy plaintext only after secure save plus non-secret presence verification. Clear handles missing, secure-only, legacy-only, both-copy, unavailable/error, malformed settings, and partial-failure states truthfully. The YouTube entry is permanently masked, has no reveal/copy control, never preloads stored credentials, remains empty after startup/reopen/status refresh/migration/Clear, and resolves configured credentials internally only at existing action time. Row 2C2 still invokes the secure store only for explicit cloud-ASR Save/Clear and safe presence probing; no real cloud-ASR provider currently consumes credentials. Environment-variable presence reporting remains supported. The initial plaintext cloud-ASR field defect and later YouTube reveal/preload/partial-clear/security defects were corrected before their commits. | Remaining credential work is unresolved and separately approval-gated: wiring real provider actions through the consumption helper, connection testing, OAuth/browser flows, provider/API calls, cloud ASR execution, uploads, network behavior, and any future reveal/copy/export behavior | Yes; extend the existing row 2 credential architecture rather than bypassing it |
...
| Most important next milestone | Gap remains after `a1cf07d` | Next unresolved provider-action integration/testing boundary; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; `credential_consumption.py`; `youtube_credential_migration.py` | Rows 2A, 2B, 2C1, 2C2, the approved secure YouTube migration/legacy-cleanup boundary, and the local-only callback-based cloud-ASR credential-consumption prerequisite are implemented. Stored cloud-ASR credentials can now be resolved only for explicit trusted internal callbacks, using secure-store/environment precedence and invalid-empty semantics aligned with safe status reporting. No real provider consumes credentials, and no connection tests/provider calls exist. YouTube credential storage now saves securely only, supports explicit legacy migration/cleanup, keeps the GUI field empty on reopen, and preserves existing extraction through internal action-time credential resolution. | Obtain separate explicit security approval before wiring provider actions through the consumption helper, connection testing, provider/API calls, OAuth/browser flows, cloud ASR execution, uploads, network behavior, or any future reveal/copy/export behavior | Do not treat the consumption prerequisite closeout as approval for provider execution, connection tests, or later roadmap rows |
...
## Ordered Audit Progress At `a1cf07d`
...
- Remaining credential work after `a1cf07d` is separately approval-gated: wiring real provider actions through the consumption helper, connection testing, OAuth/browser flows, provider/API calls, cloud ASR execution, uploads, network behavior, and any future reveal/copy/export behavior.
...
Row 1 bounded GUI presentation, row 2A non-secret credential architecture/audit, row 2B read-only local credential status integration, row 2C1 backend-only secure credential-store infrastructure, row 2C2 masked cloud-ASR Save/Clear UI, the approved secure YouTube credential migration/legacy-cleanup boundary, and the local-only cloud-ASR credential-consumption prerequisite are complete.

The next ordered credential boundary is wiring real provider actions through the trusted consumption helper and/or explicit connection testing for stored credentials. The current roadmap documents do not define a precise row label for that boundary, so it must remain a separately approved provider-action/connection-testing milestone rather than an implicitly numbered row.
