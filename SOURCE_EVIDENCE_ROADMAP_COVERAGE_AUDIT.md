...
- ReplayWeb.page / Local Web Archive closeout is covered by `source_local_web_archive_actions.py`, `source_local_web_archive_actions_test.py`, the offline `source_local_web_archive_repair_cli.py` repair path, and the derived fallback builder `source_replay_static_snapshot.py`. The action state detects optional configured ReplayWeb.page desktop viewers, parses official release metadata, builds safe argument-array Open archive and Show files previews, records the browser/PWA local-file chooser limitation, and verifies WACZ/manifest content without launching a viewer by default. Replay investigation of the accepted MSN WACZ found a real standards/replay-index defect: the underlying completed comments evidence remains accepted at `25/25` (`16` roots plus `9` replies), but the original WACZ used pre-1.2 profile `data-package`, retained removed WACZ 1.2 fields, and its `87` CDXJ rows were unsorted and not consistently canonical lower-case searchable URL keys. Commit `c3eb5e9` fixed package readiness, normalized WARC request lines, hosted ReplayWeb.page profile handling, sorted/canonical CDXJ readiness, and status wording as `REPLAYWEB_PAGE_PACKAGE_READY`, not visual compatibility. Commit `eeba5bb` added a static WACZ evidence-view entry; manual testing confirmed direct static evidence HTML and raw static evidence WARC.GZ are usable, but the WACZ static page was listed and still returned `Archived Page Not Found` from WACZ lookup. The evidence-report fallback is standalone no-JavaScript `STATIC_EVIDENCE_HTML_READY` output plus raw static-only `STATIC_EVIDENCE_WARC_READY` WARC/WARC.GZ outputs carrying source URL, integrity, comments summary, and runtime-limitation text without MSN scripts/resources. The initial static archived page view worked technically but was too report-like; the current implementation now adds a darker MSN/mobile-style `STATIC_PAGE_VIEW_HTML_READY` / `STATIC_PAGE_VIEW_WARC_READY` reconstruction with selected local image assets copied to relative `assets/` paths, plus `STATIC_TEXT_VIEW_HTML_READY` / `STATIC_TEXT_VIEW_WARC_READY`, article/comments/page TXT exports, and a Markdown export for structured collapsible reading/copying. These remain derived local evidence views with `STATIC_PAGE_VIEW_VISUAL_FIDELITY_PARTIAL`; they do not rerun MSN, load remote runtime, overwrite accepted WACZ/WARC, or fix/verify the original dynamic ReplayWeb runtime. The derived WACZ is retained as package/listing output only, and no `REPLAY_VISUALLY_VERIFIED`, WACZ visual success, dynamic runtime fix, or comments-visually-replayable claim is made.
