...
Checkpoint: `f21e578 Add ElevenLabs Scribe v2 provider adapter`
...
| API/cloud ASR provider credentials | Rows 2A, 2B, 2C1, 2C2, the approved YouTube credential-migration boundary, the local-only cloud-ASR credential-consumption prerequisite, explicit provider-action coordinator seam, local-only connection-test coordinator seam, and ElevenLabs Scribe v2 provider adapter are implemented within bounded boundaries | `ACCESS_KEYS_MANAGER_SPEC.md`; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; `credential_consumption.py`; `asr_provider_action.py`; `asr_connection_test.py`; `elevenlabs_scribe_provider.py`; `youtube_credential_migration.py`; focused tests; Access & Keys metadata/dialog; narrow `main.py` wiring | Stable credential IDs and security policy are implemented together with read-only runtime status overlays, secure-store abstraction, masked cloud-ASR Save/Clear controls, secure YouTube migration/legacy cleanup, action-time credential resolution, injected provider-action and connection-test seams, and a provider-specific fake-transport-tested ElevenLabs Scribe v2 adapter. The Scribe adapter supports local-file synchronous batch request validation, conservative keyterm handling, response normalization, structured fake-error mapping, and file-handle closure through injected trusted transport only. It is not production-live and adds no production HTTP transport, SDK/client, live API request, real credential use, media upload, GUI transcription action, Test Connection wiring, background action, or network behavior. | Remaining provider work is separately approval-gated: production ElevenLabs transport, live manual verification with explicit user permission and real credentials kept out of prompts/tests, then later user-facing ASR action and Test Connection wiring | Yes; extend the existing row 2 credential architecture rather than bypassing it |
...
| Most important next milestone | Gap remains after `f21e578` | Next unresolved production transport/live-verification/user-facing action boundary; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; `credential_consumption.py`; `asr_provider_action.py`; `asr_connection_test.py`; `elevenlabs_scribe_provider.py`; `youtube_credential_migration.py` | Rows 2A, 2B, 2C1, 2C2, the approved secure YouTube migration/legacy-cleanup boundary, the local-only callback-based cloud-ASR credential-consumption prerequisite, the explicit provider-action coordinator seam, the local-only connection-test coordinator seam, and the ElevenLabs Scribe v2 fake-transport-tested adapter are implemented. The Scribe adapter is provider-specific but not production-live: no production HTTP transport, SDK/client, live API request, real credential use, media upload, GUI action, Test Connection wiring, or network behavior exists. | Obtain separate explicit security approval before production ElevenLabs transport, live manual verification, later user-facing ASR action, and later Test Connection wiring. Live provider/network access must remain explicit/user-triggered and fake/local tested before manual live use | Do not treat the fake-transport adapter closeout as approval for live provider execution, GUI transcription wiring, GUI Test Connection wiring, or later roadmap rows |
...
## Ordered Audit Progress At `f21e578`
...
- Remaining provider work after `f21e578` is separately approval-gated: production ElevenLabs transport, then live manual verification with explicit user permission and real credentials kept out of prompts/tests, then later user-facing ASR action and Test Connection wiring. OAuth/browser flows, broader provider/API calls, uploads, network behavior, and any future reveal/copy/export behavior remain separate boundaries.
...
- Production ElevenLabs transport, live manual verification, user-facing ASR action wiring, production connection testers/provider access, user-facing Test Connection wiring, OAuth, cloud ASR uploads/runs, and all network behavior.
...
Row 1 bounded GUI presentation, row 2A non-secret credential architecture/audit, row 2B read-only local credential status integration, row 2C1 backend-only secure credential-store infrastructure, row 2C2 masked cloud-ASR Save/Clear UI, the approved secure YouTube credential migration/legacy-cleanup boundary, the local-only cloud-ASR credential-consumption prerequisite, the explicit ASR provider-action coordinator seam, the local-only ASR connection-test coordinator seam, and the ElevenLabs Scribe v2 provider-specific fake-transport-tested adapter are complete.

The next ordered provider boundary is production ElevenLabs transport, then live manual verification with explicit user permission and real credentials kept out of prompts/tests, then later user-facing ASR action and Test Connection wiring. The current roadmap documents do not define a precise row label for that boundary, so it must remain separately approved rather than an implicitly numbered row.
