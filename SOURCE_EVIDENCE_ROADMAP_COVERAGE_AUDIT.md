...
| API/cloud ASR provider credentials | Rows 2A, 2B, 2C1, 2C2, the approved YouTube credential-migration boundary, the local-only cloud-ASR credential-consumption prerequisite, and the explicit provider-action coordinator seam are implemented within bounded boundaries | `ACCESS_KEYS_MANAGER_SPEC.md`; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; `credential_consumption.py`; `asr_provider_action.py`; `youtube_credential_migration.py`; focused tests; Access & Keys metadata/dialog; narrow `main.py` wiring | Stable credential IDs and security policy are implemented together with read-only runtime status overlays, a secure credential-store abstraction for catalogued cloud-ASR credential IDs, masked cloud-ASR Save/Clear controls, explicit secure YouTube credential migration/legacy cleanup, local-only action-time cloud-ASR credential resolution through a trusted internal callback, and an explicit local provider-action coordinator seam. Outcome B applied for `058af01`: no tracked safe production cloud-ASR provider action path existed, the existing tracked runtime remained local `faster-whisper` / `whisper.cpp`, and `ASRProviderActionCoordinator.dispatch_provider_action(...)` now validates provider/action metadata, delegates credential resolution to `CloudASRCredentialConsumer`, and invokes only injected trusted executors. `elevenlabs_scribe` and `whisper_cpp_vulkan_large_v3_turbo` are dispatchable through an injected trusted executor only; this is not a production cloud-provider implementation. Unknown, pattern-adjacent, YouTube, unsupported-action, non-dispatchable-provider, and missing-executor paths fail before credential lookup. The trusted executor is internal trusted code, not a sandbox; the coordinator does not return or retain credential values or executor returns, but a malicious executor could retain a supplied string by side effect. Public action results contain only non-secret provider/action/status/provenance fields and exclude credential IDs, values, fragments, lengths, hashes, provider responses, transcripts, raw exceptions, tracebacks, payloads, headers, and audio paths/content. No connection test, SDK/client, credential validation probe, account/quota/list-models call, live API request, audio open/upload, network behavior, GUI wiring, background action, credential write/delete, environment write, YouTube behavior change, cloud-ASR Save/Clear/status behavior change, or source/export behavior change exists. | Remaining credential work is unresolved and separately approval-gated: explicit connection testing and/or a first real provider implementation. Any connection test must be explicit/user-triggered and any real provider integration must remain bounded, fake-tested first, and separately approved before provider/API calls, uploads, OAuth/browser flows, network behavior, or reveal/copy/export behavior | Yes; extend the existing row 2 credential architecture rather than bypassing it |
...
| Most important next milestone | Gap remains after `058af01` | Next unresolved provider connection/implementation boundary; `CREDENTIAL_SECURITY_AUDIT.md`; `credential_architecture.py`; `credential_runtime_status.py`; `credential_store.py`; `credential_consumption.py`; `asr_provider_action.py`; `youtube_credential_migration.py` | Rows 2A, 2B, 2C1, 2C2, the approved secure YouTube migration/legacy-cleanup boundary, the local-only callback-based cloud-ASR credential-consumption prerequisite, and the explicit provider-action coordinator seam are implemented. Stored cloud-ASR credentials can now be resolved only for explicit trusted internal callbacks, and dispatch can reach only injected trusted executors after provider/action validation and credential resolution. No real provider implementation, connection test, provider client, SDK, API call, audio upload/open, network behavior, GUI wiring, or background action exists. YouTube credential storage now saves securely only, supports explicit legacy migration/cleanup, keeps the GUI field empty on reopen, and preserves existing extraction through internal action-time credential resolution. | Obtain separate explicit security approval before explicit connection testing and/or a first real provider implementation. Connection testing must remain explicit/user-triggered and a real provider implementation must be bounded and fake-tested before live access | Do not treat the provider-action seam closeout as approval for connection tests, live provider execution, or later roadmap rows |
...
## Ordered Audit Progress At `058af01`
...
- Remaining credential work after `058af01` is separately approval-gated: explicit connection testing and/or a first real provider implementation, plus OAuth/browser flows, provider/API calls, cloud ASR execution, uploads, network behavior, and any future reveal/copy/export behavior.
...
Row 1 bounded GUI presentation, row 2A non-secret credential architecture/audit, row 2B read-only local credential status integration, row 2C1 backend-only secure credential-store infrastructure, row 2C2 masked cloud-ASR Save/Clear UI, the approved secure YouTube credential migration/legacy-cleanup boundary, the local-only cloud-ASR credential-consumption prerequisite, and the explicit ASR provider-action coordinator seam are complete.

The next ordered credential boundary is explicit connection testing and/or a first real provider implementation. The current roadmap documents do not define a precise row label for that boundary, so it must remain a separately approved connection-testing/provider-implementation milestone rather than an implicitly numbered row.
