...
from source_adapter_evidence_queue_release_dispatch_runtime import RUNTIME_STAGE, EvidenceQueueReleaseDispatchRecord


def verify_record(record: EvidenceQueueReleaseDispatchRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[EvidenceQueueReleaseDispatchRecord]) -> list[str]:
    issues: list[str] = []
