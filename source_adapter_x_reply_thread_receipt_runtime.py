...
RUNTIME_TITLE = 'X Reply Thread Receipt'
RUNTIME_STAGE = 'x_reply_thread_receipt'
...
class XReplyThreadReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    return XReplyThreadReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> XReplyThreadReceiptRecord:
    return build_record(
        "x_reply_thread_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "x_reply_thread_receipt"},
    )


def summarise_record(record: XReplyThreadReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: XReplyThreadReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> XReplyThreadReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: XReplyThreadReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> XReplyThreadReceiptRecord:
