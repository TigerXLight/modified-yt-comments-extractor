...
RUNTIME_TITLE = 'Release File Library Finalizer'
RUNTIME_STAGE = 'release_file_library_finalizer'
...
class ReleaseFileLibraryFinalizerRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ReleaseFileLibraryFinalizerRecord:
    ...
    return ReleaseFileLibraryFinalizerRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ReleaseFileLibraryFinalizerRecord:
    return build_record(
        "release_file_library_finalizer-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "release_file_library_finalizer"},
    )


def summarise_record(record: ReleaseFileLibraryFinalizerRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ReleaseFileLibraryFinalizerRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ReleaseFileLibraryFinalizerRecord:
    return build_record(
...
def write_record(path: str | Path, record: ReleaseFileLibraryFinalizerRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ReleaseFileLibraryFinalizerRecord:
