...
RUNTIME_TITLE = 'KEYS Accounts Search Scope Guard'
RUNTIME_STAGE = 'keys_accounts_search_scope_guard'
...
class KeysAccountsSearchScopeGuardRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> KeysAccountsSearchScopeGuardRecord:
    ...
    return KeysAccountsSearchScopeGuardRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> KeysAccountsSearchScopeGuardRecord:
    return build_record(
        "keys_accounts_search_scope_guard-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "keys_accounts_search_scope_guard"},
    )


def summarise_record(record: KeysAccountsSearchScopeGuardRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: KeysAccountsSearchScopeGuardRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> KeysAccountsSearchScopeGuardRecord:
    return build_record(
...
def write_record(path: str | Path, record: KeysAccountsSearchScopeGuardRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> KeysAccountsSearchScopeGuardRecord:
