...
RUNTIME_TITLE = 'Archive Polling Outcome Receipt'
RUNTIME_STAGE = 'archive_polling_outcome_receipt'
...
class ArchivePollingOutcomeReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ArchivePollingOutcomeReceiptRecord:
    ...
    return ArchivePollingOutcomeReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ArchivePollingOutcomeReceiptRecord:
    return build_record(
        "archive_polling_outcome_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "archive_polling_outcome_receipt"},
    )


def summarise_record(record: ArchivePollingOutcomeReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ArchivePollingOutcomeReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ArchivePollingOutcomeReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: ArchivePollingOutcomeReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ArchivePollingOutcomeReceiptRecord:
