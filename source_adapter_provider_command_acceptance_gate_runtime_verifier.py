...
from source_adapter_provider_command_acceptance_gate_runtime import RUNTIME_STAGE, ProviderCommandAcceptanceGateRecord


def verify_record(record: ProviderCommandAcceptanceGateRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[ProviderCommandAcceptanceGateRecord]) -> list[str]:
    issues: list[str] = []
