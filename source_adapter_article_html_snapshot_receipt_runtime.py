...
RUNTIME_TITLE = 'Article HTML Snapshot Receipt'
RUNTIME_STAGE = 'article_html_snapshot_receipt'
...
class ArticleHtmlSnapshotReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ArticleHtmlSnapshotReceiptRecord:
    ...
    return ArticleHtmlSnapshotReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ArticleHtmlSnapshotReceiptRecord:
    return build_record(
        "article_html_snapshot_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "article_html_snapshot_receipt"},
    )


def summarise_record(record: ArticleHtmlSnapshotReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ArticleHtmlSnapshotReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ArticleHtmlSnapshotReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: ArticleHtmlSnapshotReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ArticleHtmlSnapshotReceiptRecord:
