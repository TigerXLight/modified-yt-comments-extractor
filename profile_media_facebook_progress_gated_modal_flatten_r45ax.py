...
CONTRACT = {
    ...
    "r45bs_turbo_burst_rule": "R45BS adds --speed-profile turbo_burst as the default. It freezes all exact safe visible expansion coordinates inside the active comments scroller, fires through them using CDP Input.dispatchMouseEvent before waiting for Facebook materialization, then rescans once. It preserves target-story drift, same-tab, overlay, expected-total, and visible-page-only safety gates.",
}
...
function r45axExpectedTotalEvidence(expected){
  const total = parseInt(expected || 0, 10);
  if (!Number.isFinite(total) || total <= 0) return {expected_total: total || 0, found: false, reason: 'no_expected_total'};
  const patterns = [
    new RegExp('\\b\\d{1,5}\\s+of\\s+' + total + '\\b', 'i'),
    new RegExp('\\b' + total + '\\b.{0,40}\\b(?:comments?|replies)\\b', 'i'),
    new RegExp('\\b(?:comments?|replies)\\b.{0,40}\\b' + total + '\\b', 'i')
  ];
  const sources = [];
  const scroller = window.__R45AX_SCROLLER__;
  if (scroller) sources.push({name:'scroller_inner_text', text:scroller.innerText || ''}, {name:'scroller_text_content', text:scroller.textContent || ''});
  sources.push({name:'body_inner_text', text:document.body ? (document.body.innerText || '') : ''});
  sources.push({name:'body_text_content', text:document.body ? (document.body.textContent || '') : ''});
  try {
    sources.push({name:'aria_title_attributes', text:Array.from(document.querySelectorAll('[aria-label],[title]')).slice(0,5000).map(el => (el.getAttribute('aria-label') || '') + ' ' + (el.getAttribute('title') || '')).join(' ')});
  } catch(e) {}
  try {
    sources.push({name:'stripped_html', text:String(document.documentElement && document.documentElement.innerHTML || '').replace(/<[^>]+>/g, ' ').replace(/&nbsp;|&#160;|&amp;nbsp;/gi, ' ').replace(/\s+/g, ' ')});
  } catch(e) {}
  for (const src of sources) {
    ...
    for (const rx of patterns) {
      ...
      if (m) {
        ...
        return {expected_total: total, found: true, source: src.name, match: m[0], snippet: text.slice(idx, Math.min(text.length, idx + 260)).replace(/\s+/g, ' ').trim()};
      }
    }
  }
  return {expected_total: total, found: false, searched_sources: sources.map(s => s.name)};
}
...
function r45axVisibleSortFilterState(){
  const out = [];
  const rx = /\b(Most relevant|All comments|Newest|Top comments|Comment ranking|Relevant|filter|sort)\b/i;
  for (const el of Array.from(document.querySelectorAll('[role="button"], button, a, span, div')).slice(0, 12000)){
    if (r45axElementHidden(el)) continue;
    ...
    if (!text || text.length > 180 || !rx.test(text)) continue;
    const r = el.getBoundingClientRect();
    if (r.width < 6 || r.height < 6 || r.bottom < 0 || r.top > innerHeight || r.right < 0 || r.left > innerWidth) continue;
    out.push({text:text.slice(0,120), tag:el.tagName, role:el.getAttribute && (el.getAttribute('role') || ''), rect:{left:Math.round(r.left), top:Math.round(r.top), right:Math.round(r.right), bottom:Math.round(r.bottom), width:Math.round(r.width), height:Math.round(r.height)}});
  }
  return out.slice(0, 25);
}
function r45axBottomMaterializationSnapshot(expected){
  const scroller = r45axGetScroller();
  if (!scroller) return {ok:false, reason:'no_scroller'};
  const scan = r45axScanVisible();
  const text = String(scroller.innerText || scroller.textContent || '');
  const norm = r45axNorm(text);
  const hiddenMatches = norm.match(/\bHidden by Facebook\b/g) || [];
  return {
    ok:true,
    scrollTop:scroller.scrollTop || 0,
    scrollHeight:scroller.scrollHeight || 0,
    clientHeight:scroller.clientHeight || 0,
    bottomGap:Math.max(0, (scroller.scrollHeight || 0) - ((scroller.scrollTop || 0) + (scroller.clientHeight || 0))),
    scan,
    progress:r45axProgressFromPage(),
    expectedTotalEvidence: expected ? r45axExpectedTotalEvidence(expected) : null,
    hiddenByFacebookCount:hiddenMatches.length,
    bottomTextTail:norm.slice(Math.max(0, norm.length - 1800)),
    sortFilterState:r45axVisibleSortFilterState()
  };
}
async function r45axFinalBottomMaterializationProbe(expected, cycles){
  const scroller = r45axGetScroller();
  if (!scroller) return {ok:false, reason:'no_scroller'};
  const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
  const snapshots = [];
  const maxCycles = Math.max(1, Math.min(12, parseInt(cycles || 6, 10) || 6));
  for (let i=0; i<maxCycles; i++){
    const client = scroller.clientHeight || 600;
    const wiggle = Math.min(Math.max(180, Math.floor(client * (i % 2 ? 0.32 : 0.18))), 520);
    const before = scroller.scrollTop || 0;
    scroller.scrollTop = Math.max(0, (scroller.scrollHeight || 0) - client - wiggle);
    try { scroller.dispatchEvent(new WheelEvent('wheel', {deltaY:-wiggle, bubbles:true, cancelable:true, clientX:Math.max(20, scroller.getBoundingClientRect().left + 40), clientY:Math.max(20, scroller.getBoundingClientRect().bottom - 80)})); } catch(e) {}
    await sleep(650 + (i % 2) * 250);
    scroller.scrollTop = scroller.scrollHeight || scroller.scrollTop || 0;
    try { scroller.dispatchEvent(new WheelEvent('wheel', {deltaY:Math.max(260, wiggle), bubbles:true, cancelable:true, clientX:Math.max(20, scroller.getBoundingClientRect().left + 40), clientY:Math.max(20, scroller.getBoundingClientRect().bottom - 80)})); } catch(e) {}
    await sleep(950);
    const snap = r45axBottomMaterializationSnapshot(expected);
    snap.cycle = i + 1;
    snap.beforeScrollTop = before;
    snapshots.push(snap);
    if (snap.scan && snap.scan.total > 0) return {ok:true, foundCandidates:true, cycles:i+1, snapshots, finalSnapshot:snap};
  }
  const finalSnapshot = snapshots.length ? snapshots[snapshots.length - 1] : r45axBottomMaterializationSnapshot(expected);
  return {ok:true, foundCandidates:false, cycles:maxCycles, snapshots, finalSnapshot};
}
...
async def self_test(output_root: Path) -> int:
    ...
    result = {
        ...
        'checks': [
            ...
            {'name':'r45bp_final_bottom_materialization_probe_present','status':'pass' if 'r45axFinalBottomMaterializationProbe' in EXPAND_PATTERNS_JS and 'R45AX_FINAL_BOTTOM_MATERIALIZATION_PROBE' in Path(__file__).read_text(encoding='utf-8') else 'fail'},
            {'name':'r45bp_final_bottom_diagnostics_present','status':'pass' if 'hiddenByFacebookCount' in EXPAND_PATTERNS_JS and 'sortFilterState' in EXPAND_PATTERNS_JS and 'final_bottom_materialization_result' in Path(__file__).read_text(encoding='utf-8') else 'fail'},
            {'name':'speed_profile_safe_fast_present','status':'pass' if '--speed-profile' in Path(__file__).read_text(encoding='utf-8') and 'safe_fast' in Path(__file__).read_text(encoding='utf-8') else 'fail'},
            ...
            {'name':'r45ax_turbo_visible_speed_profile_present','status':'pass' if "choices=['turbo_burst','turbo_visible','safe_fast','strict']" in Path(__file__).read_text(encoding='utf-8') and "default='turbo_burst'" in Path(__file__).read_text(encoding='utf-8') else 'fail'},
...
def r45ax_active_window_keepalive(title_hint: str = '') -> Dict[str, Any]:
    """Best-effort Windows foreground helper; never clicks the page."""
    try:
        import ctypes
        from ctypes import wintypes
    except Exception as e:
        return {'ok': False, 'method': 'ctypes.SetForegroundWindow', 'reason': 'ctypes_unavailable', 'error': repr(e)}
    try:
        user32 = ctypes.windll.user32
    except Exception as e:
        return {'ok': False, 'method': 'ctypes.SetForegroundWindow', 'reason': 'user32_unavailable', 'error': repr(e)}
    hint = re.sub(r'\s+', ' ', str(title_hint or '').strip()).lower()
    if len(hint) > 80:
        hint = hint[:80]
    ...
    def _enum(hwnd, _lparam):  # type: ignore[no-untyped-def]
        ...
        try:
            ...
            title = str(buff.value or '')
            low = title.lower()
            if 'chrom' not in low and 'facebook' not in low:
                return True
            matched = bool(hint and (hint in low or low in hint))
            if matched or ('facebook' in low and 'chrom' in low):
                found.append({'title': title[:120], 'matched_title_hint': matched})
                if best_hwnd is None or matched:
                    best_hwnd = hwnd
            return True
        except Exception:
            return True
    ...
    try:
        ...
        if not best_hwnd:
            return {'ok': False, 'method': 'ctypes.SetForegroundWindow', 'reason': 'no_matching_window', 'matches': found[:5]}
        try:
            user32.ShowWindow(best_hwnd, 5)  # SW_SHOW
        except Exception:
            pass
        ok = bool(user32.SetForegroundWindow(best_hwnd))
        return {'ok': ok, 'method': 'ctypes.SetForegroundWindow', 'matches': found[:5]}
    except Exception as e:
        return {'ok': False, 'method': 'ctypes.SetForegroundWindow', 'reason': 'foreground_failed', 'error': repr(e), 'matches': found[:5]}

def r45ax_turbo_burst_items(scan: Dict[str, Any], max_clicks: int = 24) -> List[Dict[str, Any]]:
    items: List[Dict[str, Any]] = []
    seen_points = set()
    cdp_burst_categories = {'view_hidden', 'view_all_replies', 'view_n_replies', 'replied_bucket'}
    for item in list((scan or {}).get('items') or []):
        category = str(item.get('category') or '')
        if not item.get('safe') or category not in cdp_burst_categories:
            continue
        try:
            ...
            continue
        point_key = (round(x / 3), round(y / 3), category)
        if point_key in seen_points:
            continue
        seen_points.add(point_key)
        items.append({**item, 'x': x, 'y': y})
    items.sort(key=lambda it: (int(it.get('y') or 0), int(it.get('x') or 0)), reverse=True)
    return items[:max(1, min(30, int(max_clicks or 24)))]

async def r45ax_cdp_turbo_burst(page, cdp_session: Any, scan: Dict[str, Any], speed_profile: str, max_clicks: int = 24) -> Dict[str, Any]:
    """Burst-click frozen exact safe visible expansion points with CDP mouse events."""
    if speed_profile != 'turbo_burst':
        return {'ok': False, 'reason': 'speed_profile_not_turbo_burst'}
    items = r45ax_turbo_burst_items(scan, max_clicks=max_clicks)
    if not items:
        return {'ok': False, 'reason': 'no_safe_burst_items', 'burst_candidates': 0}
    validate_payload = [{'x': it.get('x'), 'y': it.get('y'), 'category': it.get('category'), 'label': it.get('label')} for it in items]
    validation = await page.evaluate(
        """(items) => {
          const scroller = r45axGetScroller();
          if (!scroller) return {ok:false, reason:'no_scroller', items:[]};
          const out = [];
    ...
    before_sig = {
        ...
        'progress': (before or {}).get('progress'),
    }
    ...
    for item in valid:
        ...
        try:
            await cdp_session.send('Input.dispatchMouseEvent', {'type': 'mouseMoved', 'x': x, 'y': y, 'button': 'none', 'buttons': 0})
            await cdp_session.send('Input.dispatchMouseEvent', {'type': 'mousePressed', 'x': x, 'y': y, 'button': 'left', 'buttons': 1, 'clickCount': 1})
            await cdp_session.send('Input.dispatchMouseEvent', {'type': 'mouseReleased', 'x': x, 'y': y, 'button': 'left', 'buttons': 0, 'clickCount': 1})
            clicked_items.append({'label': item.get('label'), 'category': item.get('category'), 'x': int(x), 'y': int(y), 'key': item.get('key')})
        except Exception as e:
            return {'ok': False, 'reason': 'cdp_dispatch_failed', 'error': repr(e), 'clicked': len(clicked_items), 'items': clicked_items[:12], 'burst_candidates': len(items)}
    dispatch_elapsed_ms = int(round((time.monotonic() - t0) * 1000))
    await page.wait_for_timeout(130 if len(clicked_items) >= 5 else 90)
    await r45ax_park_mouse(page)
    ...
    after_sig = {
        ...
        'progress': (after or {}).get('progress'),
    }
    ...
    return {
        'ok': True,
        'method': 'cdp_input_dispatch_mouse_event',
        'burst_candidates': len(items),
        'burst_clicked': len(clicked_items),
        ...
        'burst_scrollHeight_delta': after_sig['scrollHeight'] - before_sig['scrollHeight'],
        'burst_elapsed_ms': dispatch_elapsed_ms,
        'burst_clicks_per_second': round(len(clicked_items) / max(dispatch_elapsed_ms / 1000.0, 0.001), 2),
    }
...
async def r45ax_playwright_turbo_batch(page, scan: Dict[str, Any], speed_profile: str, max_clicks: int = 6) -> Dict[str, Any]:
    ...
    """
    if speed_profile not in {'turbo_visible', 'turbo_burst'}:
...
def r45ax_fast_safe_category(category: str) -> bool:
...
def r45ax_adaptive_click_wait_ms(label: str, category: str, speed_profile: str) -> int:
    if speed_profile not in {'safe_fast', 'turbo_visible', 'turbo_burst'} or not r45ax_fast_safe_category(category):
        return r45ax_click_wait_ms(label, category)
    m = re.search(r'\d+', label or '')
    n = int(m.group(0)) if m else 0
    if category == 'view_hidden':
        return 180
    if category in {'view_more', 'view_more_replies'}:
        return 240
    if category == 'view_n_replies':
        return 420 if n >= 30 else 220
    if category == 'view_all_replies':
        return 760 if n >= 100 else (430 if n >= 30 else 260)
    if category == 'replied_bucket':
        return 620 if n >= 100 else (340 if n >= 30 else 210)
    return 260
...
async def run_live(args: argparse.Namespace) -> int:
    ...
    async with async_playwright() as p:
        ...
        if getattr(args, 'keep_page_foreground', True):
            ...
            try:
                log('R45AX_ACTIVE_WINDOW_KEEPALIVE', r45ax_active_window_keepalive(await page.title()))
            except Exception as e:
                log('R45AX_ACTIVE_WINDOW_KEEPALIVE', {'ok': False, 'error': repr(e)})
...
window.r45axBottomMaterializationSnapshot = r45axBottomMaterializationSnapshot;
window.r45axFinalBottomMaterializationProbe = r45axFinalBottomMaterializationProbe;
...
return window.r45axInstallNavBlocker();
        ...
        def current_speed_metrics() -> Dict[str, Any]:
            ...
            return {
                ...
                'average_burst_batch_ms': round(burst_elapsed_total_ms / max(1, burst_batches), 1),
                'average_burst_click_ms': round(burst_elapsed_total_ms / max(1, burst_clicked), 1),
        ...
        for step in range(1, int(args.max_steps)+1):
            ...
            if speed_profile not in {'safe_fast', 'turbo_visible', 'turbo_burst'} or step <= 3 or step % 15 == 0:
            ...
            ignored_progress_text = str((prog_raw or {}).get('text') or '')
            if prog_raw and expected_total_comments > 0 and not prog and (ignored_progress_text != last_ignored_progress_text or step % 50 == 0):
                last_ignored_progress_text = ignored_progress_text
            ...
            if scan.get('total',0) > 0:
                if speed_profile in {'turbo_visible', 'turbo_burst'}:
                    ...
                    burst_result = None
                    burst_elapsed_ms = 0
                    burst_batch_clicked = 0
                    burst_materialized = False
                    if speed_profile == 'turbo_burst' and cdp_session is not None:
                        burst_t0 = time.monotonic()
                        log('R45AX_TURBO_BURST_START', {'step': step, 'visible_total': scan.get('total'), 'counts': scan.get('counts'), 'candidate_cap': 24})
                        burst_result = await r45ax_cdp_turbo_burst(page, cdp_session, scan, speed_profile, max_clicks=24)
                        burst_elapsed_ms = int(round((time.monotonic() - burst_t0) * 1000))
                    ...
                    if speed_profile == 'turbo_burst' and burst_batch_clicked > 0:
                        ...
                        if not after_target_guard.get('ok'):
                            target_drift_blocked += 1
                            receipt['status'] = 'BLOCKED_TARGET_DRIFT_AFTER_TURBO_BURST'
                            receipt['target_drift_guard'] = after_target_guard
                            receipt['last_turbo_burst'] = {'step': step, 'marker': 'R45AX_TURBO_BURST_CLICKED', 'result': burst_result}
                            log('R45AX_TARGET_DRIFT_BLOCKED', {'step': step, 'target_drift_blocked': target_drift_blocked, 'guard': after_target_guard, 'last_turbo_burst': receipt['last_turbo_burst']})
                            (run_dir/'r45ax_progress_gated_receipt.json').write_text(json.dumps(receipt, indent=2, ensure_ascii=False), encoding='utf-8')
                            await context.close()
                            return 5
                        ...
                        if int((after_side.get('messenger') or {}).get('count') or 0) > 0:
                            messenger_blocked += 1
                            close_result = await page.evaluate('r45axCloseMessengerOverlays()')
                            log('R45AX_MESSENGER_OVERLAY_BLOCKED', {'step': step, 'marker': 'R45AX_TURBO_BURST_CLICKED', 'messenger_blocked': messenger_blocked, 'after_side': after_side, 'close_result': close_result, 'burst_result': burst_result})
                            if int(((close_result or {}).get('after') or {}).get('count') or 0) > 0:
                                receipt['status'] = 'BLOCKED_MESSENGER_OVERLAY_STILL_OPEN'
                                receipt['messenger_overlay'] = {'after_side': after_side, 'close_result': close_result, 'burst_result': burst_result}
                        ...
                        if closed_new_pages:
                            log('R45AX_UNEXPECTED_PAGE_CLOSED', {'step': step, 'marker': 'R45AX_TURBO_BURST_CLICKED', 'closed_pages': closed_new_pages, 'new_pages_closed': new_pages_closed, 'burst_result': burst_result})
                        clicked += burst_batch_clicked
                        fast_clicks += burst_batch_clicked
                        turbo_clicks += burst_batch_clicked
                        burst_clicked += burst_batch_clicked
                        cdp_clicks += int((burst_result or {}).get('cdp_clicks') or burst_batch_clicked)
                        burst_batches += 1
                        turbo_batches += 1
                        burst_elapsed_total_ms += burst_elapsed_ms
                        turbo_elapsed_total_ms += burst_elapsed_ms
                        click_elapsed_total_ms += burst_elapsed_ms
                        pass_had_click = True
                        audit_had_click = True
                        after_prog_raw = ((burst_result or {}).get('after') or {}).get('progress')
                        ...
                        log('R45AX_TURBO_BURST_CLICKED', {
                            'step': step,
                            'clicked': clicked,
                            'burst_clicked': burst_batch_clicked,
                            'burst_candidates': (burst_result or {}).get('burst_candidates'),
                            'burst_elapsed_ms': burst_elapsed_ms,
                            'burst_clicks_per_second': round(burst_batch_clicked / max(burst_elapsed_ms / 1000.0, 0.001), 2),
                            'burst_materialized': burst_materialized,
                            'burst_scrollHeight_delta': (burst_result or {}).get('burst_scrollHeight_delta'),
                            'burst_visible_remaining': (burst_result or {}).get('visibleRemaining'),
                            'burst_progress_before': ((burst_result or {}).get('before') or {}).get('progress'),
                            'burst_progress_after': ((burst_result or {}).get('after') or {}).get('progress'),
                            'labels': [x.get('label') for x in ((burst_result or {}).get('items') or [])][:12],
                    ...
                    if coordinate_clicked > 0:
                        ...
                        if speed_profile == 'turbo_burst':
                            playwright_fallback_clicks += coordinate_clicked
                        turbo_batches += 1
                    ...
                    if turbo_batch_clicked > 0 and turbo_materialized:
                        ...
                        dom_clicks += turbo_batch_clicked
                        turbo_batches += 1
                ...
                if clickinfo.get('clicked') and item.get('safe'):
                    ...
                    fast_safe_click = bool(speed_profile in {'safe_fast', 'turbo_visible', 'turbo_burst'} and r45ax_fast_safe_category(category))
                    if fast_safe_click:
                        fast_clicks += 1
                    else:
                        slow_clicks += 1
                    run_expensive_click_checks = (not fast_safe_click) or clicked < 3 or fast_clicks % 12 == 0
                    before_side = await page.evaluate('r45axMessengerSideEffectState()') if run_expensive_click_checks else {'messenger': {'count': 0}, 'skipped': True, 'reason': 'safe_fast_periodic_check_not_due'}
                    await page.mouse.click(float(item.get('x')), float(item.get('y')))
                    await r45ax_park_mouse(page)
                    clicked += 1; pass_had_click = True; audit_had_click = True
                    wait_ms = r45ax_adaptive_click_wait_ms(label, category, speed_profile)
                    await page.wait_for_timeout(wait_ms)
                    if fast_safe_click and not run_expensive_click_checks:
                        profile_hover_result = {'skipped': True, 'reason': 'safe_fast_periodic_check_not_due'}
                    elif category == 'replied_bucket' and clicked % 4 != 0:
                    ...
                    if not after_target_guard.get('ok'):
                        ...
                        await context.close()
                        return 5
                    after_side = await page.evaluate('r45axMessengerSideEffectState()') if run_expensive_click_checks else {'messenger': {'count': 0}, 'skipped': True, 'reason': 'safe_fast_periodic_check_not_due'}
                    ...
                    after_prog = r45ax_filter_expected_progress(after_prog_raw, expected_total_comments)
                    ignored_after_progress_text = str((after_prog_raw or {}).get('text') or '')
                    if after_prog_raw and expected_total_comments > 0 and not after_prog and (ignored_after_progress_text != last_ignored_progress_text or step % 50 == 0):
                        last_ignored_progress_text = ignored_after_progress_text
                        log('R45AX_PROGRESS_IGNORED_EXPECTED_TOTAL_MISMATCH', {'step': step, 'expected_total_comments': expected_total_comments, 'ignored_progress': after_prog_raw})
                    ...
                    click_elapsed_total_ms += click_elapsed_ms
                    timing_delta = {
            ...
            if not at_bottom:
                scroll_mode = 'audit_fast' if audit_pass >= 2 and not audit_had_click and int(scan.get('total') or 0) == 0 else ('turbo_down' if speed_profile in {'turbo_visible', 'turbo_burst'} and int(scan.get('total') or 0) == 0 else 'down')
                ...
                if scroll_mode == 'audit_fast':
                    ...
                    hover_result = {'skipped': True, 'reason': 'r45ax_blank_audit_fast_scroll'}
                else:
                    hover_result = await r45ax_close_profile_hover_cards(page);
                ...
                if stalled_scroll_cycles >= 6:
                    ...
                    await context.close()
                    return 6
                await page.wait_for_timeout(60 if scroll_mode == 'audit_fast' else 180); continue
            if at_bottom and not progress_ok:
                ...
                if expected_total_comments > 0:
                    expected_total_evidence = await page.evaluate('(n) => r45axExpectedTotalEvidence(n)', expected_total_comments)
                log('R45AX_PROGRESS_GATE_NOT_SATISFIED', {'step':step, 'best_progress':best_progress, 'expected_total_comments': expected_total_comments, 'expected_total_evidence': expected_total_evidence, 'gate_status': r45ax_progress_gate_status(best_progress, expected_total_comments), 'stable_bottom_cycles':stable_bottom_cycles, 'scrollHeight':sh, 'audit_had_click': audit_had_click})
                if expected_total_comments > 0 and not audit_had_click and stable_bottom_cycles >= max(1, int(args.progress_stall_cycles)):
                    best_current = int((best_progress or {}).get('current') or 0)
                    near_expected_total = bool(best_current and (best_current >= expected_total_comments - 40 or best_current / max(1, expected_total_comments) >= 0.95))
                    if near_expected_total and final_bottom_materialization_runs < 2:
                        final_bottom_materialization_runs += 1
                        final_bottom_materialization_result = await page.evaluate('(args) => r45axFinalBottomMaterializationProbe(args.expected, args.cycles)', {'expected': expected_total_comments, 'cycles': 7})
                        log('R45AX_FINAL_BOTTOM_MATERIALIZATION_PROBE', {'step': step, 'run': final_bottom_materialization_runs, 'best_progress': best_progress, 'result': final_bottom_materialization_result})
                        final_snapshot = (final_bottom_materialization_result or {}).get('finalSnapshot') or {}
                        final_scan = final_snapshot.get('scan') or {}
                        final_prog_raw = final_snapshot.get('progress')
                        final_prog = r45ax_filter_expected_progress(final_prog_raw, expected_total_comments)
                        if final_prog and (not best_progress or (final_prog.get('total',0), final_prog.get('current',0)) >= (best_progress.get('total',0), best_progress.get('current',0))):
                            best_progress = final_prog
                        if int(final_scan.get('total') or 0) > 0:
                            stable_bottom_cycles = 0
                ...
                if stable_bottom_cycles >= int(args.progress_stall_cycles):
                    ...
                    audit_pass += 1; audit_had_click = False; pass_had_click = False; await page.evaluate('r45axScroll("top")'); await r45ax_park_mouse(page); await page.wait_for_timeout(400); continue
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument('--speed-profile', choices=['turbo_burst','turbo_visible','safe_fast','strict'], default='turbo_burst', help='turbo_burst fires CDP mouse events through frozen exact safe visible controls before settling; turbo_visible keeps the older batch/fallback path')
