...
from source_adapter_priority_fixture_pack_implementation import build_source_adapter_priority_fixture_pack_implementation
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build source adapter priority fixture pack implementation artifacts.")
    ...
    parser.add_argument("--fixture-pack-note", action="append", default=[])
    args = parser.parse_args(argv)
    input_package = _load_json(args.runtime_gui_provider_implementation_json) or example_runtime_gui_provider_implementation_package()
    package = build_source_adapter_priority_fixture_pack_implementation(
        input_package,
        operator_id=args.operator_id,
        fixture_pack_notes=args.fixture_pack_note,
    ).as_dict()
    if args.output_dir:
        result = store_source_adapter_priority_fixture_pack_implementation(package, args.output_dir)
    else:
        result = {"package": package, "verification": verify_source_adapter_priority_fixture_pack_implementation(package)}
