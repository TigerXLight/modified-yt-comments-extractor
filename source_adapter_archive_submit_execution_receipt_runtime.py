...
RUNTIME_TITLE = 'Archive Submit Execution Receipt'
RUNTIME_STAGE = 'archive_submit_execution_receipt'
...
class ArchiveSubmitExecutionReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ArchiveSubmitExecutionReceiptRecord:
    ...
    return ArchiveSubmitExecutionReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ArchiveSubmitExecutionReceiptRecord:
    return build_record(
        "archive_submit_execution_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "archive_submit_execution_receipt"},
    )


def summarise_record(record: ArchiveSubmitExecutionReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ArchiveSubmitExecutionReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ArchiveSubmitExecutionReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: ArchiveSubmitExecutionReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ArchiveSubmitExecutionReceiptRecord:
