...
py -3 "%~dp0r42bl_source_role_db.py" sync --root "%ROOT%" --payload "%PAYLOAD%" --changes "%CHANGES%" --db "%DB%" --summary "%SUMMARY%"
if errorlevel 1 python "%~dp0r42bl_source_role_db.py" sync --root "%ROOT%" --payload "%PAYLOAD%" --changes "%CHANGES%" --db "%DB%" --summary "%SUMMARY%"
endlocal
