# R42BI native WebView2 source-role editor process/database checkpoint

R42BI keeps the R42BG/R42BF speed architecture: native WebView2 warm server for runtime speed, JSON payload for first paint, and SQLite as the durable sidecar.  The database is for clean structure, history, current state, and maintenance; it is intentionally not in the first-paint path.
...
- Adds `maintain_r42bi_source_role_db.cmd`.
- Fixes maintenance size reporting after VACUUM by rechecking the DB file after the connection is reopened.
- Fixes the nullable `initialMode` C# build warning from R42BH.

## Database policy

The database should not slow down the edit window.  The rule is: the editor can write tiny append-only events while open; compaction, pruning, VACUUM, and historical import stay outside the source-role click/paint path.
...
```cmd
cd /d "T:\References\to go\Media\tools\Modified YouTube comment extractor" && tools\webview2_source_role_editor_native\maintain_r42bi_source_role_db.cmd
```
