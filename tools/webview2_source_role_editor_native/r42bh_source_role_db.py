#!/usr/bin/env python3
"""R42BH source-role plan SQLite sidecar.
...
workflow inspection.  R42BH adds a current-plan table and token/schema-aware
...
SCHEMA = "r42bh_source_role_plan_v3"
...
def ensure_schema(con: sqlite3.Connection) -> None:
    con.executescript(
        ...
        INSERT INTO schema_meta(key, value) VALUES('schema', 'r42bh_source_role_plan_v3')
    ...
    try:
        ...
        with con:
            ...
            for line_no, item_index, change, raw, parent in iter_changes(changes_path) or []:
                ...
                if native_schema == "r42bh" or "R42BH" in app_marker:
...
def inspect_db(db_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            out["ui_duration_ms_r42bh"] = _duration_summary(con, "native_schema='r42bh' OR app_marker LIKE '%R42BH%'")
            out["ui_duration_ms_latest_50_r42bh"] = _duration_latest(con, 50, "native_schema='r42bh' OR app_marker LIKE '%R42BH%'")
        except Exception as exc:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="R42BH source-role SQLite sidecar")
