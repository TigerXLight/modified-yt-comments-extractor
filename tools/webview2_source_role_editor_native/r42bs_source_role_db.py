#!/usr/bin/env python3
"""R42BS source-role plan SQLite sidecar.
...
workflow inspection.  R42BS adds a current-plan table and token/schema-aware
...
SCHEMA = "r42bs_source_role_plan_v3"
...
def ensure_schema(con: sqlite3.Connection) -> None:
    con.executescript(
        ...
        INSERT INTO schema_meta(key, value) VALUES('schema', 'r42bs_source_role_plan_v3')
...
def maintain_db(db_path: Path, *, keep_sessions_per_url: int = 25, keep_events: int = 5000, vacuum: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        # R42BS: on Windows the file-size shrink from VACUUM may not be visible
    ...
    try:
        ...
        with con:
            ...
            for line_no, item_index, change, raw, parent in iter_changes(changes_path) or []:
                ...
                if native_schema == "r42bs" or "R42BS" in app_marker:
...
def inspect_db(db_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            out["ui_duration_ms_r42bs"] = _duration_summary(con, "native_schema='r42bs' OR app_marker LIKE '%R42BS%'")
            out["ui_duration_ms_latest_50_r42bs"] = _duration_latest(con, 50, "native_schema='r42bs' OR app_marker LIKE '%R42BS%'")
        except Exception as exc:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="R42BS source-role SQLite sidecar")
