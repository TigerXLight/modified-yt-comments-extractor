...
from profile_media_case_manifest import (  # noqa: E402
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plan or write a guarded profile/media case manifest.")
...
def main() -> int:
    ...
    payload["confirmation_phrase_expected"] = PROFILE_MEDIA_CASE_MANIFEST_CONFIRMATION if args.execute else ""
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if result.status not in {"blocked_confirmation_required", "blocked_case_root_missing", "blocked_path_outside_case"} else 2
