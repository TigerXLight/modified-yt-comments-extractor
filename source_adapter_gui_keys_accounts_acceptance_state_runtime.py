...
RUNTIME_TITLE = 'GUI KEYS Accounts Acceptance State'
RUNTIME_STAGE = 'gui_keys_accounts_acceptance_state'
...
class GuiKeysAccountsAcceptanceStateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> GuiKeysAccountsAcceptanceStateRecord:
    ...
    return GuiKeysAccountsAcceptanceStateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> GuiKeysAccountsAcceptanceStateRecord:
    return build_record(
        "gui_keys_accounts_acceptance_state-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "gui_keys_accounts_acceptance_state"},
    )


def summarise_record(record: GuiKeysAccountsAcceptanceStateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: GuiKeysAccountsAcceptanceStateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> GuiKeysAccountsAcceptanceStateRecord:
    return build_record(
...
def write_record(path: str | Path, record: GuiKeysAccountsAcceptanceStateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> GuiKeysAccountsAcceptanceStateRecord:
