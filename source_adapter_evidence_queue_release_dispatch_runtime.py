...
RUNTIME_TITLE = 'Evidence Queue Release Dispatch'
RUNTIME_STAGE = 'evidence_queue_release_dispatch'
...
class EvidenceQueueReleaseDispatchRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> EvidenceQueueReleaseDispatchRecord:
    ...
    return EvidenceQueueReleaseDispatchRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> EvidenceQueueReleaseDispatchRecord:
    return build_record(
        "evidence_queue_release_dispatch-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "evidence_queue_release_dispatch"},
    )


def summarise_record(record: EvidenceQueueReleaseDispatchRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: EvidenceQueueReleaseDispatchRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> EvidenceQueueReleaseDispatchRecord:
    return build_record(
...
def write_record(path: str | Path, record: EvidenceQueueReleaseDispatchRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> EvidenceQueueReleaseDispatchRecord:
