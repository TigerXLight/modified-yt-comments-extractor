...
SRC = Path(__file__).with_name("profile_media_facebook_safe_visual_topdown_r45aw.py").read_text(encoding="utf-8")

def require(needle: str) -> None:
    assert needle in SRC, f"missing {needle!r}"

def test_contract_static_assets() -> None:
    require("YTCE_R45AW_SAFE_VISUAL_TOPDOWN_MAX_BANDS")
    require("PROFILE_NAV_BLOCKER_JS")
    require("R45AW_PROFILE_NAV_BLOCKER")
    require("profile_or_profile_comment_anchor_under_click_point")
    require("R45AW_UNSAFE_CLICK_POINT_SKIPPED")
    require("skipKeys")
...
if __name__ == "__main__":
    ...
    print("profile_media_facebook_safe_visual_topdown_r45aw_test: PASS")
