...
RUNTIME_TITLE = 'Source Observation Finalizer'
RUNTIME_STAGE = 'source_observation_finalizer'
...
class SourceObservationFinalizerRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> SourceObservationFinalizerRecord:
    ...
    return SourceObservationFinalizerRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> SourceObservationFinalizerRecord:
    return build_record(
        "source_observation_finalizer-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "source_observation_finalizer"},
    )


def summarise_record(record: SourceObservationFinalizerRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: SourceObservationFinalizerRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> SourceObservationFinalizerRecord:
    return build_record(
...
def write_record(path: str | Path, record: SourceObservationFinalizerRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> SourceObservationFinalizerRecord:
