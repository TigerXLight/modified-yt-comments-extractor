...
from online_asr_keys_accounts_review_release_gate_store_cli import (
    ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_GATE_STORE_CLI_SCHEMA_VERSION,
    build_online_asr_keys_accounts_review_release_gate_store_cli_result,
    online_asr_keys_accounts_review_release_gate_store_cli_result_to_json,
    run_online_asr_keys_accounts_review_release_gate_store_cli,
)


def _safe_artifact(schema_version: str, package_id: str = "release_gate_cli_fixture") -> dict[str, object]:
    return {
...
def test_release_gate_store_cli_builds_safe_result_without_full_paths() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        result = build_online_asr_keys_accounts_review_release_gate_store_cli_result(_safe_chain(), tmpdir)
        data = result.to_dict()
        encoded = online_asr_keys_accounts_review_release_gate_store_cli_result_to_json(result)

        assert data["schema_version"] == ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_GATE_STORE_CLI_SCHEMA_VERSION
        ...
        assert data["stored_file_names"] == [ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_GATE_FILENAME]
        assert data["stored_file_hashes"] and len(data["stored_file_hashes"][0]) == 64
        assert (Path(tmpdir) / ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_GATE_FILENAME).exists()
...
def test_release_gate_store_cli_rejects_secret_like_input() -> None:
    unsafe = dict(_safe_chain()[0])
    unsafe["api_key"] = "super-secret-value-that-must-not-leak"
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            build_online_asr_keys_accounts_review_release_gate_store_cli_result((unsafe,), tmpdir)
...
def test_release_gate_store_cli_entrypoint_json_and_summary() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert run_online_asr_keys_accounts_review_release_gate_store_cli(
            argv,
            stdout=stdout,
            stderr=stderr,
        ) == 0
        ...
        assert "Online ASR KEYS/ACCOUNTS review release gate store CLI" in summary.getvalue()
        assert "Release gate ready: true" in summary.getvalue()
        assert "Provider call allowed without user approval: false" in summary.getvalue()


if __name__ == "__main__":
    test_release_gate_store_cli_builds_safe_result_without_full_paths()
    test_release_gate_store_cli_rejects_secret_like_input()
    test_release_gate_store_cli_entrypoint_json_and_summary()
    print("Online ASR KEYS/ACCOUNTS review release gate store CLI self-test passed.")
