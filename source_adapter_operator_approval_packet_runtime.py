...
SCHEMA_VERSION = "source_adapter_operator_approval_packet_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_operator_approval_packet_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_OPERATOR_APPROVAL_PACKET_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['approval_id', 'named_site', 'provider_profile', 'operator_signoff']


@dataclass(frozen=True)
class OperatorApprovalPacketRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class OperatorApprovalPacketRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = OperatorApprovalPacketRuntimeRow(
            row_id=_stable_id("source_adapter.operator_approval_packet_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="operator approval packet",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "operator approval packet",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.operator_approval_packet_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "operator approval packet",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.operator_approval_packet_runtime.json"
    rows_path = out / f"{package['package_id']}.operator_approval_packet_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.operator_approval_packet_runtime.verification.json"
