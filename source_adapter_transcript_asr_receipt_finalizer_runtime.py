...
RUNTIME_TITLE = 'Transcript ASR Receipt Finalizer'
RUNTIME_STAGE = 'transcript_asr_receipt_finalizer'
...
class TranscriptAsrReceiptFinalizerRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> TranscriptAsrReceiptFinalizerRecord:
    ...
    return TranscriptAsrReceiptFinalizerRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> TranscriptAsrReceiptFinalizerRecord:
    return build_record(
        "transcript_asr_receipt_finalizer-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "transcript_asr_receipt_finalizer"},
    )


def summarise_record(record: TranscriptAsrReceiptFinalizerRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: TranscriptAsrReceiptFinalizerRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> TranscriptAsrReceiptFinalizerRecord:
    return build_record(
...
def write_record(path: str | Path, record: TranscriptAsrReceiptFinalizerRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> TranscriptAsrReceiptFinalizerRecord:
