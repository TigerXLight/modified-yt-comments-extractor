...
RUNTIME_TITLE = 'Browser Live Execution Bridge'
RUNTIME_STAGE = 'browser_live_execution_bridge'
...
class BrowserLiveExecutionBridgeRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> BrowserLiveExecutionBridgeRecord:
    ...
    return BrowserLiveExecutionBridgeRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> BrowserLiveExecutionBridgeRecord:
    return build_record(
        "browser_live_execution_bridge-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "browser_live_execution_bridge"},
    )


def summarise_record(record: BrowserLiveExecutionBridgeRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: BrowserLiveExecutionBridgeRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> BrowserLiveExecutionBridgeRecord:
    return build_record(
...
def write_record(path: str | Path, record: BrowserLiveExecutionBridgeRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> BrowserLiveExecutionBridgeRecord:
