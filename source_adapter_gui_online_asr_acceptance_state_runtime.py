...
RUNTIME_TITLE = 'GUI Online ASR Acceptance State'
RUNTIME_STAGE = 'gui_online_asr_acceptance_state'
...
class GuiOnlineAsrAcceptanceStateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> GuiOnlineAsrAcceptanceStateRecord:
    ...
    return GuiOnlineAsrAcceptanceStateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> GuiOnlineAsrAcceptanceStateRecord:
    return build_record(
        "gui_online_asr_acceptance_state-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "gui_online_asr_acceptance_state"},
    )


def summarise_record(record: GuiOnlineAsrAcceptanceStateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: GuiOnlineAsrAcceptanceStateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> GuiOnlineAsrAcceptanceStateRecord:
    return build_record(
...
def write_record(path: str | Path, record: GuiOnlineAsrAcceptanceStateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> GuiOnlineAsrAcceptanceStateRecord:
