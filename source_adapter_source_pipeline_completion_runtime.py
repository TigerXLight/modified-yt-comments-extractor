...
SCHEMA_VERSION = "source_adapter_source_pipeline_completion_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_source_pipeline_completion_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_SOURCE_PIPELINE_COMPLETION_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['evidence_sync', 'total_export_finalized', 'release_locked', 'archive_closed']


@dataclass(frozen=True)
class SourcePipelineCompletionRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class SourcePipelineCompletionRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = SourcePipelineCompletionRuntimeRow(
            row_id=_stable_id("source_adapter.source_pipeline_completion_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="source pipeline completion",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "source pipeline completion",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.source_pipeline_completion_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "source pipeline completion",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.source_pipeline_completion_runtime.json"
    rows_path = out / f"{package['package_id']}.source_pipeline_completion_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.source_pipeline_completion_runtime.verification.json"
