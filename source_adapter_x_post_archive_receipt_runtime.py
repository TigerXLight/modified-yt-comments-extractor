...
RUNTIME_TITLE = 'X Post Archive Receipt'
RUNTIME_STAGE = 'x_post_archive_receipt'
...
class XPostArchiveReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    return XPostArchiveReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> XPostArchiveReceiptRecord:
    return build_record(
        "x_post_archive_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "x_post_archive_receipt"},
    )


def summarise_record(record: XPostArchiveReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: XPostArchiveReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> XPostArchiveReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: XPostArchiveReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> XPostArchiveReceiptRecord:
