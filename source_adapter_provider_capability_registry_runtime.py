...
SCHEMA_VERSION = "source_adapter_provider_capability_registry_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_provider_capability_registry_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_PROVIDER_CAPABILITY_REGISTRY_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['browser_capture', 'archive_submit', 'release_upload', 'file_library_publish', 'credential_lookup']


@dataclass(frozen=True)
class ProviderCapabilityRegistryRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class ProviderCapabilityRegistryRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = ProviderCapabilityRegistryRuntimeRow(
            row_id=_stable_id("source_adapter.provider_capability_registry_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="provider capability registry",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "provider capability registry",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.provider_capability_registry_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "provider capability registry",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.provider_capability_registry_runtime.json"
    rows_path = out / f"{package['package_id']}.provider_capability_registry_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.provider_capability_registry_runtime.verification.json"
