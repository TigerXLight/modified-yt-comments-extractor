...
from source_adapter_final_capture_command_registry_runtime import RUNTIME_STAGE, FinalCaptureCommandRegistryRecord


def verify_record(record: FinalCaptureCommandRegistryRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[FinalCaptureCommandRegistryRecord]) -> list[str]:
    issues: list[str] = []
