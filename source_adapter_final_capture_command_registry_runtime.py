...
RUNTIME_TITLE = 'Final Capture Command Registry'
RUNTIME_STAGE = 'final_capture_command_registry'
...
class FinalCaptureCommandRegistryRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> FinalCaptureCommandRegistryRecord:
    ...
    return FinalCaptureCommandRegistryRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> FinalCaptureCommandRegistryRecord:
    return build_record(
        "final_capture_command_registry-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "final_capture_command_registry"},
    )


def summarise_record(record: FinalCaptureCommandRegistryRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: FinalCaptureCommandRegistryRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> FinalCaptureCommandRegistryRecord:
    return build_record(
...
def write_record(path: str | Path, record: FinalCaptureCommandRegistryRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> FinalCaptureCommandRegistryRecord:
