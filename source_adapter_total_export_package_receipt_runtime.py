...
RUNTIME_TITLE = 'Total Export Package Receipt'
RUNTIME_STAGE = 'total_export_package_receipt'
...
class TotalExportPackageReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> TotalExportPackageReceiptRecord:
    ...
    return TotalExportPackageReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> TotalExportPackageReceiptRecord:
    return build_record(
        "total_export_package_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "total_export_package_receipt"},
    )


def summarise_record(record: TotalExportPackageReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: TotalExportPackageReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> TotalExportPackageReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: TotalExportPackageReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> TotalExportPackageReceiptRecord:
