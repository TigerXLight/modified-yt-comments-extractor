...
MARKER = "YTCE_R45AT_SAFE_VISUAL_TOPDOWN_EXHAUST"
SCHEMA_VERSION = "facebook_safe_visual_topdown.r45at.v1"
...
def contract() -> Dict[str, Any]:
    return {
        ...
        "r45at_rule": "Open the post, lock to the target story, install a profile/comment-permalink navigation blocker, start at the top of the active comments scroller, click only safe expansion labels, rescan the same viewport, keep moving downward as comments load, then require a complete top-to-bottom visual audit pass with zero visible controls before screenshots.",
        ...
        "success_rule": "No final screenshot is accepted until a full visible top-to-bottom audit pass sees zero expansion controls; any click point resolving to a Facebook profile/comment permalink is blocked before navigation.",
...
() => {
  if (window.__YTCE_R45AT_PROFILE_NAV_BLOCKER_INSTALLED) return {installed:true, already:true};
  const isProfileOrProfileCommentHref = (hrefRaw) => {
    const href = String(hrefRaw || '');
    if (!href) return false;
    let u;
    ...
    for (let i = 0; el && i < 12; i++, el = el.parentElement) {
      if ((el.tagName || '').toLowerCase() === 'a') {
        ...
        if (isProfileOrProfileCommentHref(href)) {
          ...
          console.warn('YTCE_R45AT_BLOCKED_PROFILE_NAV', href);
          return false;
        }
      }
    }
  ...
  window.__YTCE_R45AT_PROFILE_NAV_BLOCKER_INSTALLED = true;
...
def _probe(page: Any, hint: Dict[str, Any]) -> Dict[str, Any]:
    return page.evaluate(PROBE_JS, {"hint": hint, "maxItems": 120}) or {}
...
def exhaust(page: Any, args: argparse.Namespace, story: str) -> Dict[str, Any]:
    ...
    for step in range(1, args.max_steps + 1):
        ...
        if elapsed > args.expand_max_seconds:
            ...
            emit("R45AT_BLOCKED_VISIBLE_CONTROLS", {"reason":"timeout", "clicked":clicked, "scrolls":scrolls, "audit_pass":audit_pass, "visible_total":probe.get("total"), "visible_counts":probe.get("counts"), "visible_labels":probe.get("labels")})
            return {"status":"blocked", "reason":"timeout", "clicked":clicked, "scrolls":scrolls, "probe":probe}

        guard = _guard(page, story)
        if not guard.get("ok"):
            emit("R45AT_TARGET_SURFACE_LOST", {"step":step, "clicked":clicked, "guard":guard})
            return {"status":"blocked", "reason":"target_surface_lost", "clicked":clicked, "guard":guard}

        probe = _probe(page, hint)
        emit("R45AT_VIEWPORT_SCAN", {"step":step, "audit_pass":audit_pass, "elapsed_seconds":round(elapsed,2), "visible_total":probe.get("total",0), "visible_counts":probe.get("counts",{}), "visible_labels":(probe.get("labels") or [])[:30], "scrollTop":(probe.get("scroller") or {}).get("scrollTop"), "scrollHeight":(probe.get("scroller") or {}).get("scrollHeight")})
        ...
        if int(probe.get("total") or 0) > 0:
            ...
            if not point_safety.get("ok"):
                emit("R45AT_UNSAFE_CLICK_POINT_BLOCKED", {"step": step, "audit_pass": audit_pass, "item": item, "point_safety": point_safety, "elapsed_seconds": round(time.monotonic()-start,2)})
                return {"status":"blocked", "reason":"unsafe_click_point", "clicked":clicked, "item":item, "point_safety":point_safety}
            try:
            ...
            emit("R45AT_CLICK_FIRST_VISIBLE", {"step":step, "audit_pass":audit_pass, "clicked":clicked, "category":item.get("category"), "label":item.get("label"), "x":item.get("x"), "y":item.get("y"), "target_ok":after_guard.get("ok"), "elapsed_seconds":round(time.monotonic()-start,2)})
        ...
        scrolls += 1
        time.sleep(args.scroll_settle_seconds)
        emit("R45AT_SCROLL_DOWN", {"step":step, "audit_pass":audit_pass, "scrolls":scrolls, **scr, "elapsed_seconds":round(time.monotonic()-start,2)})
        ...
        if scr.get("atBottom") or no_scroll_cycles >= 2:
            ...
            bottom_probe = _probe(page, hint)
            if int(bottom_probe.get("total") or 0) > 0:
                continue
            if clean_bottom_once:
                emit("R45AT_COMPLETE_AFTER_FULL_VISUAL_AUDIT", {"status":"PASS_FULL_TOPDOWN_VISUAL_AUDIT_ZERO_VISIBLE_CONTROLS", "clicked":clicked, "scrolls":scrolls, "audit_passes":audit_pass, "elapsed_seconds":round(time.monotonic()-start,2)})
                return {"status":"pass", "clicked":clicked, "scrolls":scrolls, "audit_passes":audit_pass}
            clean_bottom_once = True
            audit_pass += 1
            no_scroll_cycles = 0
            last_scroll = None
            top = _scroll(page, hint, "top")
            emit("R45AT_AUDIT_RESTART_TOP", {"audit_pass":audit_pass, **top, "elapsed_seconds":round(time.monotonic()-start,2)})
            time.sleep(args.scroll_settle_seconds)

    probe = _probe(page, hint)
    emit("R45AT_BLOCKED_VISIBLE_CONTROLS", {"reason":"max_steps", "clicked":clicked, "scrolls":scrolls, "visible_total":probe.get("total"), "visible_counts":probe.get("counts"), "visible_labels":probe.get("labels")})
...
def capture_tiles(page: Any, hint: Dict[str, Any], run_dir: Path, settle: float) -> List[str]:
    ...
    for i in range(1, 800):
        ...
        p = run_dir / f"r45at_comments_tile_{i:04d}_y{int(st):08d}.png"
        ...
        seen.add(key)
        time.sleep(settle)
    zip_path = run_dir / "r45at_comments_tiles.zip"
...
def run_live(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_dir = out_root / f"r45at_safe_visual_topdown_{stamp}"
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                nav_block = page.evaluate(PROFILE_NAV_BLOCKER_JS)
                emit("R45AT_PROFILE_NAV_BLOCKER_INITIAL", nav_block)
            except Exception as e:
                emit("R45AT_PROFILE_NAV_BLOCKER_INITIAL", {"installed": False, "error": str(e)})
            ...
            if summary.get("status") != "pass":
                ...
                rp = run_dir / "r45at_safe_visual_topdown_receipt.json"
                rp.write_text(json.dumps(receipt, ensure_ascii=False, indent=2), encoding="utf-8")
                receipt["receipt_path"] = str(rp)
                print("R45AT blocked: visible audit did not finish cleanly. No final screenshot accepted.", flush=True)
                return receipt
            ...
            receipt["status"] = "PASS_R45AT_FULL_VISUAL_AUDIT_AND_SCREENSHOT"
            rp = run_dir / "r45at_safe_visual_topdown_receipt.json"
            rp.write_text(json.dumps(receipt, ensure_ascii=False, indent=2), encoding="utf-8")
            receipt["receipt_path"] = str(rp)
            print("YTCE_R45AT_VISUAL_TOPDOWN_AUDIT_DONE", flush=True)
...
def self_test(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    checks = [
        ...
        {"name":"full_visual_audit_required", "status":"pass" if "R45AT_COMPLETE_AFTER_FULL_VISUAL_AUDIT" in text else "fail"},
        {"name":"target_surface_lost_failfast", "status":"pass" if "R45AT_TARGET_SURFACE_LOST" in text else "fail"},
        {"name":"profile_nav_blocker_present", "status":"pass" if "R45AT_PROFILE_NAV_BLOCKER" in text and "profile_or_profile_comment_anchor_under_click_point" in text else "fail"},
        {"name":"no_hidden_platform_api", "status":"pass" if contract()["hidden_platform_api_scraping_enabled"] is False else "fail"},
    ]
    status = "PASS_R45AT_SELF_TEST" if all(c["status"] == "pass" for c in checks) else "FAIL_R45AT_SELF_TEST"
    ...
    (out / "r45at_self_test.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    print(MARKER); print(status); print(json.dumps(result, ensure_ascii=False, indent=2))
    return result


def parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="R45AT visual top-down Facebook comments expansion and screenshot")
    ...
    ap.add_argument("--output-root", default="profile_media_live_captures/r45at_safe_visual_topdown")
    return ap
