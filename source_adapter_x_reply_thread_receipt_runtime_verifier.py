...
from source_adapter_x_reply_thread_receipt_runtime import RUNTIME_STAGE, XReplyThreadReceiptRecord


def verify_record(record: XReplyThreadReceiptRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[XReplyThreadReceiptRecord]) -> list[str]:
    issues: list[str] = []
