...
RUNTIME_TITLE = 'MSN Article Capture Receipt'
RUNTIME_STAGE = 'msn_article_capture_receipt'
...
class MsnArticleCaptureReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> MsnArticleCaptureReceiptRecord:
    ...
    return MsnArticleCaptureReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> MsnArticleCaptureReceiptRecord:
    return build_record(
        "msn_article_capture_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "msn_article_capture_receipt"},
    )


def summarise_record(record: MsnArticleCaptureReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: MsnArticleCaptureReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> MsnArticleCaptureReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: MsnArticleCaptureReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> MsnArticleCaptureReceiptRecord:
