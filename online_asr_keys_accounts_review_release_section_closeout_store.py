...
ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_STORE_SCHEMA_VERSION = (
    "online_asr_keys_accounts_review_release_section_closeout_store_v1"
)
ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_FILENAME = (
    "online_asr_keys_accounts_review_release_section_closeout_report.json"
)
...
class OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoredFile:
    filename: str
    file_role: str
    sha256: str
    byte_count: int
    schema_version: str = ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_STORE_SCHEMA_VERSION
...
@dataclass(frozen=True)
class OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreResult:
    roadmap_section: str
    ...
    file_count: int
    files: tuple[OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoredFile, ...]
    schema_version: str = ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_STORE_SCHEMA_VERSION
    ...
    def to_summary_text(self) -> str:
        return "\n".join(
            (
                "Online ASR KEYS/ACCOUNTS release section closeout stored",
...
def build_online_asr_keys_accounts_review_release_section_closeout_store_payloads(
    report: OnlineASRKeysAccountsReviewReleaseSectionCloseoutReport,
) -> dict[str, str]:
    return {
        ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_FILENAME: (
            online_asr_keys_accounts_review_release_section_closeout_report_to_json(report)
        ),
    }


def write_online_asr_keys_accounts_review_release_section_closeout_report(
    report: OnlineASRKeysAccountsReviewReleaseSectionCloseoutReport,
    output_directory: str | Path,
    *,
    allow_overwrite: bool = True,
) -> OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreResult:
    """Write a metadata-only release-section closeout report to a user-selected directory."""
    output_path = Path(output_directory)
    output_path.mkdir(parents=True, exist_ok=True)
    payload_texts = build_online_asr_keys_accounts_review_release_section_closeout_store_payloads(report)
    stored_files: list[OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoredFile] = []
    for filename, payload_text in payload_texts.items():
        file_path = output_path / filename
        if file_path.exists() and not allow_overwrite:
            raise FileExistsError(
                f"Refusing to overwrite existing review release-section closeout file: {filename}"
            )
        payload_bytes = payload_text.encode("utf-8")
        file_path.write_bytes(payload_bytes)
        stored_files.append(
            OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoredFile(
                filename=filename,
    ...
    return OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreResult(
        ...
        output_directory_role="user_selected_online_asr_keys_accounts_review_release_section_closeout_directory",
        file_count=len(stored_files),
        files=tuple(stored_files),
    )


def online_asr_keys_accounts_review_release_section_closeout_store_result_to_json(
    result: OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreResult,
) -> str:
