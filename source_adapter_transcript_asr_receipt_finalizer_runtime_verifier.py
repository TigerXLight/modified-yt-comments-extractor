...
from source_adapter_transcript_asr_receipt_finalizer_runtime import RUNTIME_STAGE, TranscriptAsrReceiptFinalizerRecord


def verify_record(record: TranscriptAsrReceiptFinalizerRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[TranscriptAsrReceiptFinalizerRecord]) -> list[str]:
    issues: list[str] = []
