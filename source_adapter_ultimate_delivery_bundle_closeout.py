...
SCHEMA_VERSION = "source_adapter_ultimate_delivery_bundle_closeout_v1"
STATUS = "SOURCE_ADAPTER_ULTIMATE_DELIVERY_BUNDLE_CLOSEOUT_BUILT"
HANDOFF_STATUS = "SOURCE_ADAPTER_ULTIMATE_DELIVERY_BUNDLE_CLOSEOUT_READY_FOR_NEXT_IMPLEMENTATION_BUNDLE"
ROW_STATUS = "SOURCE_ADAPTER_ULTIMATE_DELIVERY_BUNDLE_CLOSEOUT_ROWS_READY"
ROW_KEY = "ultimate_delivery_bundle_closeout_rows"
DESCRIPTION = "ultimate delivery bundle closeout rows covering runbooks, browser/comment/media/archive/evidence/Total Export/release/file-library/KEYS/ACCOUNTS/Online ASR/provider/operator/GUI/profile/readiness/traceability surfaces"


def build_ultimate_delivery_bundle_closeout_rows(operator_id: str = "operator") -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for site in named_sites():
        for action_index, action in enumerate(PROVIDER_ACTIONS):
            seed = {"aspect": "ultimate_delivery_bundle_closeout", "site": site, "action": action, "operator_id": operator_id}
            rows.append({
                "schema_version": "source_adapter_ultimate_delivery_bundle_closeout_row_v1",
                "row_id": stable_id("source_adapter.ultimate_delivery_bundle_closeout.row", seed),
                "row_index": len(rows),
...
def example_source_adapter_ultimate_delivery_bundle_closeout_package(operator_id: str = "operator") -> dict[str, Any]:
    rows = build_ultimate_delivery_bundle_closeout_rows(operator_id=operator_id)
    package = {
        "schema_version": SCHEMA_VERSION,
        "id": stable_id("source_adapter.ultimate_delivery_bundle_closeout.package", {"rows": rows, "operator_id": operator_id}),
        "status": STATUS,
        "description": DESCRIPTION,
        "row_key": ROW_KEY,
        "ultimate_delivery_bundle_closeout_rows_count": len(rows),
        ROW_KEY: rows,
        "operator_summary": {
            "schema_version": "source_adapter_ultimate_delivery_bundle_closeout_operator_summary_v1",
            "status": STATUS,
        ...
        "handoff": {
            "schema_version": "source_adapter_ultimate_delivery_bundle_closeout_handoff_v1",
...
def write_source_adapter_ultimate_delivery_bundle_closeout_receipts(output_dir: str | Path | None = None, operator_id: str = "operator") -> dict[str, Any]:
    package = example_source_adapter_ultimate_delivery_bundle_closeout_package(operator_id=operator_id)
    output_root = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix="source_adapter_ultimate_delivery_bundle_closeout_receipts_"))
    stored_files = []
    ...
    stored_files.append(write_json(output_root / f"{package['id']}.ultimate_delivery_bundle_closeout_rows.json", package[ROW_KEY]))
    for role, meta in zip(("package", "handoff", ROW_KEY), stored_files):
        meta["role"] = role
    return {
        "schema_version": "source_adapter_ultimate_delivery_bundle_closeout_receipt_writer_v1",
        "id": package["id"],
...
if __name__ == "__main__":
    print(json.dumps(example_source_adapter_ultimate_delivery_bundle_closeout_package(), indent=2, sort_keys=True))
