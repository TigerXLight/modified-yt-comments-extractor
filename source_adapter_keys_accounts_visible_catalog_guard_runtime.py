...
RUNTIME_TITLE = 'KEYS Accounts Visible Catalog Guard'
RUNTIME_STAGE = 'keys_accounts_visible_catalog_guard'
...
class KeysAccountsVisibleCatalogGuardRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> KeysAccountsVisibleCatalogGuardRecord:
    ...
    return KeysAccountsVisibleCatalogGuardRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> KeysAccountsVisibleCatalogGuardRecord:
    return build_record(
        "keys_accounts_visible_catalog_guard-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "keys_accounts_visible_catalog_guard"},
    )


def summarise_record(record: KeysAccountsVisibleCatalogGuardRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: KeysAccountsVisibleCatalogGuardRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> KeysAccountsVisibleCatalogGuardRecord:
    return build_record(
...
def write_record(path: str | Path, record: KeysAccountsVisibleCatalogGuardRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> KeysAccountsVisibleCatalogGuardRecord:
