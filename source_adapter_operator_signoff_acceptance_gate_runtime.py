...
RUNTIME_TITLE = 'Operator Signoff Acceptance Gate'
RUNTIME_STAGE = 'operator_signoff_acceptance_gate'
...
class OperatorSignoffAcceptanceGateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> OperatorSignoffAcceptanceGateRecord:
    ...
    return OperatorSignoffAcceptanceGateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> OperatorSignoffAcceptanceGateRecord:
    return build_record(
        "operator_signoff_acceptance_gate-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "operator_signoff_acceptance_gate"},
    )


def summarise_record(record: OperatorSignoffAcceptanceGateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: OperatorSignoffAcceptanceGateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> OperatorSignoffAcceptanceGateRecord:
    return build_record(
...
def write_record(path: str | Path, record: OperatorSignoffAcceptanceGateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> OperatorSignoffAcceptanceGateRecord:
