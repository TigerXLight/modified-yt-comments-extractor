...
RUNTIME_TITLE = 'GUI Archive Review Acceptance State'
RUNTIME_STAGE = 'gui_archive_review_acceptance_state'
...
class GuiArchiveReviewAcceptanceStateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> GuiArchiveReviewAcceptanceStateRecord:
    ...
    return GuiArchiveReviewAcceptanceStateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> GuiArchiveReviewAcceptanceStateRecord:
    return build_record(
        "gui_archive_review_acceptance_state-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "gui_archive_review_acceptance_state"},
    )


def summarise_record(record: GuiArchiveReviewAcceptanceStateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: GuiArchiveReviewAcceptanceStateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> GuiArchiveReviewAcceptanceStateRecord:
    return build_record(
...
def write_record(path: str | Path, record: GuiArchiveReviewAcceptanceStateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> GuiArchiveReviewAcceptanceStateRecord:
