...
RUNTIME_TITLE = 'Project Handoff Acceptance Audit'
RUNTIME_STAGE = 'project_handoff_acceptance_audit'
...
class ProjectHandoffAcceptanceAuditRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ProjectHandoffAcceptanceAuditRecord:
    ...
    return ProjectHandoffAcceptanceAuditRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ProjectHandoffAcceptanceAuditRecord:
    return build_record(
        "project_handoff_acceptance_audit-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "project_handoff_acceptance_audit"},
    )


def summarise_record(record: ProjectHandoffAcceptanceAuditRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ProjectHandoffAcceptanceAuditRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ProjectHandoffAcceptanceAuditRecord:
    return build_record(
...
def write_record(path: str | Path, record: ProjectHandoffAcceptanceAuditRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ProjectHandoffAcceptanceAuditRecord:
