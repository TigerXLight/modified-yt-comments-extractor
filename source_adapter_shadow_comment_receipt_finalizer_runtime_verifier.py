...
from source_adapter_shadow_comment_receipt_finalizer_runtime import RUNTIME_STAGE, ShadowCommentReceiptFinalizerRecord


def verify_record(record: ShadowCommentReceiptFinalizerRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[ShadowCommentReceiptFinalizerRecord]) -> list[str]:
    issues: list[str] = []
