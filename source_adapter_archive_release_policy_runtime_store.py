...
SCHEMA_VERSION = "source_adapter_archive_release_policy_runtime_store_v1"
STORE_STATUS = "STORED"
...
def store_source_adapter_archive_release_policy_runtime_package(package: Mapping[str, Any] | None = None, output_dir: str | Path | None = None) -> dict[str, Any]:
    package = dict(package or example_source_adapter_archive_release_policy_runtime_package())
    output_root = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix="source_adapter_archive_release_policy_runtime_store_"))
    stored_files = []
...
if __name__ == "__main__":
    print(json.dumps(store_source_adapter_archive_release_policy_runtime_package(), indent=2, sort_keys=True))
