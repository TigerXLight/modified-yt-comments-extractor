...
- `Best-tested local profile` is now a selectable/saved profile that sets `engine=whispercpp_vulkan`, `device=vulkan`, `compute_type=whisper.cpp`, and `model_name=large-v3`; Local ASR dispatch therefore reaches the existing whisper.cpp production runner through the established local wrapper rather than silently falling back to faster-whisper.
