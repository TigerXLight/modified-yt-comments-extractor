...
- MODEL_ONLY live/manual site-smoke planning scaffold now records candidate site labels, source URL placeholders, source adapter families, APPROVAL_REQUIRED / MANUAL_OPERATOR_ONLY status, manual checklist items, expected artifact declaration types, result placeholders, and explicit safety prohibitions. Approval requires explicit named site label, non-placeholder source URL, approver metadata, and named manual action/scope IDs. It emits no execution commands and does not mark default plans as completed manually.
