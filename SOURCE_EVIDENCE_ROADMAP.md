...
- Local ASR timeout hardening scales the real whisper.cpp Vulkan subprocess timeout for long normalized audio, records the timeout policy in metadata, and gives configurable environment-variable guidance (`ASR_WHISPERCPP_TIMEOUT`, `ASR_WHISPERCPP_TIMEOUT_REALTIME_MULTIPLIER`, `ASR_WHISPERCPP_MAX_TIMEOUT`) without downgrading the benchmark-backed `large-v3` Vulkan recommendation.
