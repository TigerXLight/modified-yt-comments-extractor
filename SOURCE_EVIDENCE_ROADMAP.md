...
- Roadmap status: the first full production run proved browser/live capture dependencies are available, but it also exposed adapter closeout gaps. The failed York output remains `MSN_SOURCE_ADAPTER_REVIEW_REQUIRED` until direct V6/V15 screenshots, 25 imported comments, York media, and source-role exports all pass together.
- The closeout path now imports rendered-browser comments from completed live capture output into the production V34/V35 exporter, preserving source-role metadata in the generated comments/profile sidecars.
- The accepted V15 screenshot path now uses the same broad nested-shadow internal-scroller candidate rule as the standalone V15 runner and records detailed diagnostics when no scroller is selected. Debug-only stitch segments remain gated behind debug mode.
- WARC/WACZ generation stays separate from replay verification. `NOT_TESTED` replay status is allowed as an honest nonfatal warning only after the concrete article/comment/media/source-role gates succeed.
