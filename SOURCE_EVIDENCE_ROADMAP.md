...
- ReplayWeb.page / Local Web Archive action support is implemented in `source_local_web_archive_actions.py`. It detects optional configured ReplayWeb.page desktop viewers, parses official release metadata, builds safe argument-array Open archive previews, builds Show files previews, records the browser/PWA local-file chooser limitation, and verifies WACZ/manifest structure without launching viewers by default. Official release metadata was checked as ReplayWeb.page App `v2.4.7`; no binary was downloaded or installed, and no AGPL viewer code was copied into the app. The accepted MSN WACZ verifies as `STRUCTURAL_WACZ_VALID` with ZIP integrity, datapackage profile `data-package`, WACZ version `1.2.0`, one page, 87 index lines, 174 readable WARC records, expected source URL present, matching WACZ/manifest hashes, and `25/25` comments evidence including `16` roots and `9` replies. Replay visual verification remains `USER_VISUAL_CONFIRMATION_REQUIRED` until a configured external viewer/user visual check confirms rendering.
