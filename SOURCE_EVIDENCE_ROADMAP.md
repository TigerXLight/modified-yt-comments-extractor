...
- MODEL_ONLY live/manual site-smoke planning scaffold now records candidate site labels, source URL placeholders, source adapter families, APPROVAL_REQUIRED / MANUAL_OPERATOR_ONLY status, manual checklist items, expected artifact declaration types, result placeholders, and explicit safety prohibitions. Named-site template validation requires explicit site label, non-placeholder source URL, adapter family, named manual action/scope IDs, approver metadata placeholder, and safety-boundary acknowledgement. It emits no execution commands, approves no real site through the template path, and does not mark default plans as completed manually.
