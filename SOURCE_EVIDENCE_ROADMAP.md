...
- Local ASR timeout hardening scales the real whisper.cpp Vulkan subprocess timeout for long normalized audio, records the timeout policy in metadata, gives configurable environment-variable guidance (`ASR_WHISPERCPP_TIMEOUT`, `ASR_WHISPERCPP_TIMEOUT_REALTIME_MULTIPLIER`, `ASR_WHISPERCPP_MAX_TIMEOUT`), and now surfaces a UI/log-friendly effective-timeout status line plus long-media retry guidance without downgrading the benchmark-backed `large-v3` Vulkan recommendation.
