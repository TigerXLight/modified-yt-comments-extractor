...
- ReplayWeb.page / Local Web Archive action support is implemented in `source_local_web_archive_actions.py`, with optional configured viewer detection, official release metadata parsing, safe argument-array Open archive previews, Show files previews, browser/PWA file-chooser limitation reporting, WACZ/manifest verification, and side-by-side offline WACZ replay repair. Replay investigation found a genuine package/index compatibility defect in the accepted MSN WACZ rather than a reason to repeat capture: its already-accepted comments evidence remains `25/25` (`16` roots, `9` replies), but the original WACZ used pre-1.2 profile `data-package`, retained removed 1.2 metadata fields, and its `87` CDXJ entries were not byte-wise sorted or consistently canonical lower-case lookup keys. Commit `c3eb5e9` fixed malformed WARC request lines, the old `_wb_method=HTTP/1.1` symptom, hosted ReplayWeb.page package/profile readiness, and sorted canonical CDXJ lookup, with status wording `REPLAYWEB_PAGE_PACKAGE_READY` rather than visual success. Manual validation still records normalized WACZ `Archived Page Not Found`; normalized raw WARC/WARC.GZ can reach partial runtime replay but MSN remains slow/broken with privacy modal, black/empty page area, Firefox lag, and no comments visible. `source_replay_static_snapshot.py` and `source_local_web_archive_repair_cli.py --add-static-evidence-page` now add a separate `STATIC_EVIDENCE_VIEW_READY` no-JavaScript static evidence page to a derived side-by-side WACZ. The original accepted archive and original dynamic page entry remain preserved; the derived page is a stable review fallback and not a `REPLAY_VISUALLY_VERIFIED` claim.
