...
import profile_media_facebook_bounded_modal_capture_runner_r45h_legacy_failfast_20260919 as r45h
...
def run_live(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    with sync_playwright() as p:
        ...
        page.on('console', lambda msg: print(msg.text) if (msg.text.startswith('R45H_PROGRESS') or msg.text.startswith('R45AK')) else None)
        ...
        if args.auto_expand:
            ...
            preflight_report = {}
            try:
                preflight_report = page.evaluate(R45AK_VISIBLE_EXPAND_PREFLIGHT_JS) or {}
            except Exception as e:
                preflight_report = {'preflight_error': str(e)[:1000]}
            print('R45AK_PRE_EXPAND_VISIBLE_CONTROL_REPORT ' + json.dumps(preflight_report, ensure_ascii=False))
            try:
                print(f'R45AK_FAILFAST_AUTO_EXPAND_START max_seconds={args.expand_max_seconds}')
                auto_expand_summary = page.evaluate(r45h.JS_BOUNDED_MODAL_AUTO_EXPAND, opts)
                print('R45AK_FAILFAST_AUTO_EXPAND_DONE ' + json.dumps({
                ...
                if not auto_expand_summary or int((auto_expand_summary or {}).get('rounds_completed') or 0) <= 0:
                    post_zero = {}
                    try:
                        post_zero = page.evaluate(R45AK_VISIBLE_EXPAND_PREFLIGHT_JS) or {}
                    except Exception as e:
                        post_zero = {'post_zero_probe_error': str(e)[:1000]}
                    print('R45AK_AUTO_EXPAND_ZERO_ROUNDS ' + json.dumps({
                        'status': 'BLOCKED_AUTO_EXPAND_DID_NOT_RUN',
                        'preflight': preflight_report,
                        'post_zero_probe': post_zero,
                    }, ensure_ascii=False))
                    raise SystemExit('R45AK blocked: auto-expand returned zero rounds. See R45AK_AUTO_EXPAND_ZERO_ROUNDS above.')
                try:
                    ...
                    print('R45AK_VISIBLE_EXPAND_MISSED_REPORT ' + json.dumps({
                    ...
                    print('R45AK_VISIBLE_REPORT_FAILED ' + json.dumps({'error': str(e)[:1000]}, ensure_ascii=False))
            except SystemExit:
                raise
            except Exception as e:
                ...
                failed_report = {}
                try:
                    failed_report = page.evaluate(R45AK_VISIBLE_EXPAND_PREFLIGHT_JS) or {}
                except Exception as e2:
                    failed_report = {'failed_report_probe_error': str(e2)[:1000]}
                print('R45AK_AUTO_EXPAND_FAILED ' + json.dumps({
                    'status': 'BLOCKED_AUTO_EXPAND_EXCEPTION',
                    'error': str(e)[:2000],
                    'preflight': preflight_report,
                    'post_failure_probe': failed_report,
                }, ensure_ascii=False))
                raise SystemExit('R45AK blocked: auto-expand failed before opening controls. See R45AK_AUTO_EXPAND_FAILED above.')
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.description = 'R45AJ Facebook preserved visual screenshot runner: legacy 2026-09-19 engine with visible missed expand report'
    ap.set_defaults(output_root='profile_media_live_captures/r45aj_legacy_visible_report_20260919')
