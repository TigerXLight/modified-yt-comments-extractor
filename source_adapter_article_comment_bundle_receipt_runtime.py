...
RUNTIME_TITLE = 'Article Comment Bundle Receipt'
RUNTIME_STAGE = 'article_comment_bundle_receipt'
...
class ArticleCommentBundleReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ArticleCommentBundleReceiptRecord:
    ...
    return ArticleCommentBundleReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ArticleCommentBundleReceiptRecord:
    return build_record(
        "article_comment_bundle_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "article_comment_bundle_receipt"},
    )


def summarise_record(record: ArticleCommentBundleReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ArticleCommentBundleReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ArticleCommentBundleReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: ArticleCommentBundleReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ArticleCommentBundleReceiptRecord:
