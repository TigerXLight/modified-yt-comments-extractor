...
SCHEMA_VERSION = "source_adapter_evidence_release_sync_closeout_runtime_v1"
STATUS = "SOURCE_ADAPTER_EVIDENCE_RELEASE_SYNC_CLOSEOUT_RUNTIME_BUILT"
HANDOFF_STATUS = "SOURCE_ADAPTER_EVIDENCE_RELEASE_SYNC_CLOSEOUT_RUNTIME_READY_FOR_NEXT_IMPLEMENTATION_STAGE"
ROW_STATUS = "SOURCE_ADAPTER_EVIDENCE_RELEASE_SYNC_CLOSEOUT_READY"
ROW_KEY = "sync_closeout_rows"


def build_source_adapter_evidence_release_sync_closeout_runtime_rows(operator_id: str = "operator") -> list[dict[str, Any]]:
...
def example_source_adapter_evidence_release_sync_closeout_runtime_package(operator_id: str = "operator") -> dict[str, Any]:
    rows = build_source_adapter_evidence_release_sync_closeout_runtime_rows(operator_id=operator_id)
    package = {
        "schema_version": SCHEMA_VERSION,
        "id": stable_id("source_adapter.source_adapter_evidence_release_sync_closeout_runtime", {"rows": rows, "operator_id": operator_id}),
        "status": STATUS,
        "row_key": ROW_KEY,
        "sync_closeout_rows_row_count": len(rows),
        "sync_closeout_rows": rows,
        "operator_summary": {
            "schema_version": "source_adapter_evidence_release_sync_closeout_runtime_operator_summary_v1",
            "status": STATUS,
            "operator_id": operator_id,
            "keys_accounts_label": KEYS_ACCOUNTS_LABEL,
        ...
        "handoff": {
            "schema_version": "source_adapter_evidence_release_sync_closeout_runtime_handoff_v1",
...
def write_source_adapter_evidence_release_sync_closeout_runtime_receipts(output_dir: str | Path | None = None, operator_id: str = "operator") -> dict[str, Any]:
    package = example_source_adapter_evidence_release_sync_closeout_runtime_package(operator_id=operator_id)
    output_root = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix="source_adapter_evidence_release_sync_closeout_runtime_receipts_"))
    stored_files = []
    ...
    stored_files.append(_write_json(output_root / f"{package['id']}.sync_closeout_rows.json", package["sync_closeout_rows"]))
    for role, meta in zip(("package", "handoff", "sync_closeout_rows"), stored_files):
        meta["role"] = role
    return {
        "schema_version": "source_adapter_evidence_release_sync_closeout_runtime_receipt_writer_v1",
        "id": package["id"],
...
if __name__ == "__main__":
    print(json.dumps(example_source_adapter_evidence_release_sync_closeout_runtime_package(), indent=2, sort_keys=True))
