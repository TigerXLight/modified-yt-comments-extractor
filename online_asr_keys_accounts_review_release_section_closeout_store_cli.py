...
from online_asr_keys_accounts_review_release_gate import OnlineASRKeysAccountsReviewReleaseGateError
...
ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_STORE_CLI_SCHEMA_VERSION = (
    "online_asr_keys_accounts_review_release_section_closeout_store_cli_v1"
)
...
class OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIError(ValueError):
    pass
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            if (
                ...
                raise OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIError(
                    f"Refusing {source}: secret-like field '{key}' must not be provided to release section closeout CLI"
                )
...
class OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIResult:
    roadmap_section: str
    source_artifact_count: int
    review_verdict: str
    section_ready: bool
    issue_count: int
    closeout_schema_version: str
    ...
    schema_version: str = ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_SECTION_CLOSEOUT_STORE_CLI_SCHEMA_VERSION
    ...
    def to_summary_text(self) -> str:
        return "\n".join(
            (
                "Online ASR KEYS/ACCOUNTS release section closeout store CLI",
...
def build_online_asr_keys_accounts_review_release_section_closeout_store_cli_result(
    artifacts: Sequence[Mapping[str, Any]],
    output_directory: str | Path,
    *,
    allow_overwrite: bool = True,
) -> OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIResult:
    """Build and persist one metadata-only closeout for the release-gate roadmap section."""
    if not artifacts:
        raise OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIError(
            "at least one artifact JSON object is required"
        )
    normalised = tuple(dict(artifact) for artifact in artifacts)
    _reject_unsafe_secret_keys(normalised, source="artifact JSON")
    report = build_online_asr_keys_accounts_review_release_section_closeout_report(normalised)
    store_result = write_online_asr_keys_accounts_review_release_section_closeout_report(
        report,
        output_directory,
        allow_overwrite=allow_overwrite,
    )
    files = tuple(store_result.files)
    return OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIResult(
        roadmap_section=report.roadmap_section,
        source_artifact_count=report.source_artifact_count,
        review_verdict=report.review_verdict,
        section_ready=report.section_ready,
        issue_count=report.issue_count,
        closeout_schema_version=report.schema_version,
...
def online_asr_keys_accounts_review_release_section_closeout_store_cli_result_to_json(
    result: OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIResult,
) -> str:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build and persist a metadata-only Online ASR KEYS/ACCOUNTS release-section closeout "
            "from safe JSON artifacts. This does not read secrets, call providers, or process media."
        )
    )
    parser.add_argument(
        "--artifact-json",
...
def run_online_asr_keys_accounts_review_release_section_closeout_store_cli(
    ...
    *,
    ...
    try:
        ...
        result = build_online_asr_keys_accounts_review_release_section_closeout_store_cli_result(
            artifacts,
            args.output_directory,
            allow_overwrite=not args.no_overwrite,
        )
    except (
        OnlineASRKeysAccountsReviewReleaseSectionCloseoutStoreCLIError,
        OnlineASRKeysAccountsReviewReleaseGateError,
    ) as exc:
        print(f"Online ASR KEYS/ACCOUNTS release section closeout CLI error: {exc}", file=err)
        return 2
    except FileExistsError as exc:
        print(f"Online ASR KEYS/ACCOUNTS release section closeout CLI error: {exc}", file=err)
        return 3
    except Exception as exc:  # pragma: no cover - CLI safety net for user-facing command failures.
        print(f"Online ASR KEYS/ACCOUNTS release section closeout CLI unexpected error: {exc}", file=err)
        return 1
    ...
    if args.summary:
        ...
        print(
            online_asr_keys_accounts_review_release_section_closeout_store_cli_result_to_json(result),
            end="",
            file=out,
        )
    return 0
...
def main() -> int:
    return run_online_asr_keys_accounts_review_release_section_closeout_store_cli()
