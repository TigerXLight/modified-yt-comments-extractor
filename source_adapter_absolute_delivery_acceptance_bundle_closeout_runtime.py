...
RUNTIME_TITLE = 'Absolute Delivery Acceptance Bundle Closeout'
RUNTIME_STAGE = 'absolute_delivery_acceptance_bundle_closeout'
...
class AbsoluteDeliveryAcceptanceBundleCloseoutRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> AbsoluteDeliveryAcceptanceBundleCloseoutRecord:
    ...
    return AbsoluteDeliveryAcceptanceBundleCloseoutRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> AbsoluteDeliveryAcceptanceBundleCloseoutRecord:
    return build_record(
        "absolute_delivery_acceptance_bundle_closeout-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "absolute_delivery_acceptance_bundle_closeout"},
    )


def summarise_record(record: AbsoluteDeliveryAcceptanceBundleCloseoutRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: AbsoluteDeliveryAcceptanceBundleCloseoutRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> AbsoluteDeliveryAcceptanceBundleCloseoutRecord:
    return build_record(
...
def write_record(path: str | Path, record: AbsoluteDeliveryAcceptanceBundleCloseoutRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> AbsoluteDeliveryAcceptanceBundleCloseoutRecord:
