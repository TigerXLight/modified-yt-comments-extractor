...
RUNTIME_TITLE = 'Online ASR Receipt Import Bridge'
RUNTIME_STAGE = 'online_asr_receipt_import_bridge'
...
class OnlineAsrReceiptImportBridgeRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> OnlineAsrReceiptImportBridgeRecord:
    ...
    return OnlineAsrReceiptImportBridgeRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> OnlineAsrReceiptImportBridgeRecord:
    return build_record(
        "online_asr_receipt_import_bridge-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "online_asr_receipt_import_bridge"},
    )


def summarise_record(record: OnlineAsrReceiptImportBridgeRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: OnlineAsrReceiptImportBridgeRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> OnlineAsrReceiptImportBridgeRecord:
    return build_record(
...
def write_record(path: str | Path, record: OnlineAsrReceiptImportBridgeRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> OnlineAsrReceiptImportBridgeRecord:
