...
SCHEMA_VERSION = "source_adapter_execution_closure_mega_bundle_closeout_v1"
STATUS = "SOURCE_ADAPTER_EXECUTION_CLOSURE_MEGA_BUNDLE_CLOSEOUT_BUILT"
HANDOFF_STATUS = "SOURCE_ADAPTER_EXECUTION_CLOSURE_MEGA_BUNDLE_CLOSEOUT_READY_FOR_NEXT_IMPLEMENTATION_BUNDLE"
ROW_STATUS = "SOURCE_ADAPTER_EXECUTION_CLOSURE_MEGA_BUNDLE_CLOSEOUT_ROWS_READY"
ROW_KEY = "execution_closure_mega_bundle_closeout_rows"
DESCRIPTION = "execution closure mega bundle closeout rows covering source coordination, site/profile runners, archive polling/import, evidence database commits, Total Export/release/file-library delivery, KEYS/ACCOUNTS routing, GUI panels, live smoke gates, quality gates, and end-to-end acceptance reports"


def build_execution_closure_mega_bundle_closeout_rows(operator_id: str = "operator") -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for site in named_sites():
        for action_index, action in enumerate(PROVIDER_ACTIONS):
            seed = {"aspect": "execution_closure_mega_bundle_closeout", "site": site, "action": action, "operator_id": operator_id}
            rows.append({
                "schema_version": "source_adapter_execution_closure_mega_bundle_closeout_row_v1",
                "row_id": stable_id("source_adapter.execution_closure_mega_bundle_closeout.row", seed),
                "row_index": len(rows),
...
def example_source_adapter_execution_closure_mega_bundle_closeout_package(operator_id: str = "operator") -> dict[str, Any]:
    rows = build_execution_closure_mega_bundle_closeout_rows(operator_id=operator_id)
    package = {
        "schema_version": SCHEMA_VERSION,
        "id": stable_id("source_adapter.execution_closure_mega_bundle_closeout.package", {"rows": rows, "operator_id": operator_id}),
        "status": STATUS,
        "description": DESCRIPTION,
        "row_key": ROW_KEY,
        "execution_closure_mega_bundle_closeout_rows_count": len(rows),
        ROW_KEY: rows,
        "operator_summary": {
            "schema_version": "source_adapter_execution_closure_mega_bundle_closeout_operator_summary_v1",
            "status": STATUS,
        ...
        "handoff": {
            "schema_version": "source_adapter_execution_closure_mega_bundle_closeout_handoff_v1",
            "handoff_status": HANDOFF_STATUS,
            "input_checkpoint": "source_adapter.capture_delivery_mega_bundle_closeout",
...
def write_source_adapter_execution_closure_mega_bundle_closeout_receipts(output_dir: str | Path | None = None, operator_id: str = "operator") -> dict[str, Any]:
    package = example_source_adapter_execution_closure_mega_bundle_closeout_package(operator_id=operator_id)
    output_root = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix="source_adapter_execution_closure_mega_bundle_closeout_receipts_"))
    stored_files = []
    ...
    stored_files.append(write_json(output_root / f"{package['id']}.execution_closure_mega_bundle_closeout_rows.json", package[ROW_KEY]))
    for role, meta in zip(("package", "handoff", ROW_KEY), stored_files):
        meta["role"] = role
    return {
        "schema_version": "source_adapter_execution_closure_mega_bundle_closeout_receipt_writer_v1",
        "id": package["id"],
...
if __name__ == "__main__":
    print(json.dumps(example_source_adapter_execution_closure_mega_bundle_closeout_package(), indent=2, sort_keys=True))
