...
RUNTIME_TITLE = 'Shadow Comment Receipt Finalizer'
RUNTIME_STAGE = 'shadow_comment_receipt_finalizer'
...
class ShadowCommentReceiptFinalizerRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ShadowCommentReceiptFinalizerRecord:
    ...
    return ShadowCommentReceiptFinalizerRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ShadowCommentReceiptFinalizerRecord:
    return build_record(
        "shadow_comment_receipt_finalizer-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "shadow_comment_receipt_finalizer"},
    )


def summarise_record(record: ShadowCommentReceiptFinalizerRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ShadowCommentReceiptFinalizerRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ShadowCommentReceiptFinalizerRecord:
    return build_record(
...
def write_record(path: str | Path, record: ShadowCommentReceiptFinalizerRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ShadowCommentReceiptFinalizerRecord:
