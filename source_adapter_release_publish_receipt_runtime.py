...
SCHEMA_VERSION = "source_adapter_release_publish_receipt_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_release_publish_receipt_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_RELEASE_PUBLISH_RECEIPT_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['upload_receipt', 'library_receipt', 'archive_receipt', 'operator_signoff']


@dataclass(frozen=True)
class ReleasePublishReceiptRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class ReleasePublishReceiptRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = ReleasePublishReceiptRuntimeRow(
            row_id=_stable_id("source_adapter.release_publish_receipt_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="release publish receipt",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "release publish receipt",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.release_publish_receipt_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "release publish receipt",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.release_publish_receipt_runtime.json"
    rows_path = out / f"{package['package_id']}.release_publish_receipt_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.release_publish_receipt_runtime.verification.json"
