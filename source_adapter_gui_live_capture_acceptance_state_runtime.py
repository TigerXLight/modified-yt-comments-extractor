...
RUNTIME_TITLE = 'GUI Live Capture Acceptance State'
RUNTIME_STAGE = 'gui_live_capture_acceptance_state'
...
class GuiLiveCaptureAcceptanceStateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> GuiLiveCaptureAcceptanceStateRecord:
    ...
    return GuiLiveCaptureAcceptanceStateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> GuiLiveCaptureAcceptanceStateRecord:
    return build_record(
        "gui_live_capture_acceptance_state-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "gui_live_capture_acceptance_state"},
    )


def summarise_record(record: GuiLiveCaptureAcceptanceStateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: GuiLiveCaptureAcceptanceStateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> GuiLiveCaptureAcceptanceStateRecord:
    return build_record(
...
def write_record(path: str | Path, record: GuiLiveCaptureAcceptanceStateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> GuiLiveCaptureAcceptanceStateRecord:
