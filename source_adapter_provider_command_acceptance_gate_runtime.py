...
RUNTIME_TITLE = 'Provider Command Acceptance Gate'
RUNTIME_STAGE = 'provider_command_acceptance_gate'
...
class ProviderCommandAcceptanceGateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ProviderCommandAcceptanceGateRecord:
    ...
    return ProviderCommandAcceptanceGateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ProviderCommandAcceptanceGateRecord:
    return build_record(
        "provider_command_acceptance_gate-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "provider_command_acceptance_gate"},
    )


def summarise_record(record: ProviderCommandAcceptanceGateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ProviderCommandAcceptanceGateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ProviderCommandAcceptanceGateRecord:
    return build_record(
...
def write_record(path: str | Path, record: ProviderCommandAcceptanceGateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ProviderCommandAcceptanceGateRecord:
