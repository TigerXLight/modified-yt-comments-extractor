...
RUNTIME_TITLE = 'Roadmap Coverage Acceptance Audit'
RUNTIME_STAGE = 'roadmap_coverage_acceptance_audit'
...
class RoadmapCoverageAcceptanceAuditRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> RoadmapCoverageAcceptanceAuditRecord:
    ...
    return RoadmapCoverageAcceptanceAuditRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> RoadmapCoverageAcceptanceAuditRecord:
    return build_record(
        "roadmap_coverage_acceptance_audit-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "roadmap_coverage_acceptance_audit"},
    )


def summarise_record(record: RoadmapCoverageAcceptanceAuditRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: RoadmapCoverageAcceptanceAuditRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> RoadmapCoverageAcceptanceAuditRecord:
    return build_record(
...
def write_record(path: str | Path, record: RoadmapCoverageAcceptanceAuditRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> RoadmapCoverageAcceptanceAuditRecord:
