...
RUNTIME_TITLE = 'Provider Failure Resolution Gate'
RUNTIME_STAGE = 'provider_failure_resolution_gate'
...
class ProviderFailureResolutionGateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ProviderFailureResolutionGateRecord:
    ...
    return ProviderFailureResolutionGateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ProviderFailureResolutionGateRecord:
    return build_record(
        "provider_failure_resolution_gate-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "provider_failure_resolution_gate"},
    )


def summarise_record(record: ProviderFailureResolutionGateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ProviderFailureResolutionGateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ProviderFailureResolutionGateRecord:
    return build_record(
...
def write_record(path: str | Path, record: ProviderFailureResolutionGateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ProviderFailureResolutionGateRecord:
