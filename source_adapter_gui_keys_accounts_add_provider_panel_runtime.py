...
SCHEMA_VERSION = "source_adapter_gui_keys_accounts_add_provider_panel_runtime_v1"
STATUS = "SOURCE_ADAPTER_GUI_KEYS_ACCOUNTS_ADD_PROVIDER_PANEL_RUNTIME_BUILT"
HANDOFF_STATUS = "SOURCE_ADAPTER_GUI_KEYS_ACCOUNTS_ADD_PROVIDER_PANEL_RUNTIME_READY_FOR_NEXT_IMPLEMENTATION_BUNDLE"
ROW_STATUS = "SOURCE_ADAPTER_GUI_KEYS_ACCOUNTS_ADD_PROVIDER_PANEL_RUNTIME_ROWS_READY"
ROW_KEY = "gui_keys_accounts_add_provider_panel_rows"
DESCRIPTION = "GUI KEYS/ACCOUNTS add-provider panel rows for full catalogue search, add-provider workflow, added-provider display, and account binding"


def build_gui_keys_accounts_add_provider_panel_rows(operator_id: str = "operator") -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for site in named_sites():
        for action_index, action in enumerate(PROVIDER_ACTIONS):
            seed = {"aspect": "gui_keys_accounts_add_provider_panel", "site": site, "action": action, "operator_id": operator_id}
            rows.append({
                "schema_version": "source_adapter_gui_keys_accounts_add_provider_panel_runtime_row_v1",
                "row_id": stable_id("source_adapter.gui_keys_accounts_add_provider_panel.row", seed),
                "row_index": len(rows),
...
def example_source_adapter_gui_keys_accounts_add_provider_panel_runtime_package(operator_id: str = "operator") -> dict[str, Any]:
    rows = build_gui_keys_accounts_add_provider_panel_rows(operator_id=operator_id)
    package = {
        "schema_version": SCHEMA_VERSION,
        "id": stable_id("source_adapter.gui_keys_accounts_add_provider_panel.package", {"rows": rows, "operator_id": operator_id}),
        "status": STATUS,
        "description": DESCRIPTION,
        "row_key": ROW_KEY,
        "gui_keys_accounts_add_provider_panel_rows_count": len(rows),
        ROW_KEY: rows,
        "operator_summary": {
            "schema_version": "source_adapter_gui_keys_accounts_add_provider_panel_runtime_operator_summary_v1",
            "status": STATUS,
        ...
        "handoff": {
            "schema_version": "source_adapter_gui_keys_accounts_add_provider_panel_runtime_handoff_v1",
...
def write_source_adapter_gui_keys_accounts_add_provider_panel_runtime_receipts(output_dir: str | Path | None = None, operator_id: str = "operator") -> dict[str, Any]:
    package = example_source_adapter_gui_keys_accounts_add_provider_panel_runtime_package(operator_id=operator_id)
    output_root = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix="source_adapter_gui_keys_accounts_add_provider_panel_runtime_receipts_"))
    stored_files = []
    ...
    stored_files.append(write_json(output_root / f"{package['id']}.gui_keys_accounts_add_provider_panel_rows.json", package[ROW_KEY]))
    for role, meta in zip(("package", "handoff", ROW_KEY), stored_files):
        meta["role"] = role
    return {
        "schema_version": "source_adapter_gui_keys_accounts_add_provider_panel_runtime_receipt_writer_v1",
        "id": package["id"],
...
if __name__ == "__main__":
    print(json.dumps(example_source_adapter_gui_keys_accounts_add_provider_panel_runtime_package(), indent=2, sort_keys=True))
