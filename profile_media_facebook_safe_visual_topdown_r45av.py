...
MARKER = "YTCE_R45AV_SAFE_VISUAL_TOPDOWN_SEPARATED_BOX"
SCHEMA_VERSION = "facebook_safe_visual_topdown.r45av.v1"
...
def contract() -> Dict[str, Any]:
    return {
        ...
        "r45av_rule": "Open the post, lock to the target story, install a profile/comment-permalink navigation blocker, start at the top of the active comments scroller, click the first safe visible expansion label, skip unsafe false-positive click points that resolve to profile anchors, rescan the same viewport, keep moving downward as comments load, then require a complete top-to-bottom visual audit pass with zero safe visible controls before applying the R45J/R45L comments-box crop and screenshots.",
        ...
        "success_rule": "No final screenshot is accepted until a full visible top-to-bottom audit pass sees zero safe expansion controls; after that the page is visually cleaned so the screenshot contains the separated Facebook-rendered comments box. Unsafe false-positive points resolving to Facebook profile/comment links are skipped and logged instead of ending the run or navigating away.",
...
# R45AV uses the already-working R45J/R45L post-expansion visual cleanup/crop path.
...
() => {
  if (window.__YTCE_R45AV_PROFILE_NAV_BLOCKER_INSTALLED) return {installed:true, already:true};
  const isProfileOrProfileCommentHref = (hrefRaw) => {
    const href = String(hrefRaw || '');
    if (!href) return false;
    let u;
    ...
    for (let i = 0; el && i < 12; i++, el = el.parentElement) {
      if ((el.tagName || '').toLowerCase() === 'a') {
        ...
        if (isProfileOrProfileCommentHref(href)) {
          ...
          console.warn('YTCE_R45AV_BLOCKED_PROFILE_NAV', href);
          return false;
        }
      }
    }
  ...
  window.__YTCE_R45AV_PROFILE_NAV_BLOCKER_INSTALLED = true;
...
def exhaust(page: Any, args: argparse.Namespace, story: str) -> Dict[str, Any]:
    ...
    for step in range(1, args.max_steps + 1):
        ...
        if elapsed > args.expand_max_seconds:
            ...
            emit("R45AV_BLOCKED_VISIBLE_CONTROLS", {"reason":"timeout", "clicked":clicked, "scrolls":scrolls, "audit_pass":audit_pass, "visible_total":probe.get("total"), "visible_counts":probe.get("counts"), "visible_labels":probe.get("labels")})
            return {"status":"blocked", "reason":"timeout", "clicked":clicked, "scrolls":scrolls, "probe":probe}

        guard = _guard(page, story)
        if not guard.get("ok"):
            emit("R45AV_TARGET_SURFACE_LOST", {"step":step, "clicked":clicked, "guard":guard})
            return {"status":"blocked", "reason":"target_surface_lost", "clicked":clicked, "guard":guard}

        probe = _probe(page, hint, list(unsafe_skip_keys))
        emit("R45AV_VIEWPORT_SCAN", {"step":step, "audit_pass":audit_pass, "elapsed_seconds":round(elapsed,2), "visible_total":probe.get("total",0), "visible_counts":probe.get("counts",{}), "visible_labels":(probe.get("labels") or [])[:30], "scrollTop":(probe.get("scroller") or {}).get("scrollTop"), "scrollHeight":(probe.get("scroller") or {}).get("scrollHeight")})
        ...
        if int(probe.get("total") or 0) > 0:
            ...
            if not point_safety.get("ok"):
                ...
                unsafe_skipped += 1
                emit("R45AV_UNSAFE_CLICK_POINT_SKIPPED", {"step": step, "audit_pass": audit_pass, "unsafe_skipped": unsafe_skipped, "item": item, "point_safety": point_safety, "skip_key_count": len(unsafe_skip_keys), "elapsed_seconds": round(time.monotonic()-start,2)})
            ...
            emit("R45AV_CLICK_FIRST_VISIBLE", {"step":step, "audit_pass":audit_pass, "clicked":clicked, "category":item.get("category"), "label":item.get("label"), "x":item.get("x"), "y":item.get("y"), "target_ok":after_guard.get("ok"), "elapsed_seconds":round(time.monotonic()-start,2)})
        ...
        scrolls += 1
        time.sleep(args.scroll_settle_seconds)
        emit("R45AV_SCROLL_DOWN", {"step":step, "audit_pass":audit_pass, "scrolls":scrolls, **scr, "elapsed_seconds":round(time.monotonic()-start,2)})
        ...
        if scr.get("atBottom") or no_scroll_cycles >= 2:
            ...
            if clean_bottom_once:
                emit("R45AV_COMPLETE_AFTER_FULL_VISUAL_AUDIT", {"status":"PASS_FULL_TOPDOWN_VISUAL_AUDIT_ZERO_VISIBLE_CONTROLS", "clicked":clicked, "scrolls":scrolls, "audit_passes":audit_pass, "unsafe_skipped":unsafe_skipped, "elapsed_seconds":round(time.monotonic()-start,2)})
            ...
            last_scroll = None
            top = _scroll(page, hint, "top")
            emit("R45AV_AUDIT_RESTART_TOP", {"audit_pass":audit_pass, **top, "elapsed_seconds":round(time.monotonic()-start,2)})
            time.sleep(args.scroll_settle_seconds)

    probe = _probe(page, hint, list(unsafe_skip_keys))
    emit("R45AV_BLOCKED_VISIBLE_CONTROLS", {"reason":"max_steps", "clicked":clicked, "scrolls":scrolls, "visible_total":probe.get("total"), "visible_counts":probe.get("counts"), "visible_labels":probe.get("labels")})
...
def _capture_isolated_page_tiles(page: Any, run_dir: Path, settle: float, prefix: str = "r45av_separated_comments_tile", max_tiles: int = 1200) -> List[str]:
    paths: List[str] = []
    try:
        total_height = int(page.evaluate('() => Math.max(document.body.scrollHeight, document.documentElement.scrollHeight)'))
        view_h = int(page.evaluate('() => window.innerHeight || 900'))
    except Exception:
        total_height, view_h = 0, 900
    step = max(360, min(1200, view_h - 80))
    y = 0
    seen = set()
    for i in range(1, max_tiles + 1):
        ...
        p = run_dir / f"{prefix}_{i:04d}_y{int(y):08d}.png"
        try:
        ...
        paths.append(str(p))
        try:
            now = int(page.evaluate('() => window.scrollY || document.documentElement.scrollTop || document.body.scrollTop || 0'))
            total_height = int(page.evaluate('() => Math.max(document.body.scrollHeight, document.documentElement.scrollHeight)'))
            view_h = int(page.evaluate('() => window.innerHeight || 900'))
        except Exception:
            now = y
        if now + view_h >= total_height - 8:
            break
        key = (now, total_height)
        if key in seen:
            break
        seen.add(key)
        y = min(max(0, total_height - view_h), now + step)
    if paths:
        zip_path = run_dir / "r45av_separated_comments_tiles.zip"
...
def capture_separated_comments_box(page: Any, run_dir: Path, settle: float, make_tiles: bool) -> Dict[str, Any]:
    ...
    try:
        result["clean_dom_path"] = str(run_dir / "r45av_separated_comments_clean_dom.html")
    ...
    try:
        result["clean_text_path"] = str(run_dir / "r45av_separated_comments_visible_text.txt")
    ...
    result["comments_box_screenshot_path"] = _safe_locator_screenshot(
        page,
        '[data-r45j-preserved-comments-root="true"]',
        run_dir / "facebook_preserved_visual_comments_column.png",
        warnings,
    )
    try:
        p = run_dir / "facebook_preserved_visual_comments_only_full_page.png"
        page.screenshot(path=str(p), full_page=True)
        result["comments_only_full_page_screenshot_path"] = str(p)
    except Exception as e:
        warnings.append(f"separated_comments_box_full_page_screenshot_warning={e}")
    if make_tiles:
        result["comments_only_tile_screenshot_paths"] = _capture_isolated_page_tiles(page, run_dir, settle)
        result["comments_only_tile_screenshot_count"] = len([p for p in result.get("comments_only_tile_screenshot_paths", []) if p.lower().endswith('.png')])
    result["status"] = "PASS_R45AV_SEPARATED_COMMENTS_BOX_CAPTURED" if (result.get("comments_box_screenshot_path") or result.get("comments_only_full_page_screenshot_path") or result.get("comments_only_tile_screenshot_paths")) else "WARNING_NO_SEPARATED_SCREENSHOT"
    return result
...
def run_live(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_dir = out_root / f"r45av_safe_visual_topdown_{stamp}"
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                nav_block = page.evaluate(PROFILE_NAV_BLOCKER_JS)
                emit("R45AV_PROFILE_NAV_BLOCKER_INITIAL", nav_block)
            except Exception as e:
                emit("R45AV_PROFILE_NAV_BLOCKER_INITIAL", {"installed": False, "error": str(e)})
            ...
            if summary.get("status") != "pass":
                ...
                rp = run_dir / "r45av_safe_visual_topdown_receipt.json"
                rp.write_text(json.dumps(receipt, ensure_ascii=False, indent=2), encoding="utf-8")
                receipt["receipt_path"] = str(rp)
                print("R45AV blocked: visual audit did not finish cleanly. No final screenshot accepted.", flush=True)
                return receipt
            ...
            if str(separated.get("status", "")).startswith("PASS"):
                receipt["status"] = "PASS_R45AV_FULL_VISUAL_AUDIT_AND_SEPARATED_COMMENTS_BOX_SCREENSHOT"
            else:
                receipt["status"] = "PASS_R45AV_FULL_VISUAL_AUDIT_WITH_SEPARATED_SCREENSHOT_WARNINGS"
            rp = run_dir / "r45av_safe_visual_topdown_receipt.json"
            rp.write_text(json.dumps(receipt, ensure_ascii=False, indent=2), encoding="utf-8")
            receipt["receipt_path"] = str(rp)
            print("YTCE_R45AV_VISUAL_TOPDOWN_SKIP_UNSAFE_DONE", flush=True)
...
def self_test(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    checks = [
        ...
        {"name":"full_visual_audit_required", "status":"pass" if "R45AV_COMPLETE_AFTER_FULL_VISUAL_AUDIT" in text else "fail"},
        {"name":"target_surface_lost_failfast", "status":"pass" if "R45AV_TARGET_SURFACE_LOST" in text else "fail"},
        {"name":"profile_nav_blocker_present", "status":"pass" if "R45AV_PROFILE_NAV_BLOCKER" in text and "profile_or_profile_comment_anchor_under_click_point" in text else "fail"},
        {"name":"unsafe_click_skip_present", "status":"pass" if "R45AV_UNSAFE_CLICK_POINT_SKIPPED" in text and "unsafe_skip_keys" in text else "fail"},
        {"name":"separated_comments_box_present", "status":"pass" if "capture_separated_comments_box" in text and "facebook_preserved_visual_comments_column.png" in text else "fail"},
        {"name":"r45j_r45l_visual_cleanup_reused", "status":"pass" if "data-r45j-preserved-comments-root" in text and "r45l_comment_column_crop_used" in text else "fail"},
        {"name":"no_hidden_platform_api", "status":"pass" if contract()["hidden_platform_api_scraping_enabled"] is False else "fail"},
    ]
    status = "PASS_R45AV_SELF_TEST" if all(c["status"] == "pass" for c in checks) else "FAIL_R45AV_SELF_TEST"
    ...
    (out / "r45av_self_test.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    print(MARKER); print(status); print(json.dumps(result, ensure_ascii=False, indent=2))
    return result


def parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="R45AV visual top-down Facebook comments expansion and screenshot")
    ...
    ap.add_argument("--output-root", default="profile_media_live_captures/r45av_safe_visual_topdown")
    return ap
