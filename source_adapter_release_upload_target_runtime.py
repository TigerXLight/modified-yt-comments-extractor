...
SCHEMA_VERSION = "source_adapter_release_upload_target_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_release_upload_target_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_RELEASE_UPLOAD_TARGET_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['local_folder', 'github_release', 'library_publish', 'manual_handoff']


@dataclass(frozen=True)
class ReleaseUploadTargetRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class ReleaseUploadTargetRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = ReleaseUploadTargetRuntimeRow(
            row_id=_stable_id("source_adapter.release_upload_target_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="release upload target",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "release upload target",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.release_upload_target_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "release upload target",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.release_upload_target_runtime.json"
    rows_path = out / f"{package['package_id']}.release_upload_target_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.release_upload_target_runtime.verification.json"
