...
def render_asr_term_coverage_summary(records: Sequence, *, output_format: str) -> str:
    result = build_asr_term_coverage_summary(records)
    if output_format == "markdown":
        return build_asr_term_coverage_summary_markdown(result)
    if output_format == "text":
        return build_asr_term_coverage_summary_text(result)
    if output_format == "json":
        return json.dumps(
            asr_term_coverage_summary_to_dict(result),
            indent=2,
            sort_keys=True,
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render local/manual ASR term coverage records without provider calls.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        rendered = render_asr_term_coverage_summary(records, output_format=args.format)
        if args.output:
