...
R45AN_ALL_LOADED_TEXT_EXPAND_COUNT_JS = r"""
() => {
  const patterns = [
    {key: 'view_all_replies', rx: /^view all\s+\d+\s+repl/i},
    {key: 'view_hidden', rx: /^view hidden\s+(repl|comments?)/i},
  ...
  const rectLoaded = (r) => r && r.width > 0 && r.height > 0;
  const rows = [];
  const addRow = (category, label, r, source) => {
    const pageTop = Math.round(r.top + window.scrollY);
    const pageLeft = Math.round(r.left + window.scrollX);
    // Dedupe the same visible Facebook label found both as text-node and role=button.
    for (const old of rows) {
      if (old.label === label && Math.abs(old.pageTop - pageTop) <= 24 && Math.abs(old.pageLeft - pageLeft) <= 160) return;
    }
    rows.push({
      category, label,
      pageTop, pageLeft,
  ...
  const root = document.querySelector('[data-r45j-preserved-comments-root="true"]') || document.body || document.documentElement;
  ...
  while ((node = walker.nextNode())) {
    ...
    for (const r of rects) {
      if (rectLoaded(r)) { addRow(c.category, c.label, r, 'text_node_all_loaded'); break; }
    }
  }

  const els = Array.from(root.querySelectorAll('div[role="button"], span[role="button"], a[role="button"], button, a, [tabindex="0"]'));
  for (const el of els) {
    if (!styleVisible(el)) continue;
    const raw = norm(el.innerText || el.getAttribute('aria-label') || el.getAttribute('title') || el.textContent || '');
    const c = classify(raw);
    if (!c) continue;
    const r = el.getBoundingClientRect();
    if (!rectLoaded(r)) continue;
    addRow(c.category, c.label, r, 'clickable_element_all_loaded');
  }

  rows.sort((a,b) => a.pageTop - b.pageTop || a.pageLeft - b.pageLeft || a.label.localeCompare(b.label));
  const counts = {};
  for (const p of ['view_all_replies','view_hidden','view_more_replies','view_number_replies','replied_buckets']) counts[p] = 0;
  for (const r of rows) counts[r.category] = (counts[r.category] || 0) + 1;
  return {
    total_count: rows.length,
    counts,
    labels: rows.slice(0, 120).map(r => r.label),
    items: rows.slice(0, 80),
    url: location.href,
    viewport: {width: window.innerWidth, height: window.innerHeight},
    scroll: {x: Math.round(window.scrollX), y: Math.round(window.scrollY)},
    root_is_preserved_comments: !!document.querySelector('[data-r45j-preserved-comments-root="true"]'),
    body_text_chars: document.body && document.body.innerText ? document.body.innerText.length : 0
  };
}
"""
...
R45AO_CLICK_ALL_LOADED_EXPAND_CONTROLS_JS = r"""
async (opts) => {
  opts = opts || {};
  const maxPasses = Number(opts.maxPasses || 80);
  const waitAfterClickMs = Number(opts.waitAfterClickMs || 650);
  const waitAfterScrollMs = Number(opts.waitAfterScrollMs || 180);
  const patterns = [
    {key: 'view_all_replies', rx: /^view all\s+\d+\s+repl/i},
    {key: 'view_hidden', rx: /^view hidden\s+(repl|comments?)/i},
    {key: 'view_more_replies', rx: /^view more repl/i},
  ...
  const classify = (text) => {
    ...
    if (/^(like|reply|share|send|comment|edited|follow|message|copy link)$/i.test(t)) return null;
    const lower = t.toLowerCase();
    for (const p of patterns) if (p.rx.test(lower)) return {category:p.key, label:t};
    return null;
  };
  const styleVisible = (el) => {
    if (!el || el.nodeType !== 1) return false;
  ...
  const rows = () => {
    const out = [];
    const seen = [];
    const addRow = (category, label, r, source, el) => {
      const pageTop = Math.round(r.top + window.scrollY);
      const pageLeft = Math.round(r.left + window.scrollX);
      for (const old of seen) {
        if (old.label === label && Math.abs(old.pageTop - pageTop) <= 24 && Math.abs(old.pageLeft - pageLeft) <= 160) return;
      }
      seen.push({label, pageTop, pageLeft});
      out.push({category, label, pageTop, pageLeft, top: Math.round(r.top), left: Math.round(r.left), bottom: Math.round(r.bottom), source, el});
    };
    const rt = root();
    const walker = document.createTreeWalker(rt, NodeFilter.SHOW_TEXT, {
      acceptNode(node) { return classify(node.nodeValue) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT; }
    });
    let node;
    while ((node = walker.nextNode())) {
      const parent = node.parentElement;
      ...
      for (const r of rects) {
        if (rectLoaded(r)) { addRow(c.category, c.label, r, 'text_node_all_loaded', parent); break; }
      }
    }
    const els = Array.from(rt.querySelectorAll('div[role="button"], span[role="button"], a[role="button"], button, a, [tabindex="0"]'));
    for (const el of els) {
      if (!styleVisible(el)) continue;
      const raw = norm(el.innerText || el.getAttribute('aria-label') || el.getAttribute('title') || el.textContent || '');
      const c = classify(raw);
      ...
      const r = el.getBoundingClientRect();
      if (!rectLoaded(r)) continue;
      addRow(c.category, c.label, r, 'clickable_element_all_loaded', el);
    }
    out.sort((a,b) => a.pageTop - b.pageTop || a.pageLeft - b.pageLeft || a.label.localeCompare(b.label));
    return out;
  };
  const clickableFor = (el) => {
    let n = el;
    for (let i = 0; n && i < 8; i++, n = n.parentElement) {
      if (!styleVisible(n)) continue;
      const role = (n.getAttribute('role') || '').toLowerCase();
      const tag = (n.tagName || '').toLowerCase();
      const label = norm(n.innerText || n.getAttribute('aria-label') || n.textContent || '');
      if ((role === 'button' || tag === 'button' || tag === 'a' || n.getAttribute('tabindex') === '0') && classify(label)) return n;
    }
    return el;
  };
  const clicked = [];
  ...
  for (let pass = 1; pass <= maxPasses; pass++) {
    ...
    const row = current[0];
    const key = row.label + '|' + row.pageTop + '|' + row.pageLeft;
    const target = clickableFor(row.el);
    if (!target) break;
    try { target.scrollIntoView({block:'center', inline:'nearest'}); } catch (e) { try { window.scrollTo(0, Math.max(0, row.pageTop - Math.round(window.innerHeight * 0.45))); } catch (_) {} }
    await sleep(waitAfterScrollMs);
    let r = target.getBoundingClientRect();
    let x = Math.max(4, Math.min(window.innerWidth - 4, r.left + Math.min(Math.max(20, r.width / 2), 160)));
    let y = Math.max(4, Math.min(window.innerHeight - 4, r.top + Math.min(Math.max(8, r.height / 2), 28)));
    const beforeText = document.body && document.body.innerText ? document.body.innerText.length : 0;
    ...
    const afterText = document.body && document.body.innerText ? document.body.innerText.length : 0;
    clicked.push({pass, label: row.label, before_total: current.length, before_text_chars: beforeText, after_text_chars: afterText, pageTop: row.pageTop});
    const afterRows = rows();
    const firstAfter = afterRows[0] ? (afterRows[0].label + '|' + afterRows[0].pageTop + '|' + afterRows[0].pageLeft) : '';
    if (key === previousFirstKey && firstAfter === key && afterText === beforeText) repeatedNoChange += 1;
    else repeatedNoChange = 0;
    previousFirstKey = firstAfter;
    if (repeatedNoChange >= 3) break;
  }
  const remaining = rows().map(r => ({category:r.category, label:r.label, pageTop:r.pageTop, pageLeft:r.pageLeft, source:r.source}));
  const counts = {view_all_replies:0, view_hidden:0, view_more_replies:0, view_number_replies:0, replied_buckets:0};
  for (const r of remaining) counts[r.category] = (counts[r.category] || 0) + 1;
  return {clicked_count: clicked.length, clicked_labels: clicked.map(c => c.label).slice(0,120), clicked: clicked.slice(0,120), remaining_count: remaining.length, remaining_counts: counts, remaining_labels: remaining.map(r => r.label).slice(0,120), remaining_items: remaining.slice(0,80)};
}
"""
...
def contract() -> Dict[str, Any]:
    ...
    base.update({
        ...
        'r45ao_click_leftovers_rule': 'R45AO clicks whole-loaded leftover View all/View hidden/View more/replied controls before visual cleanup, then recounts before pausing.',
...
def run_live(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    with sync_playwright() as p:
        ...
        page.on('console', lambda msg: print(msg.text) if (msg.text.startswith('R45H_PROGRESS') or msg.text.startswith('R45AK') or msg.text.startswith('R45AL') or msg.text.startswith('R45AM') or msg.text.startswith('R45AO')) else None)
        ...
        if args.auto_expand:
            ...
            try:
                ...
                post_all_loaded_report = {}
                try:
                    post_all_loaded_report = page.evaluate(R45AN_ALL_LOADED_TEXT_EXPAND_COUNT_JS) or {}
                except Exception as e:
                    post_all_loaded_report = {'all_loaded_text_count_error': str(e)[:1000]}
                print('R45AN_POST_EXPAND_ALL_LOADED_TEXT_CONTROL_COUNT ' + json.dumps(post_all_loaded_report, ensure_ascii=False))
                try:
                    all_loaded_total = int((post_all_loaded_report or {}).get('total_count') or 0)
                except Exception:
                    all_loaded_total = 0
                if all_loaded_total:
                    print('R45AN_ALL_LOADED_MISSED_REPORT ' + json.dumps({
                        'status': 'NEEDS_MORE_EXPANSION',
                        'all_loaded_missed_expand_count': all_loaded_total,
                        'counts': (post_all_loaded_report or {}).get('counts') or {},
                        'labels': list((post_all_loaded_report or {}).get('labels') or [])[:120],
                    }, ensure_ascii=False))
                    warnings.append(f'all_loaded_text_expand_controls_remaining={all_loaded_total}')
                    try:
                        print('R45AO_CLICK_LEFTOVER_ALL_LOADED_START ' + json.dumps({'all_loaded_missed_expand_count': all_loaded_total}, ensure_ascii=False))
                        leftover_click_report = page.evaluate(R45AO_CLICK_ALL_LOADED_EXPAND_CONTROLS_JS, {
                            'maxPasses': args.leftover_expand_max_passes,
                            'waitAfterClickMs': int(args.leftover_expand_wait_seconds * 1000),
                            'waitAfterScrollMs': 200,
                        }) or {}
                        print('R45AO_CLICK_LEFTOVER_ALL_LOADED_DONE ' + json.dumps(leftover_click_report, ensure_ascii=False))
                        post_all_loaded_report = page.evaluate(R45AN_ALL_LOADED_TEXT_EXPAND_COUNT_JS) or {}
                        print('R45AO_AFTER_LEFTOVER_ALL_LOADED_TEXT_CONTROL_COUNT ' + json.dumps(post_all_loaded_report, ensure_ascii=False))
                        all_loaded_total = int((post_all_loaded_report or {}).get('total_count') or 0)
                        print('R45AO_AFTER_LEFTOVER_MISSED_REPORT ' + json.dumps({
                            'status': 'NEEDS_MORE_EXPANSION' if all_loaded_total else 'PASS_NO_ALL_LOADED_EXPAND_CONTROLS',
                            'all_loaded_missed_expand_count': all_loaded_total,
                            'counts': (post_all_loaded_report or {}).get('counts') or {},
                            'labels': list((post_all_loaded_report or {}).get('labels') or [])[:120] if all_loaded_total else [],
                        }, ensure_ascii=False))
                        if all_loaded_total:
                            warnings.append(f'all_loaded_leftover_clicks_still_remaining={all_loaded_total}')
                    except Exception as e:
                        warnings.append(f'leftover_all_loaded_click_warning={e}')
                        print('R45AO_CLICK_LEFTOVER_ALL_LOADED_FAILED ' + json.dumps({'error': str(e)[:2000]}, ensure_ascii=False))
                else:
                    print('R45AN_ALL_LOADED_MISSED_REPORT ' + json.dumps({
                        'status': 'PASS_NO_ALL_LOADED_EXPAND_CONTROLS',
                        'all_loaded_missed_expand_count': 0,
                        'counts': (post_all_loaded_report or {}).get('counts') or {},
                        'labels': [],
                    }, ensure_ascii=False))
        ...
        try:
            ...
            try:
                post_clean_all_loaded_report = page.evaluate(R45AN_ALL_LOADED_TEXT_EXPAND_COUNT_JS) or {}
            except Exception as e:
                post_clean_all_loaded_report = {'post_clean_all_loaded_text_count_error': str(e)[:1000]}
            print('R45AN_POST_CLEAN_ALL_LOADED_TEXT_CONTROL_COUNT ' + json.dumps(post_clean_all_loaded_report, ensure_ascii=False))
            try:
                post_clean_total = int((post_clean_all_loaded_report or {}).get('total_count') or 0)
            except Exception:
                post_clean_total = 0
            print('R45AN_POST_CLEAN_ALL_LOADED_MISSED_REPORT ' + json.dumps({
                'status': 'NEEDS_MORE_EXPANSION' if post_clean_total else 'PASS_NO_ALL_LOADED_EXPAND_CONTROLS',
                'all_loaded_missed_expand_count': post_clean_total,
                'counts': (post_clean_all_loaded_report or {}).get('counts') or {},
                'labels': list((post_clean_all_loaded_report or {}).get('labels') or [])[:120] if post_clean_total else [],
            }, ensure_ascii=False))
            if post_clean_total:
                warnings.append(f'post_clean_all_loaded_text_expand_controls_remaining={post_clean_total}')
        except Exception as e:
...
def run_self_test(args: argparse.Namespace) -> Dict[str, Any]:
    checks = [
        ...
        {'name': 'r45ao_click_leftovers_present', 'status': 'pass' if 'R45AO_CLICK_ALL_LOADED_EXPAND_CONTROLS_JS' in globals() and 'R45AO_CLICK_LEFTOVER_ALL_LOADED_START' in open(__file__, encoding='utf-8').read() else 'fail'},
        {'name': 'r45an_all_loaded_text_count_present', 'status': 'pass' if 'R45AN_ALL_LOADED_TEXT_EXPAND_COUNT_JS' in globals() and 'R45AN_POST_EXPAND_ALL_LOADED_TEXT_CONTROL_COUNT' in open(__file__, encoding='utf-8').read() and 'R45AN_POST_CLEAN_ALL_LOADED_TEXT_CONTROL_COUNT' in open(__file__, encoding='utf-8').read() else 'fail'},
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.description = 'R45AO Facebook preserved visual screenshot runner: autostart, all-loaded count, and leftover reply opener click pass'
    ap.set_defaults(output_root='profile_media_live_captures/r45ao_legacy_clickleftovers_autostart_20260919')
    ...
    ap.add_argument('--leftover-expand-max-passes', type=int, default=120)
    ap.add_argument('--leftover-expand-wait-seconds', type=float, default=0.7)
