...
ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_VERIFIER_STORE_SCHEMA_VERSION = "online_asr_keys_accounts_review_verifier_store_v1"
ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_VERIFIER_FILENAME = "online_asr_keys_accounts_review_verification_report.json"
...
class OnlineASRKeysAccountsReviewVerifierStoredFile:
    filename: str
    file_role: str
    sha256: str
    byte_count: int
    schema_version: str = ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_VERIFIER_STORE_SCHEMA_VERSION
...
@dataclass(frozen=True)
class OnlineASRKeysAccountsReviewVerifierStoreResult:
    package_id: str
    ...
    file_count: int
    files: tuple[OnlineASRKeysAccountsReviewVerifierStoredFile, ...]
    schema_version: str = ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_VERIFIER_STORE_SCHEMA_VERSION
    ...
    def to_summary_text(self) -> str:
        return "\n".join(
            (
                "Online ASR KEYS/ACCOUNTS review verifier stored",
...
def build_online_asr_keys_accounts_review_verifier_store_payloads(
    report: OnlineASRKeysAccountsReviewVerificationReport,
) -> dict[str, str]:
    """Build stable verifier report JSON payload text without writing files."""
    return {
        ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_VERIFIER_FILENAME: online_asr_keys_accounts_review_verification_report_to_json(report),
    }


def write_online_asr_keys_accounts_review_verification_report(
    report: OnlineASRKeysAccountsReviewVerificationReport,
    output_directory: str | Path,
    *,
    allow_overwrite: bool = True,
) -> OnlineASRKeysAccountsReviewVerifierStoreResult:
    """Write the metadata-only Online ASR KEYS/ACCOUNTS verifier report.
    ...
    payload_texts = build_online_asr_keys_accounts_review_verifier_store_payloads(report)
    stored_files: list[OnlineASRKeysAccountsReviewVerifierStoredFile] = []
    for filename, payload_text in payload_texts.items():
        file_path = output_path / filename
        if file_path.exists() and not allow_overwrite:
            raise FileExistsError(f"Refusing to overwrite existing review verifier file: {filename}")
        file_path.write_text(payload_text, encoding="utf-8")
        stored_files.append(
            OnlineASRKeysAccountsReviewVerifierStoredFile(
                filename=filename,
    ...
    return OnlineASRKeysAccountsReviewVerifierStoreResult(
        package_id=report.package_id,
        selected_provider_id=report.selected_provider_id,
        review_verdict=report.review_verdict,
        issue_count=report.issue_count,
        output_directory_role="user_selected_online_asr_keys_accounts_review_verifier_directory",
        file_count=len(stored_files),
        files=tuple(stored_files),
    )


def online_asr_keys_accounts_review_verifier_store_result_to_json(
    result: OnlineASRKeysAccountsReviewVerifierStoreResult,
) -> str:
