...
RUNTIME_TITLE = 'KEYS Accounts Add Provider Receipt'
RUNTIME_STAGE = 'keys_accounts_add_provider_receipt'
...
class KeysAccountsAddProviderReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> KeysAccountsAddProviderReceiptRecord:
    ...
    return KeysAccountsAddProviderReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> KeysAccountsAddProviderReceiptRecord:
    return build_record(
        "keys_accounts_add_provider_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "keys_accounts_add_provider_receipt"},
    )


def summarise_record(record: KeysAccountsAddProviderReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: KeysAccountsAddProviderReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> KeysAccountsAddProviderReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: KeysAccountsAddProviderReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> KeysAccountsAddProviderReceiptRecord:
