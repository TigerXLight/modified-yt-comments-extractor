...
RUNTIME_TITLE = 'Operator Approval Acceptance Gate'
RUNTIME_STAGE = 'operator_approval_acceptance_gate'
...
class OperatorApprovalAcceptanceGateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> OperatorApprovalAcceptanceGateRecord:
    ...
    return OperatorApprovalAcceptanceGateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> OperatorApprovalAcceptanceGateRecord:
    return build_record(
        "operator_approval_acceptance_gate-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "operator_approval_acceptance_gate"},
    )


def summarise_record(record: OperatorApprovalAcceptanceGateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: OperatorApprovalAcceptanceGateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> OperatorApprovalAcceptanceGateRecord:
    return build_record(
...
def write_record(path: str | Path, record: OperatorApprovalAcceptanceGateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> OperatorApprovalAcceptanceGateRecord:
