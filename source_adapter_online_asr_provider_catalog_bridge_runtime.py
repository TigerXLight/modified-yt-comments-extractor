...
RUNTIME_TITLE = 'Online ASR Provider Catalog Bridge'
RUNTIME_STAGE = 'online_asr_provider_catalog_bridge'
...
class OnlineAsrProviderCatalogBridgeRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> OnlineAsrProviderCatalogBridgeRecord:
    ...
    return OnlineAsrProviderCatalogBridgeRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> OnlineAsrProviderCatalogBridgeRecord:
    return build_record(
        "online_asr_provider_catalog_bridge-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "online_asr_provider_catalog_bridge"},
    )


def summarise_record(record: OnlineAsrProviderCatalogBridgeRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: OnlineAsrProviderCatalogBridgeRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> OnlineAsrProviderCatalogBridgeRecord:
    return build_record(
...
def write_record(path: str | Path, record: OnlineAsrProviderCatalogBridgeRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> OnlineAsrProviderCatalogBridgeRecord:
