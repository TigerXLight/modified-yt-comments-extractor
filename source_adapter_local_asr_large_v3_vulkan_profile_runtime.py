...
RUNTIME_TITLE = 'Local ASR Large V3 Vulkan Profile'
RUNTIME_STAGE = 'local_asr_large_v3_vulkan_profile'
...
class LocalAsrLargeV3VulkanProfileRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> LocalAsrLargeV3VulkanProfileRecord:
    ...
    return LocalAsrLargeV3VulkanProfileRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> LocalAsrLargeV3VulkanProfileRecord:
    return build_record(
        "local_asr_large_v3_vulkan_profile-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "local_asr_large_v3_vulkan_profile"},
    )


def summarise_record(record: LocalAsrLargeV3VulkanProfileRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: LocalAsrLargeV3VulkanProfileRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> LocalAsrLargeV3VulkanProfileRecord:
    return build_record(
...
def write_record(path: str | Path, record: LocalAsrLargeV3VulkanProfileRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> LocalAsrLargeV3VulkanProfileRecord:
