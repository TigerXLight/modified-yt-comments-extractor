...
RUNTIME_TITLE = 'Evidence Database Release Receipt'
RUNTIME_STAGE = 'evidence_database_release_receipt'
...
class EvidenceDatabaseReleaseReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> EvidenceDatabaseReleaseReceiptRecord:
    ...
    return EvidenceDatabaseReleaseReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> EvidenceDatabaseReleaseReceiptRecord:
    return build_record(
        "evidence_database_release_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "evidence_database_release_receipt"},
    )


def summarise_record(record: EvidenceDatabaseReleaseReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: EvidenceDatabaseReleaseReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> EvidenceDatabaseReleaseReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: EvidenceDatabaseReleaseReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> EvidenceDatabaseReleaseReceiptRecord:
