...
RUNTIME_TITLE = 'MSN Comment Shadow Receipt'
RUNTIME_STAGE = 'msn_comment_shadow_receipt'
...
class MsnCommentShadowReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    return MsnCommentShadowReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> MsnCommentShadowReceiptRecord:
    return build_record(
        "msn_comment_shadow_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "msn_comment_shadow_receipt"},
    )


def summarise_record(record: MsnCommentShadowReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: MsnCommentShadowReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> MsnCommentShadowReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: MsnCommentShadowReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> MsnCommentShadowReceiptRecord:
