...
async (opts) => {
  ...
  const clickCandidates = async () => {
    ...
    for (const item of candidates) {
      ...
      try {
        // R45AI: the old engine worked, but scrollIntoView could make the
        // browser jump while clicking a batch. Click only controls that are
        // already visible in the current viewport, in top-to-bottom order.
        const r = item.el.getBoundingClientRect();
        if (r.bottom < 8 || r.top > (window.innerHeight - 8) || r.right < 8 || r.left > (window.innerWidth - 8)) continue;
    ...
    return {candidate_count: candidates.length, clicked, clicked_labels: labels.slice(0, 30), r45ai_no_scroll_into_view: true, r45ai_top_to_bottom_visible_order: true};
  };
  const stats = [];
  let totalClicks = 0;
...
def contract() -> Dict[str, Any]:
    ...
    base.update({
        ...
        'r45ai_fast_ordered_legacy_rule': 'R45AI keeps the working 2026-09-19 legacy bounded modal engine, but clicks visible expansion controls in viewport top-to-bottom order without scrollIntoView jumps and uses faster conservative delays.',
