...
RUNTIME_TITLE = 'ASR Source Traceability Receipt'
RUNTIME_STAGE = 'asr_source_traceability_receipt'
...
class AsrSourceTraceabilityReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> AsrSourceTraceabilityReceiptRecord:
    ...
    return AsrSourceTraceabilityReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> AsrSourceTraceabilityReceiptRecord:
    return build_record(
        "asr_source_traceability_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "asr_source_traceability_receipt"},
    )


def summarise_record(record: AsrSourceTraceabilityReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: AsrSourceTraceabilityReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> AsrSourceTraceabilityReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: AsrSourceTraceabilityReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> AsrSourceTraceabilityReceiptRecord:
