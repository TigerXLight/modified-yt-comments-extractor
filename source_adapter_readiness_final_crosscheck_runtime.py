...
RUNTIME_TITLE = 'Readiness Final Crosscheck'
RUNTIME_STAGE = 'readiness_final_crosscheck'
...
class ReadinessFinalCrosscheckRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ReadinessFinalCrosscheckRecord:
    ...
    return ReadinessFinalCrosscheckRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ReadinessFinalCrosscheckRecord:
    return build_record(
        "readiness_final_crosscheck-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "readiness_final_crosscheck"},
    )


def summarise_record(record: ReadinessFinalCrosscheckRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ReadinessFinalCrosscheckRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ReadinessFinalCrosscheckRecord:
    return build_record(
...
def write_record(path: str | Path, record: ReadinessFinalCrosscheckRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ReadinessFinalCrosscheckRecord:
