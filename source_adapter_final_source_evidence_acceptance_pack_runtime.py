...
RUNTIME_TITLE = 'Final Source Evidence Acceptance Pack'
RUNTIME_STAGE = 'final_source_evidence_acceptance_pack'
...
class FinalSourceEvidenceAcceptancePackRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> FinalSourceEvidenceAcceptancePackRecord:
    ...
    return FinalSourceEvidenceAcceptancePackRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> FinalSourceEvidenceAcceptancePackRecord:
    return build_record(
        "final_source_evidence_acceptance_pack-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "final_source_evidence_acceptance_pack"},
    )


def summarise_record(record: FinalSourceEvidenceAcceptancePackRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: FinalSourceEvidenceAcceptancePackRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> FinalSourceEvidenceAcceptancePackRecord:
    return build_record(
...
def write_record(path: str | Path, record: FinalSourceEvidenceAcceptancePackRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> FinalSourceEvidenceAcceptancePackRecord:
