...
RUNTIME_TITLE = 'Source Chain Acceptance Audit'
RUNTIME_STAGE = 'source_chain_acceptance_audit'
...
class SourceChainAcceptanceAuditRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> SourceChainAcceptanceAuditRecord:
    ...
    return SourceChainAcceptanceAuditRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> SourceChainAcceptanceAuditRecord:
    return build_record(
        "source_chain_acceptance_audit-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "source_chain_acceptance_audit"},
    )


def summarise_record(record: SourceChainAcceptanceAuditRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: SourceChainAcceptanceAuditRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> SourceChainAcceptanceAuditRecord:
    return build_record(
...
def write_record(path: str | Path, record: SourceChainAcceptanceAuditRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> SourceChainAcceptanceAuditRecord:
