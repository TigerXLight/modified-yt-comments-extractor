...
async (opts) => {
  ...
  const isReportableExpandLabel = (text) => {
    const s = norm(text);
    if (!s) return false;
    if (/^see more\b/.test(s)) return false;
    return /\bview\s+(all\s+)?\d+\s+repl(?:y|ies)\b/.test(s)
      || /\bview\s+\d+\s+more\s+repl(?:y|ies)\b/.test(s)
      || /\bview\s+hidden\s+repl(?:y|ies)\b/.test(s)
      || /\bview\s+more\s+repl(?:y|ies)\b/.test(s)
      || /\bview\s+hidden\s+comments?\b/.test(s)
      || /\bview\s+(all\s+)?\d+\s+comments?\b/.test(s)
      || /\breplied\s*[·•-]\s*\d+\s+repl(?:y|ies)\b/.test(s);
  };
  const summarizeVisibleExpandControls = (limit = 30) => {
    const rows = [];
    for (const item of findCandidates()) {
      const r = visibleRect(item);
      if (!r) continue;
      if (!isReportableExpandLabel(item.text)) continue;
      rows.push({
        text: item.text.slice(0, 160),
        top: Math.round(r.top),
        left: Math.round(r.left),
        bottom: Math.round(r.bottom),
        x: Math.round(r.left + Math.min(Math.max(6, r.width / 2), Math.max(6, r.width - 6))),
        y: Math.round(r.top + Math.min(Math.max(6, r.height / 2), Math.max(6, r.height - 6)))
      });
    }
    rows.sort((a,b) => a.top - b.top || a.left - b.left || a.text.localeCompare(b.text));
    return {count: rows.length, labels: rows.slice(0, limit).map(r => r.text), items: rows.slice(0, limit)};
  };
  const clickCandidates = async () => {
    ...
    for (const item of allCandidates) {
      const r = visibleRect(item);
      if (!r) continue;
      visibleCandidates.push({item, r});
    }
    visibleCandidates.sort((a,b) => a.r.top - b.r.top || a.r.left - b.r.left || b.item.priority - a.item.priority);
    // R45AJ: filter to visible candidates BEFORE slicing. R45AI sliced first,
    // so offscreen controls could consume the batch and leave visible controls unclicked.
    const candidates = visibleCandidates.slice(0, opts.maxClicksPerRound);
    let clicked = 0;
    const labels = [];
    ...
    const remaining = summarizeVisibleExpandControls(20);
    return {
      candidate_count: allCandidates.length,
      visible_candidate_count: visibleCandidates.length,
      clicked,
      clicked_labels: labels.slice(0, 30),
      remaining_visible_expand_count: remaining.count,
      remaining_visible_expand_labels: remaining.labels,
      r45aj_visible_before_slice: true,
      r45aj_missed_report_enabled: true
    };
  };
  const stats = [];
  let totalClicks = 0;
  ...
  for (let round = 1; round <= opts.rounds; round++) {
    ...
    for (let s = 0; s < opts.scrollsPerRound; s++) {
      ...
      c.visible_candidate_count = (c.visible_candidate_count || 0) + (c2.visible_candidate_count || 0);
      c.remaining_visible_expand_count = c2.remaining_visible_expand_count || 0;
      c.remaining_visible_expand_labels = c2.remaining_visible_expand_labels || [];
    ...
    const row = {round, candidate_count: c.candidate_count, visible_candidate_count: c.visible_candidate_count || 0, clicked: c.clicked, remaining_visible_expand_count: c.remaining_visible_expand_count || 0, remaining_visible_expand_labels: (c.remaining_visible_expand_labels || []).slice(0, 12), delta_text_chars: delta, text_chars: currentTextLength, scroll_changed: scrollChanged, progress, progress_changed: progressChanged, elapsed_seconds: Math.round((Date.now()-startedAt)/1000), scroll_targets: lastTargets, clicked_labels: c.clicked_labels.slice(0, 25), r45aj_visible_miss_report: true};
    stats.push(row);
    log(row);
  ...
  return {rounds_completed: stats.length, total_clicks: totalClicks, total_scroll_events: totalScrollEvents, final_text_chars: previousTextLength, final_progress: previousProgress, final_visible_expand_report: summarizeVisibleExpandControls(80), timed_out: timedOut, elapsed_seconds: Math.round((Date.now()-startedAt)/1000), stats};
}
'''
...
def contract() -> Dict[str, Any]:
    ...
    base.update({
        ...
        'r45aj_visible_miss_report_rule': 'R45AJ keeps the working 2026-09-19 legacy bounded modal engine, filters visible candidates before batching, and prints visible missed expand controls in CMD.',
      'r45al_failfast_v2_rule': 'R45AL fixes the R45AK diagnostics regression by defining the visible-control preflight probe and fixing the missing norm() helper inside the JS auto-expander.',
