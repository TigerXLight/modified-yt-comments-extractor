...
RUNTIME_TITLE = 'Release Operator Signoff Receipt'
RUNTIME_STAGE = 'release_operator_signoff_receipt'
...
class ReleaseOperatorSignoffReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> ReleaseOperatorSignoffReceiptRecord:
    ...
    return ReleaseOperatorSignoffReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ReleaseOperatorSignoffReceiptRecord:
    return build_record(
        "release_operator_signoff_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "release_operator_signoff_receipt"},
    )


def summarise_record(record: ReleaseOperatorSignoffReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ReleaseOperatorSignoffReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ReleaseOperatorSignoffReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: ReleaseOperatorSignoffReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ReleaseOperatorSignoffReceiptRecord:
