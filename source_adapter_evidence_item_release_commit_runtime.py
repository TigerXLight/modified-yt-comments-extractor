...
RUNTIME_TITLE = 'Evidence Item Release Commit'
RUNTIME_STAGE = 'evidence_item_release_commit'
...
class EvidenceItemReleaseCommitRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> EvidenceItemReleaseCommitRecord:
    ...
    return EvidenceItemReleaseCommitRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> EvidenceItemReleaseCommitRecord:
    return build_record(
        "evidence_item_release_commit-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "evidence_item_release_commit"},
    )


def summarise_record(record: EvidenceItemReleaseCommitRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: EvidenceItemReleaseCommitRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> EvidenceItemReleaseCommitRecord:
    return build_record(
...
def write_record(path: str | Path, record: EvidenceItemReleaseCommitRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> EvidenceItemReleaseCommitRecord:
