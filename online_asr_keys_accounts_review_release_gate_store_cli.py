...
ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_GATE_STORE_CLI_SCHEMA_VERSION = (
...
class OnlineASRKeysAccountsReviewReleaseGateStoreCLIError(ValueError):
    pass
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            if (
                ...
                raise OnlineASRKeysAccountsReviewReleaseGateStoreCLIError(
                    f"Refusing {source}: secret-like field '{key}' must not be provided to release gate store CLI"
                )
...
def _load_safe_json_mapping(path: str | Path, *, source: str) -> dict[str, Any]:
    ...
    if not isinstance(data, Mapping):
        raise OnlineASRKeysAccountsReviewReleaseGateStoreCLIError(f"{source} must be a JSON object")
    return dict(data)


@dataclass(frozen=True)
class OnlineASRKeysAccountsReviewReleaseGateStoreCLIResult:
    source_artifact_count: int
    review_verdict: str
    release_gate_ready: bool
    issue_count: int
    release_gate_schema_version: str
    ...
    schema_version: str = ONLINE_ASR_KEYS_ACCOUNTS_REVIEW_RELEASE_GATE_STORE_CLI_SCHEMA_VERSION
    ...
    def to_summary_text(self) -> str:
        return "\n".join(
            (
                "Online ASR KEYS/ACCOUNTS review release gate store CLI",
                ...
                f"Stored release-gate files: {self.stored_file_count}",
...
def build_online_asr_keys_accounts_review_release_gate_store_cli_result(
    artifacts: Sequence[Mapping[str, Any]],
    output_directory: str | Path,
    *,
    allow_overwrite: bool = True,
) -> OnlineASRKeysAccountsReviewReleaseGateStoreCLIResult:
    """Build and persist a metadata-only Online ASR KEYS/ACCOUNTS release gate.
    ...
    report = build_online_asr_keys_accounts_review_release_gate_report(normalised)
    store_result = write_online_asr_keys_accounts_review_release_gate_report(
        report,
        output_directory,
        allow_overwrite=allow_overwrite,
    )
    files = tuple(store_result.files)
    return OnlineASRKeysAccountsReviewReleaseGateStoreCLIResult(
        ...
        release_gate_schema_version=report.schema_version,
...
def online_asr_keys_accounts_review_release_gate_store_cli_result_to_json(
    result: OnlineASRKeysAccountsReviewReleaseGateStoreCLIResult,
) -> str:
...
def run_online_asr_keys_accounts_review_release_gate_store_cli(
    ...
    *,
    ...
    try:
        ...
        result = build_online_asr_keys_accounts_review_release_gate_store_cli_result(
            artifacts,
            args.output_directory,
            allow_overwrite=not args.no_overwrite,
        )
    except (
        OnlineASRKeysAccountsReviewReleaseGateStoreCLIError,
        OnlineASRKeysAccountsReviewReleaseGateError,
    ) as exc:
        print(f"Online ASR KEYS/ACCOUNTS release gate store CLI error: {exc}", file=err)
        return 2
    except FileExistsError as exc:
        print(f"Online ASR KEYS/ACCOUNTS release gate store CLI error: {exc}", file=err)
        return 3
    except Exception as exc:  # pragma: no cover - CLI safety net for user-facing command failures.
        print(f"Online ASR KEYS/ACCOUNTS release gate store CLI unexpected error: {exc}", file=err)
        return 1
    ...
    if args.summary:
        ...
        print(
            online_asr_keys_accounts_review_release_gate_store_cli_result_to_json(result),
            end="",
            file=out,
        )
    return 0
...
def main() -> int:
    return run_online_asr_keys_accounts_review_release_gate_store_cli()
