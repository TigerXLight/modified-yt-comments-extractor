...
RUNTIME_TITLE = 'Total Export Release Bundle Index'
RUNTIME_STAGE = 'total_export_release_bundle_index'
...
class TotalExportReleaseBundleIndexRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> TotalExportReleaseBundleIndexRecord:
    ...
    return TotalExportReleaseBundleIndexRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> TotalExportReleaseBundleIndexRecord:
    return build_record(
        "total_export_release_bundle_index-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "total_export_release_bundle_index"},
    )


def summarise_record(record: TotalExportReleaseBundleIndexRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: TotalExportReleaseBundleIndexRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> TotalExportReleaseBundleIndexRecord:
    return build_record(
...
def write_record(path: str | Path, record: TotalExportReleaseBundleIndexRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> TotalExportReleaseBundleIndexRecord:
