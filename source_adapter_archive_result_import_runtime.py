...
SCHEMA_VERSION = "source_adapter_archive_result_import_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_archive_result_import_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_ARCHIVE_RESULT_IMPORT_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['archive_url', 'job_id', 'poll_status', 'result_receipt']


@dataclass(frozen=True)
class ArchiveResultImportRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class ArchiveResultImportRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = ArchiveResultImportRuntimeRow(
            row_id=_stable_id("source_adapter.archive_result_import_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="archive result import",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "archive result import",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.archive_result_import_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "archive result import",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.archive_result_import_runtime.json"
    rows_path = out / f"{package['package_id']}.archive_result_import_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.archive_result_import_runtime.verification.json"
