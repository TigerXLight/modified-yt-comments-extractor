...
from source_adapter_execution_policy_bundle_closeout_store import store_source_adapter_execution_policy_bundle_closeout_package
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Source Adapter Execution Policy Bundle Closeout")
    ...
    if args.store_dir:
        result["store"] = store_source_adapter_execution_policy_bundle_closeout_package(package, args.store_dir)
    if args.json:
        print(json.dumps(result, indent=2, sort_keys=True))
    else:
        print("Source Adapter Execution Policy Bundle Closeout: " + package["status"])
        print("handoff: " + package["handoff"]["handoff_status"])
        print("rows: " + str(package["bundle_closeout_rows_row_count"]))
