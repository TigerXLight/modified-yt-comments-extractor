...
SCHEMA_VERSION = "source_adapter_browser_session_profile_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_browser_session_profile_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_BROWSER_SESSION_PROFILE_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['desktop_firefox', 'mobile_firefox_rdm', 'android_user_agent', 'manual_observation_import']


@dataclass(frozen=True)
class BrowserSessionProfileRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class BrowserSessionProfileRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = BrowserSessionProfileRuntimeRow(
            row_id=_stable_id("source_adapter.browser_session_profile_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="browser session profile",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "browser session profile",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.browser_session_profile_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "browser session profile",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.browser_session_profile_runtime.json"
    rows_path = out / f"{package['package_id']}.browser_session_profile_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.browser_session_profile_runtime.verification.json"
