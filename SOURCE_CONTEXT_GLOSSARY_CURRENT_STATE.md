...
| ElevenLabs Scribe v2 SDK transport | `elevenlabs_scribe_transport.py`, `elevenlabs_scribe_transport_test.py`, `elevenlabs_scribe_provider.py`, `requirements.txt`, `pyproject.toml` | Production-capable official SDK transport for the existing adapter, fake-tested only and not live-verified. It declares `elevenlabs>=2.58.0,<3`, lazy-loads `elevenlabs.client` only during explicit provider execution, constructs a short-lived `ElevenLabs(api_key=..., timeout=240)` client with the resolved credential, maps local-file batch parameters to `client.speech_to_text.convert(...)`, passes through the already-open file object, disables SDK retries with `max_retries=0`, uses local `timeout_in_seconds=240`, converts supported synchronous single-channel SDK responses, and maps structured SDK errors safely. No package installation, live request, real key, media upload, GUI action, Test Connection wiring, startup/background call, account/quota/model call, or live network verification occurred. |
...
1. Ordered Access & Keys boundary:
   ...
   - the next separately approved provider boundary is live manual ElevenLabs verification with explicit user permission, real credentials kept out of prompts/logs/tests, preferably a disposable or revocable key, a tiny non-sensitive media sample, no automatic retries, explicit one-call confirmation, and safe capture of only non-secret result metadata. Later user-facing ASR action and Test Connection wiring remain separate boundaries. The current roadmap does not define a precise row label for that boundary. Production/user-facing connection testing must be explicit/user-triggered, and broader provider/API calls, cloud-ASR execution, OAuth/browser flows, uploads, network behavior, and any future reveal/copy/export behavior remain later boundaries.
