...
```text
2bb9340 Add local source resource and archive UI
```

## Purpose

This document is the local current-state handoff for the source URL, source adapter, local source-resource UI scaffold, source capture plan, provenance, context/glossary, evidence queue, Access & Keys metadata/catalog/window, row 2A credential architecture/audit, row 2B read-only credential status integration, row 2C1 secure credential-store infrastructure, row 2C2 masked cloud-ASR Save/Clear controls, secure YouTube credential migration/legacy cleanup, the local-only cloud-ASR credential-consumption prerequisite, the explicit injected-executor ASR provider-action seam, the explicit injected-tester ASR connection-test seam, the ElevenLabs Scribe v2 fake-transport-tested provider adapter, the production-capable official SDK transport, and evidence database taxonomy skeletons.
...
The current source/context/glossary helpers are local planning and metadata helpers only. The local source-resource UI scaffold is presentation/controller state only and does not perform operational capture.
...
- add secret-bearing GUI/runtime wiring beyond the bounded Access & Keys presentation, read-only status overlay, row 2C1 store abstraction, row 2C2 masked cloud-ASR Save/Clear controls, committed secure YouTube credential migration/legacy cleanup, local-only trusted-callback cloud-ASR credential-consumption prerequisite, explicit injected-executor ASR provider-action seam, explicit injected-tester ASR connection-test seam, ElevenLabs Scribe v2 fake-transport-tested provider adapter, and production-capable official SDK transport,
...
| Source adapter metadata | `source_adapters.py`, `source_adapters_test.py`, `source_adapters_registry_test.py` | Local adapter capability metadata, YouTube URL support, metadata-only News Website known-host URL support, local-only MSN adapter scaffold metadata, `source_name` registry helpers, and name lookup. |
| Source-resource UI scaffold | `source_resource_state.py`, `source_resource_state_test.py`, `main_source_resource_ui_test.py`, narrow `main.py` wiring | Local-only source row and source-resource state for MSN fixture-backed presentation: Enter-driven multi-URL intake, Shift+Enter newline, canonical visible-source dedupe, removable session rows, clean title/hostname display, fixture image/video-audio resource windows, dry-run-only Download, fixture/mock Wayback and archive.ph status presentation, ArchiveBox local-software scaffold/icon, source archive auto-check preference state, selected-source discussion dropdown, Webpage/Comments/Livechat controls with independent Screenshot intent state, Transcript `Get` label preserving the previous callback, sidebar order `UPDATES`, `KEYS/ACCOUNTS`, `EXPORT`, `FILES`, and main-page wheel routing. It performs no live MSN fetching, resource discovery from pages, downloads, screenshots, browser automation, archive checks/submissions, ArchiveBox execution, credentials, provider calls, or network/API behavior. |
...
| ElevenLabs Scribe v2 provider adapter | `elevenlabs_scribe_provider.py`, `elevenlabs_scribe_provider_test.py` | Provider-specific fake-transport-tested executor for `elevenlabs_scribe`, credential ID `elevenlabs_scribe_api_key`, and model `scribe_v2`. It supports local-file synchronous batch request validation, conservative keyterm handling, response normalization, file-handle closure across success/failure/control paths, and action-time credential flow through the existing explicit action/credential-consumption seams. It is not a production-live ElevenLabs integration and adds no production HTTP transport, SDK/client, live API request, real credential use, media upload, GUI transcription action, Test Connection wiring, or network behavior. |
| ElevenLabs Scribe v2 SDK transport and Online ASR action | `elevenlabs_scribe_transport.py`, `elevenlabs_scribe_transport_test.py`, `elevenlabs_scribe_provider.py`, `main.py`, `online_asr_ui_test.py`, `requirements.txt`, `pyproject.toml` | Production-capable official SDK transport for the existing adapter plus explicit user-facing Online ASR action wiring. The SDK transport declares `elevenlabs>=2.58.0,<3`, lazy-loads `elevenlabs.client` only during explicit provider execution, constructs a short-lived `ElevenLabs(api_key=..., timeout=240)` client with the resolved credential, maps local-file batch parameters to `client.speech_to_text.convert(...)`, passes through the already-open file object, disables SDK retries with `max_retries=0`, uses local `timeout_in_seconds=240`, converts supported synchronous single-channel SDK responses, and maps structured SDK errors safely. One approved live request succeeded for the narrow local-file path, but exact-phrase accuracy was not confirmed by the synthetic sample. The `Online ASR` UI opens a local-file workflow without dispatching and calls the provider only after explicit `Transcribe` through the committed coordinator/credential/provider/transport path. No Test Connection UI, provider catalog redesign, automatic/background call, account/quota/model call, or package installation occurred. |
...
- `f21e578`: `elevenlabs_scribe_provider_test.py` plus provider-action, connection-test, credential consumption/store/runtime/architecture, Access & Keys metadata/catalog/view/dialog, ASR-provider metadata, YouTube credential migration/settings fallback, source adapter registry/report/gap, `youtube_url_utils_test.py`, `main_export_state_test.py`, compile, whitespace/final-newline, and `git diff --check` verification; confirmed the ElevenLabs Scribe v2 fake-transport-tested provider adapter for local-file synchronous batch requests, conservative request/keyterm validation, action-time credential flow, response normalization, structured fake-error handling, file-handle closure across success/failure/control paths, non-secret public results, no production HTTP transport, no SDK/client, no live API request, no real credential use, no media upload, no GUI transcription/Test Connection wiring, no network behavior, and unchanged YouTube/cloud-ASR Save/Clear/status behavior. `asr_tools_test.py` remains compile-checked only and is not executed as an automated test because it is an interactive/manual harness.
- `38aee73`: `elevenlabs_scribe_transport_test.py` plus `elevenlabs_scribe_provider_test.py`, provider-action, connection-test, credential consumption/store/runtime/architecture, Access & Keys metadata/catalog/view/dialog, ASR-provider metadata, YouTube credential migration/settings fallback, source adapter registry/report/gap tests and CLIs, `youtube_url_utils_test.py`, `main_export_state_test.py`, compile, dependency declaration/TOML validation, whitespace/final-newline, and `git diff --check` verification; confirmed production-capable official SDK transport with lazy import, explicit short-lived client construction, `speech_to_text.convert(...)` mapping, `max_retries=0` one-attempt policy verified against SDK source and fakes, local 240-second timeout validation, missing top-level package versus transitive/import-failure distinction, supported response conversion, structured safe error mapping, provider file-handle ownership, no live API request, no real credential use, no media upload, no GUI transcription/Test Connection wiring, no startup/background network behavior, and unchanged YouTube/cloud-ASR Save/Clear/status behavior. `asr_tools_test.py` remains compile-checked only and is not executed as an automated test because it is an interactive/manual harness.
- `2bb9340`: `source_resource_state_test.py`, `main_source_resource_ui_test.py`, `source_adapters_test.py`, source capture contract/plan/provenance tests, export/session/Access & Keys/ASR UI regressions, compile, and `git diff --check` verification; confirmed the local MSN/source-resource/archive/discussion UI scaffold, fixture resource windows, dry-run Download, fixture/mock archive status presentation, ArchiveBox scaffold/icon, selected-source Webpage/Comments/Livechat screenshot intent state, Transcript `Get` label, sidebar order, and main-page wheel routing without live MSN fetching, resource discovery from pages, downloads, screenshots, browser automation, archive checks/submissions, ArchiveBox execution, credentials, provider calls, or network/API behavior.

The queue/taxonomy schemas and Access & Keys metadata/view-model remain data/reporting foundations. Commits `1b57e74`, `0ff528d`, and `ee945fe` add the bounded `KEYS` button/window, complete planned catalog, corrected local interactions, and short-family visibility behavior described above. Commit `ef92017` adds row 2A non-secret credential architecture/audit, `7c1db2a` adds the row 2B read-only safe status/provenance overlay, `29af218` adds row 2C1 secure-store infrastructure, `97de48d` adds row 2C2 masked cloud-ASR Save/Clear controls with safe presence/provenance refresh, `3abb49d` adds secure YouTube credential migration/legacy cleanup, `a1cf07d` adds the local-only trusted-callback cloud-ASR credential-consumption prerequisite, `058af01` adds the explicit injected-executor ASR provider-action seam, `f709f8e` adds the explicit injected-tester ASR connection-test seam, `f21e578` adds the ElevenLabs Scribe v2 fake-transport-tested provider adapter, `38aee73` adds the production-capable official SDK transport, one approved live request verified the narrow local-file transport path, and the Online ASR action wiring milestone adds the explicit user-facing local-file transcription workflow. No source fetching, production/user-facing provider connection testing, database scanning, file movement, Test Connection wiring, startup/background network behavior, provider catalog redesign, or unrelated runtime behavior was added.
...
Only local planning, metadata, bounded Access & Keys presentation, row 2A credential architecture/audit, row 2B read-only safe status/provenance, row 2C1 secure-store infrastructure, row 2C2 masked cloud-ASR Save/Clear controls, secure YouTube credential migration/legacy cleanup, the local-only cloud-ASR credential-consumption prerequisite, the explicit injected-executor ASR provider-action seam, the explicit injected-tester ASR connection-test seam, the ElevenLabs Scribe v2 fake-transport-tested provider adapter, and the production-capable official SDK transport exist here. The evidence queue can describe links among source URLs, local media, reference text, transcript/subtitle candidates, ASR results, archive/snapshot records, packages, and taxonomy suggestions, but it is not persisted or wired into this pipeline. The credential runtime layer reports only boolean presence, safe provenance, and fixed diagnostic categories; it does not render or retain values. Cloud-ASR secure-store usage includes explicit Save/Clear, safe presence probing, explicit trusted-callback resolution, explicit dispatch to injected trusted executors, explicit dispatch to injected trusted connection testers, fake-transport-tested Scribe v2 request execution, and production-capable SDK transport code only during explicit provider execution; no live provider request, real credential use, media upload, GUI transcription action, startup/background call, account/quota/model call, or production/user-facing connection test exists. YouTube storage is secure-only for new/updated saves, explicit for legacy migration, and never preloads stored credentials into the UI field. Fetching, capture, live ASR provider execution, provider keyterm use in a live request, reveal/copy UI, production/user-facing credential connection tests, database scanning, file movement, and new persistence workflows are not implemented by this layer.
...
- The current ElevenLabs Scribe v2 adapter uses a conservative local keyterm policy for fake-transport-tested request validation only: at most 1000 terms, at most five words by local whitespace normalization, first-occurrence dedupe, delimiter rejection, whitespace-only rejection, and a 49-character term limit. The 49-character value reflects an official wording conflict where the endpoint reference says less than 50 characters while the guide says 50 characters; it is not claimed as an unambiguous provider limit, and local normalization is not claimed to exactly match provider-side normalization.
...
1. Ordered Access & Keys boundary:
   ...
   - the ElevenLabs Scribe v2 fake-transport-tested provider adapter is implemented at `f21e578`,
   - the production-capable official ElevenLabs SDK transport is implemented at `38aee73`,
   - one approved live request verified the narrow local-file transport path, but exact-phrase accuracy was not confirmed by the synthetic sample,
   - explicit user-facing Online ASR action wiring is implemented as a local-file workflow that dispatches only after `Transcribe`,
   - the next separately approved provider boundary is user-facing Test Connection wiring. Production/user-facing connection testing must be explicit/user-triggered, and broader provider/API calls, cloud-ASR execution outside explicit selected-file transcription, OAuth/browser flows, uploads beyond selected-file transcription, background/network checks, and any future reveal/copy/export behavior remain later boundaries.
2. Local-only compatibility/reporting:
   - defer explicit transforms or reports across queue, adapter, access-status, taxonomy, provenance, and Total Export metadata until the next ordered provider boundary is separately approved or intentionally deferred,
