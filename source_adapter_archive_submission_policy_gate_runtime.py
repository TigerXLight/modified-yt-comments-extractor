...
SCHEMA_VERSION = "source_adapter_archive_submission_policy_gate_runtime_v1"
STATUS = "SOURCE_ADAPTER_ARCHIVE_SUBMISSION_POLICY_GATE_RUNTIME_BUILT"
HANDOFF_STATUS = "SOURCE_ADAPTER_ARCHIVE_SUBMISSION_POLICY_GATE_RUNTIME_READY_FOR_NEXT_IMPLEMENTATION_BUNDLE"
ROW_STATUS = "SOURCE_ADAPTER_ARCHIVE_SUBMISSION_POLICY_GATE_RUNTIME_ROWS_READY"
ROW_KEY = "archive_submission_policy_gate_rows"
DESCRIPTION = "archive submission policy gate rows for provider eligibility, fallback order, no-auto-live execution gates, and operator approval receipts"


def build_archive_submission_policy_gate_rows(operator_id: str = "operator") -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for site in named_sites():
        for action_index, action in enumerate(PROVIDER_ACTIONS):
            seed = {"aspect": "archive_submission_policy_gate", "site": site, "action": action, "operator_id": operator_id}
            rows.append({
                "schema_version": "source_adapter_archive_submission_policy_gate_runtime_row_v1",
                "row_id": stable_id("source_adapter.archive_submission_policy_gate.row", seed),
                "row_index": len(rows),
...
def example_source_adapter_archive_submission_policy_gate_runtime_package(operator_id: str = "operator") -> dict[str, Any]:
    rows = build_archive_submission_policy_gate_rows(operator_id=operator_id)
    package = {
        "schema_version": SCHEMA_VERSION,
        "id": stable_id("source_adapter.archive_submission_policy_gate.package", {"rows": rows, "operator_id": operator_id}),
        "status": STATUS,
        "description": DESCRIPTION,
        "row_key": ROW_KEY,
        "archive_submission_policy_gate_rows_count": len(rows),
        ROW_KEY: rows,
        "operator_summary": {
            "schema_version": "source_adapter_archive_submission_policy_gate_runtime_operator_summary_v1",
            "status": STATUS,
        ...
        "handoff": {
            "schema_version": "source_adapter_archive_submission_policy_gate_runtime_handoff_v1",
...
def write_source_adapter_archive_submission_policy_gate_runtime_receipts(output_dir: str | Path | None = None, operator_id: str = "operator") -> dict[str, Any]:
    package = example_source_adapter_archive_submission_policy_gate_runtime_package(operator_id=operator_id)
    output_root = Path(output_dir) if output_dir is not None else Path(tempfile.mkdtemp(prefix="source_adapter_archive_submission_policy_gate_runtime_receipts_"))
    stored_files = []
    ...
    stored_files.append(write_json(output_root / f"{package['id']}.archive_submission_policy_gate_rows.json", package[ROW_KEY]))
    for role, meta in zip(("package", "handoff", ROW_KEY), stored_files):
        meta["role"] = role
    return {
        "schema_version": "source_adapter_archive_submission_policy_gate_runtime_receipt_writer_v1",
        "id": package["id"],
...
if __name__ == "__main__":
    print(json.dumps(example_source_adapter_archive_submission_policy_gate_runtime_package(), indent=2, sort_keys=True))
