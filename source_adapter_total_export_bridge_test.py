...
from source_adapter_total_export_bridge import TOTAL_EXPORT_BRIDGE_STATUS, build_source_adapter_total_export_bridge
from source_adapter_total_export_bridge_verifier import verify_source_adapter_total_export_bridge


def main() -> None:
    package = build_source_adapter_total_export_bridge(fixture_capture_bundle_bridge(), package_notes=["reviewed"])
    assert package["total_export_bridge_status"] == TOTAL_EXPORT_BRIDGE_STATUS
    assert package["total_export_package_count"] == 1
    assert package["source_adapter_total_export_package_index"]["total_export_package_count"] == 1
    assert package["source_adapter_evidence_queue_handoff"]["ready_for_evidence_queue"] is True
    assert package["total_export_outputs"][0]["total_export_package"]["content"]["title"] == "Bridge title"
    assert package["total_export_outputs"][0]["total_export_manifest"]["artifact_count"] == 2
    assert package["implementation_logic"]["shared_stage_executed"] == "source_total_export_package"
    assert verify_source_adapter_total_export_bridge(package)["verified"] is True


if __name__ == "__main__":
    main()
    print("Source Adapter Total Export Bridge self-test passed.")
