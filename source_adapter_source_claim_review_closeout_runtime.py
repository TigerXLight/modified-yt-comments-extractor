...
RUNTIME_TITLE = 'Source Claim Review Closeout'
RUNTIME_STAGE = 'source_claim_review_closeout'
...
class SourceClaimReviewCloseoutRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> SourceClaimReviewCloseoutRecord:
    ...
    return SourceClaimReviewCloseoutRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> SourceClaimReviewCloseoutRecord:
    return build_record(
        "source_claim_review_closeout-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "source_claim_review_closeout"},
    )


def summarise_record(record: SourceClaimReviewCloseoutRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: SourceClaimReviewCloseoutRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> SourceClaimReviewCloseoutRecord:
    return build_record(
...
def write_record(path: str | Path, record: SourceClaimReviewCloseoutRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> SourceClaimReviewCloseoutRecord:
