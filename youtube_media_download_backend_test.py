...
def test_build_youtube_ytdlp_download_plan_can_queue_separate_audio() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        plan = build_youtube_ytdlp_download_plan(
            "https://www.youtube.com/watch?v=abc123",
            output_dir=tmp,
        ...
        assert len(plan.extra_commands) == 1
        audio_command = plan.extra_commands[0]
        assert "-x" in audio_command
        assert "--audio-format" in audio_command
        assert "m4a" in audio_command
        assert "bestaudio/best" in audio_command
        assert "--simulate" in audio_command


def test_run_youtube_ytdlp_download_plan_runs_extra_audio_command() -> None:
    seen = []
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        completed = run_youtube_ytdlp_download_plan(plan, runner=runner)
        assert completed.returncode == 0
        assert len(seen) == 2
        assert seen[0] == plan.command
        assert seen[1] == plan.extra_commands[0]
        assert "extra-1" in (completed.stdout or "")
...
if __name__ == "__main__":
    ...
    test_build_youtube_ytdlp_download_plan_can_queue_separate_audio()
    test_run_youtube_ytdlp_download_plan_runs_extra_audio_command()
