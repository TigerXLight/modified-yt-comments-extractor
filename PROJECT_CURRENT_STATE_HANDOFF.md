...
- `evidence_item_queue.py` now bridges the source-role / claim-level roadmap into queue/export review metadata with `SourceRoleReviewMetadata`, `queue_source_role_reviews_to_claim_notes(...)`, `queue_source_role_reviews_to_ui_rows(...)`, and `build_source_role_review_ui_summary(...)`. Queue items can carry review-required scoped source-role claims, primary-source/currentness status, temporal/source-chain limitations, evidence basis, and safeguards that keep automatic classification false and sensitive inference prohibited; the export helper maps them into existing `ClaimEvidenceNote` records for Total Export manifests in deterministic queue order, and the UI helper emits safe metadata-only rows/status summaries without raw claim/evidence payloads or full local paths. This remains metadata-only and local-tested. It does not add visible editor UI, execute classification, infer sensitive attributes, scan folders, move files, call providers/network, or claim completed evidence.
