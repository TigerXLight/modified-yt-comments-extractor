...
2. The exact next ordered REV4 boundary is WARC/WACZ modeling, followed by later GUI/controller integration after backend fixture coverage exists. Live/manual site-smoke approval and any real-site operational adapter step must remain separate, and must not silently expand from the local/mock scaffolds into real HTTP/API retrieval, browser/DevTools/shadow-root execution against live sites, scraping, live comments, live resource enumeration, downloads, real screen recording, webpage/comment screenshots, archive checks/submissions, ArchiveBox execution, credentials, provider/network behavior, protected-output bypass, or evidence-database implementation.
