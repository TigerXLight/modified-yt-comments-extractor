...
Checkpoint: `38aee73 Add ElevenLabs Scribe v2 SDK transport`
...
- Current checkpoint: `38aee73 Add ElevenLabs Scribe v2 SDK transport`.
...
| ElevenLabs Scribe v2 provider adapter and SDK transport | `elevenlabs_scribe_provider.py`, `elevenlabs_scribe_transport.py` | `elevenlabs_scribe_provider_test.py`, `elevenlabs_scribe_transport_test.py`; provider-specific fake-transport-tested executor plus production-capable official SDK transport for local-file synchronous batch requests, conservative keyterm validation, lazy SDK import, explicit short-lived client construction, `speech_to_text.convert(...)` mapping, retry disabling with `max_retries=0`, local 240-second timeout policy, response conversion, structured safe error mapping, file-handle ownership through provider code, action-time credential flow through existing seams, no live API request, no real credential use, no real media upload, no GUI/Test Connection wiring, and no startup/background network behavior |
