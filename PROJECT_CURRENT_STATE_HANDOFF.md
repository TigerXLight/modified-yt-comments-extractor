...
- Operational site-capture REV4 local implementation through `48daf04`, plus the later capture-plan action-log artifact hardening: `capture_status.py`, `capture_contracts.py`, `capture_action_log.py`, `capture_dependency_audit.py`, `capture_fixture_server.py`, `capture_browser.py`, `capture_article.py`, `capture_page_outline.py`, `capture_snapshots.py`, `capture_comments.py`, `capture_livechat.py`, `capture_media_discovery.py`, `capture_media_download.py`, `capture_archive_providers.py`, `capture_archive_wayback.py`, `capture_archive_today.py`, `capture_archivebox.py`, and `capture_controller.py` provide deterministic capture status/contract/action-log metadata, a standard-library localhost fixture server, optional lazy browser-runner wrappers with fake-runner coverage, supplied-HTML article/page/snapshot/comment/livechat/media helpers, localhost-only explicit media-download tests, mock-only archive provider checks, ArchiveBox command planning only, and source UI operational-capture plan wiring. The source UI can produce a local scaffold plan for selected source/mode/screenshot preferences; controller results now include deterministic in-memory `ACTION_LOG` artifact metadata, chained action events, and a sanitized JSONL hash for review. This does not execute live capture or write the action log file. No real external website access, real browser profile/cookie access, scraping, screenshots, external media downloads, live archive checks/submissions, ArchiveBox process/server execution, credentials, provider calls, or evidence-database work was added.
