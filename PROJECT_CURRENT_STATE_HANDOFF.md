...
- `evidence_item_queue.py` now bridges the source-role / claim-level roadmap into queue/export review metadata with `SourceRoleReviewMetadata` and `queue_source_role_reviews_to_claim_notes(...)`. Queue items can carry review-required scoped source-role claims, primary-source/currentness status, temporal/source-chain limitations, evidence basis, and safeguards that keep automatic classification false and sensitive inference prohibited; the helper maps them into existing `ClaimEvidenceNote` records for Total Export manifests in deterministic queue order. This remains metadata-only and local-tested. It does not add editor UI, execute classification, infer sensitive attributes, scan folders, move files, call providers/network, or claim completed evidence.
