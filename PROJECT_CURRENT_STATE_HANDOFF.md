...
- Static result matrix: source identification STATIC_HTTP_LIVE_TESTED; Local Web Archive LOCAL_PACKAGE_STRUCTURALLY_VERIFIED; article text, visible outline, resource inventory, and Wayback check PARTIAL; comments shadow DOM, faithful screenshot, and derived screenshot BLOCKED; representative download, Wayback submit, ArchiveBox, and Evidence Database N_A.
...
- Follow-up rendered-browser validation upgrades the status language and capability coverage. `source_msn_rendered_browser_validation.py` uses a project-local Playwright Chromium runtime plus `warcio` to capture the same approved MSN URL only, with disposable desktop and Android/mobile contexts. The latest run at `C:\Users\fahad\AppData\Local\Temp\ytce_msn_rendered_browser_validation_dk1_t38t` produced manifest SHA-256 `abb0a2d6b6d391aa06daa7018d6a9f05c5f217f7d786c3c2cb97bb60f09eba0f`, WARC SHA-256 `80fd93dc2e9c451ef80abeafa1e4a4a5e04e1ba2f53a7fc8444a8d3d0ce6a389`, and WACZ SHA-256 `883076d26a77108f86c4c6732b94997e4399b0bca7f40664c585c9634fb6f886`.
- Rendered result matrix: browser runtime, desktop render, mobile render, source-canonicalization, article-only text for the approved story, page outline, final DOM, faithful article screenshot, image discovery, representative download, comments component detection, comments/replies capture, comments screenshot, and comment completeness are `RENDERED_BROWSER_LIVE_TESTED`; WARC/WACZ/bundle/action-log are `LOCAL_PACKAGE_STRUCTURALLY_VERIFIED`. The Android/mobile comments collector now recursively walks `social-comment-wc` and nested open shadow roots, drives internal scrollports, expands `show-more-replies` controls, serializes rows before/after interaction passes, and reconciles same-session comments API `items` payloads. The latest run captured `16` top-level comments plus `9` replies, matching the provider-declared `25` with gap `0`; stop reason is `stable_and_provider_declared_count_reconciled`, stable pass target/count was `12/12`, and true virtualization was detected without losing incrementally serialized records. ReplayWeb.page remains `VIEWER_NOT_CONFIGURED`; no configured local executable was found and no installer was run. Wayback CHECK was rate-limited; Wayback submit is explicitly NOT RUN.
- ReplayWeb.page / Local Web Archive closeout now includes optional viewer detection, official release metadata parsing, safe argument-array Open archive previews, Show files previews, browser/PWA file-chooser limitation reporting, WACZ/manifest verification, and non-destructive offline replay-compatibility repair. Investigation after ReplayWeb.page showed `Archived Page Not Found` proved a real WACZ packaging/index defect without invalidating the accepted comments evidence: comments remain `25/25` with `16` roots and `9` replies, while the original package used pre-1.2 profile `data-package`, retained removed 1.2 metadata fields, and had `87` CDXJ rows that were unsorted and not consistently canonical lower-case lookup keys. Commit `c3eb5e9` fixed package readiness, request-line normalization, hosted ReplayWeb.page profile handling, and sorted/canonical CDXJ readiness without claiming visual success. Commit `eeba5bb` added derived static WACZ evidence-view infrastructure; manual testing confirmed direct static evidence HTML and raw static evidence WARC.GZ work, but the derived static WACZ page was listed and still returned `Archived Page Not Found` when clicked. The usable evidence-report fallback is therefore direct no-JavaScript static HTML plus raw static-only WARC/WARC.GZ outputs, reported as `STATIC_EVIDENCE_HTML_READY` / `STATIC_EVIDENCE_WARC_READY`. The first static archived page-view output worked technically but looked too report-like; the current static page-view fallback now renders a dark MSN/mobile-style page reconstruction from local manifest article/comment/screenshot artifacts, copies selected local images into relative `assets/` paths for direct HTML, and reports `STATIC_PAGE_VIEW_VISUAL_FIDELITY_PARTIAL` rather than dynamic replay success. A separate structured `static-text-view.html` plus `static-article.txt`, `static-comments.txt`, `static-page-text.txt`, and `static-page-text.md` support collapsible reading/copying and editor review. The original capture, original dynamic page entry, WACZ lookup failure, and MSN runtime limitations remain preserved; the fallback is not a claim that ReplayWeb visually verified or replayed the original MSN runtime/comments.
...
## Final App/Operator Wiring Checkpoint - 2026-08-08

- Latest app/operator wiring commits: `131e9fb Add source app operator controller surfaces`; `1ec8472 Wire source operator controller into app workflow`.
- `source_app_operator_controller.py` is the current app-facing controller facade for the previously implemented execution bridges. It exposes summary/counts-only Source URL/FILES state, approval-gateway rows, unified execution job state, local E2E Total Export fixture package state, evidence database movement preview/temp execution state, manual live-smoke dry-run helper state, archive/media/ArchiveBox launch previews, receipt import/review readiness, and Access/Online ASR preservation state.
