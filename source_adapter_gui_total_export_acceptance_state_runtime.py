...
RUNTIME_TITLE = 'GUI Total Export Acceptance State'
RUNTIME_STAGE = 'gui_total_export_acceptance_state'
...
class GuiTotalExportAcceptanceStateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> GuiTotalExportAcceptanceStateRecord:
    ...
    return GuiTotalExportAcceptanceStateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> GuiTotalExportAcceptanceStateRecord:
    return build_record(
        "gui_total_export_acceptance_state-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "gui_total_export_acceptance_state"},
    )


def summarise_record(record: GuiTotalExportAcceptanceStateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: GuiTotalExportAcceptanceStateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> GuiTotalExportAcceptanceStateRecord:
    return build_record(
...
def write_record(path: str | Path, record: GuiTotalExportAcceptanceStateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> GuiTotalExportAcceptanceStateRecord:
