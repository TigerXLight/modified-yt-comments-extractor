...
RUNTIME_TITLE = 'Release Upload Finalizer'
RUNTIME_STAGE = 'release_upload_finalizer'
...
class ReleaseUploadFinalizerRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    return ReleaseUploadFinalizerRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> ReleaseUploadFinalizerRecord:
    return build_record(
        "release_upload_finalizer-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "release_upload_finalizer"},
    )


def summarise_record(record: ReleaseUploadFinalizerRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: ReleaseUploadFinalizerRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> ReleaseUploadFinalizerRecord:
    return build_record(
...
def write_record(path: str | Path, record: ReleaseUploadFinalizerRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> ReleaseUploadFinalizerRecord:
