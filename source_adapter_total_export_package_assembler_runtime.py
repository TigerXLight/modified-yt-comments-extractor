...
SCHEMA_VERSION = "source_adapter_total_export_package_assembler_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_total_export_package_assembler_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_TOTAL_EXPORT_PACKAGE_ASSEMBLER_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['manifest', 'bundle_index', 'release_package', 'hash_chain']


@dataclass(frozen=True)
class TotalExportPackageAssemblerRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class TotalExportPackageAssemblerRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = TotalExportPackageAssemblerRuntimeRow(
            row_id=_stable_id("source_adapter.total_export_package_assembler_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="Total Export package assembler",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "Total Export package assembler",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.total_export_package_assembler_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "Total Export package assembler",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.total_export_package_assembler_runtime.json"
    rows_path = out / f"{package['package_id']}.total_export_package_assembler_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.total_export_package_assembler_runtime.verification.json"
