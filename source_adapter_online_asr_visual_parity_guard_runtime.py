...
RUNTIME_TITLE = 'Online ASR Visual Parity Guard'
RUNTIME_STAGE = 'online_asr_visual_parity_guard'
...
class OnlineAsrVisualParityGuardRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> OnlineAsrVisualParityGuardRecord:
    ...
    return OnlineAsrVisualParityGuardRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> OnlineAsrVisualParityGuardRecord:
    return build_record(
        "online_asr_visual_parity_guard-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "online_asr_visual_parity_guard"},
    )


def summarise_record(record: OnlineAsrVisualParityGuardRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: OnlineAsrVisualParityGuardRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> OnlineAsrVisualParityGuardRecord:
    return build_record(
...
def write_record(path: str | Path, record: OnlineAsrVisualParityGuardRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> OnlineAsrVisualParityGuardRecord:
