import io
import json
from contextlib import redirect_stderr, redirect_stdout
from pathlib import Path
from tempfile import TemporaryDirectory

from source_adapter_gap_analysis_cli import main


def _run_cli(argv: list[str]):
    stdout = io.StringIO()
    stderr = io.StringIO()
    with redirect_stdout(stdout), redirect_stderr(stderr):
        code = main(argv)
    return code, stdout.getvalue(), stderr.getvalue()


def run_self_test() -> None:
    code, markdown, stderr = _run_cli([])
    assert code == 0
    assert stderr == ""
    assert "# Source Adapter / Preservation Gap Analysis" in markdown
    assert "youtube, msn, news_website" in markdown
    assert "Substack" in markdown
    assert "ArchiveBox" in markdown
    assert "does not fetch URLs" in markdown

    code, text, stderr = _run_cli(["--format", "text"])
    assert code == 0
    assert stderr == ""
    assert "Source adapter / preservation gap analysis" in text
    assert "social_video" in text
    assert "newsletter_sites" in text
    assert "self_hosted_preservation" in text
    assert "no fetch/capture/network" in text

    code, json_output, stderr = _run_cli(["--format", "json"])
    assert code == 0
    assert stderr == ""
    parsed = json.loads(json_output)
    assert parsed["implemented_adapter_count"] == 3
    assert parsed["current_adapter_ids"] == ["youtube", "msn", "news_website"]
    categories = {category["category_id"]: category for category in parsed["categories"]}
    assert "TikTok" in categories["social_video"]["example_platforms"]
    assert "Substack" in categories["newsletter_sites"]["example_platforms"]
    assert "ArchiveBox" in categories["self_hosted_preservation"]["example_platforms"][0]

    with TemporaryDirectory() as temp_dir:
        output_path = Path(temp_dir) / "SOURCE_ADAPTER_GAP_ANALYSIS.md"
        output_args = ["--output", str(output_path)]
        code, stdout, stderr = _run_cli(output_args)
        assert code == 0
        assert stdout == ""
        assert stderr == ""
        assert output_path.read_text(encoding="utf-8").startswith(
            "# Source Adapter / Preservation Gap Analysis"
        )

        code, stdout, stderr = _run_cli(output_args)
        assert code == 1
        assert stdout == ""
        assert "output path already exists" in stderr

        output_path.write_text("old\n", encoding="utf-8")
        code, stdout, stderr = _run_cli([*output_args, "--overwrite"])
        assert code == 0
        assert stdout == ""
        assert stderr == ""
        assert "Current adapters: youtube, msn, news_website" in output_path.read_text(
            encoding="utf-8"
        )


if __name__ == "__main__":
    run_self_test()
    print("Source adapter gap analysis CLI self-test passed.")
