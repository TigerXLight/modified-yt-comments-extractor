...
SCHEMA_VERSION = "source_adapter_live_smoke_readiness_runtime_v1"
ROW_SCHEMA_VERSION = "source_adapter_live_smoke_readiness_runtime_v1_row_v1"
STATUS = "SOURCE_ADAPTER_LIVE_SMOKE_READINESS_RUNTIME_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['named_site_profile', 'approval_packet', 'provider_health', 'rollback_plan']


@dataclass(frozen=True)
class LiveSmokeReadinessRuntimeRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class LiveSmokeReadinessRuntimePackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = LiveSmokeReadinessRuntimeRow(
            row_id=_stable_id("source_adapter.live_smoke_readiness_runtime.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="live smoke readiness",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "live smoke readiness",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.live_smoke_readiness_runtime.package", package_seed),
        "status": STATUS,
        "purpose": "live smoke readiness",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.live_smoke_readiness_runtime.json"
    rows_path = out / f"{package['package_id']}.live_smoke_readiness_runtime.rows.json"
    verification_path = out / f"{package['package_id']}.live_smoke_readiness_runtime.verification.json"
