...
RUNTIME_TITLE = 'Operator Receipt Acceptance Gate'
RUNTIME_STAGE = 'operator_receipt_acceptance_gate'
...
class OperatorReceiptAcceptanceGateRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> OperatorReceiptAcceptanceGateRecord:
    ...
    return OperatorReceiptAcceptanceGateRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> OperatorReceiptAcceptanceGateRecord:
    return build_record(
        "operator_receipt_acceptance_gate-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "operator_receipt_acceptance_gate"},
    )


def summarise_record(record: OperatorReceiptAcceptanceGateRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: OperatorReceiptAcceptanceGateRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> OperatorReceiptAcceptanceGateRecord:
    return build_record(
...
def write_record(path: str | Path, record: OperatorReceiptAcceptanceGateRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> OperatorReceiptAcceptanceGateRecord:
