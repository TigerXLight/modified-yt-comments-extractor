...
SCHEMA_VERSION = "source_adapter_operational_runtime_bundle_closeout_v1"
ROW_SCHEMA_VERSION = "source_adapter_operational_runtime_bundle_closeout_v1_row_v1"
STATUS = "SOURCE_ADAPTER_OPERATIONAL_RUNTIME_BUNDLE_CLOSEOUT_BUILT"
KEYS_ACCOUNTS_LABEL = "KEYS/ACCOUNTS"
CAPABILITY_IDS = ['all_runtime_surfaces', 'policy_closed', 'release_ready', 'operator_dashboard_ready']


@dataclass(frozen=True)
class OperationalRuntimeBundleCloseoutRow:
    row_id: str
    row_index: int
    capability_id: str
    runtime_status: str
...
@dataclass(frozen=True)
class OperationalRuntimeBundleCloseoutPackage:
    schema_version: str
    package_id: str
    status: str
    purpose: str
...
def build_rows(*, execution_mode: str = "operator_controlled") -> list[dict[str, Any]]:
    ...
    for index, capability_id in enumerate(CAPABILITY_IDS):
        ...
        row = OperationalRuntimeBundleCloseoutRow(
            row_id=_stable_id("source_adapter.operational_runtime_bundle_closeout.row", seed),
            row_index=index,
            capability_id=capability_id,
            ...
            source_artifact_role="operational runtime bundle closeout",
        )
...
def build_package(*, operator_id: str = "operator", execution_mode: str = "operator_controlled") -> dict[str, Any]:
    ...
    package_seed = {
        ...
        "purpose": "operational runtime bundle closeout",
    }
    package: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "package_id": _stable_id("source_adapter.operational_runtime_bundle_closeout.package", package_seed),
        "status": STATUS,
        "purpose": "operational runtime bundle closeout",
        "operator_id": operator_id,
        "execution_mode": execution_mode,
...
def write_package(output_dir: str | Path, *, operator_id: str = "operator") -> dict[str, Any]:
    ...
    package_path = out / f"{package['package_id']}.operational_runtime_bundle_closeout.json"
    rows_path = out / f"{package['package_id']}.operational_runtime_bundle_closeout.rows.json"
    verification_path = out / f"{package['package_id']}.operational_runtime_bundle_closeout.verification.json"
