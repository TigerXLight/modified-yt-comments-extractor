...
def test_contract_and_static_assets():
    ...
    assert 'r45af_textrect_fallback_first_rule' in r45j.contract()
    assert 'r45ag_restore_r45h_modal_engine_rule' in r45j.contract()
    assert '_r45ag_restored_r45h_modal_auto_expand' in inspect.getsource(r45j)
    assert 'R45AG_RESTORED_R45H_MODAL_AUTO_EXPAND_START' in inspect.getsource(r45j)
    assert 'JS_BOUNDED_MODAL_AUTO_EXPAND' in inspect.getsource(r45j._r45ag_restored_r45h_modal_auto_expand)
    assert 'R45AF_TEXTRECT_FALLBACK_FIRST' in inspect.getsource(r45j)
    assert 'textRectForLabel' in inspect.getsource(r45j)
    assert 'rolebutton_fallback_first' in inspect.getsource(r45j)
    assert 'text_rect_click' in inspect.getsource(r45j._playwright_mouse_downward_expand)
