...
from source_adapter_article_html_snapshot_receipt_runtime import RUNTIME_STAGE, ArticleHtmlSnapshotReceiptRecord


def verify_record(record: ArticleHtmlSnapshotReceiptRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[ArticleHtmlSnapshotReceiptRecord]) -> list[str]:
    issues: list[str] = []
