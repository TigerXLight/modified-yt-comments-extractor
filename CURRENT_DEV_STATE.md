...
- Cloud-ASR row 2C2 Save/Clear/status behavior and the secure YouTube credential workflow remain unchanged. No real cloud-ASR provider consumes credentials yet.
- The exact next ordered credential boundary is wiring real provider actions through the trusted consumption helper and/or explicit connection testing for stored credentials. The roadmap does not currently define a precise row label for that boundary, so it remains separately approval-gated. Provider/API calls, cloud-ASR execution, uploads, OAuth/browser flows, network behavior, and any future reveal/copy/export behavior remain later independently approved boundaries.
