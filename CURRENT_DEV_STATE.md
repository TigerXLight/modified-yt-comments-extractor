...
- Future media preservation planning should include an explicit user choice for webpage media downloads: `all` to include every discovered image/video/media asset, or `select` so the user can choose individual assets; this must remain opt-in and should not download media by default.
