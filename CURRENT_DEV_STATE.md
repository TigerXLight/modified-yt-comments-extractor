...
- This is not a production-live ElevenLabs integration. No production HTTP transport, SDK client, live API request, real credential use, media upload, realtime, webhook/async retrieval, remote source URL submission, YouTube/TikTok URL submission, multichannel mode, account/quota/model lookup, retry/fallback, GUI transcription action, Test Connection wiring, or network behavior exists. `asr_tools_test.py` remains compile-checked only because it is an interactive/manual harness.
- Cloud-ASR row 2C2 Save/Clear/status behavior and the secure YouTube credential workflow remain unchanged. The exact next ordered provider boundary is production ElevenLabs transport, then live manual verification with explicit user permission and real credentials kept out of prompts/tests, then later user-facing ASR action and Test Connection wiring. The roadmap does not currently define a precise row label for that boundary, so it remains separately approval-gated. OAuth/browser flows, broader provider/API calls, uploads, network behavior, and any future reveal/copy/export behavior remain later independently approved boundaries.
