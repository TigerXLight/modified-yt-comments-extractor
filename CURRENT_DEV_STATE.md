...
- ReplayWeb.page / Local Web Archive closeout now includes optional ReplayWeb.page viewer detection, official release metadata parsing, safe argument-array Open archive previews, Show files previews, browser/PWA local-file limitation reporting, WACZ/manifest verification, and a non-destructive offline WACZ replay-compatibility repair path. Commit `c3eb5e9` fixed malformed WARC request-line generation, the old `_wb_method=HTTP/1.1` ReplayWeb symptom, hosted ReplayWeb.page profile handling, sorted/canonical CDXJ readiness, and package-readiness wording as `REPLAYWEB_PAGE_PACKAGE_READY` rather than visual compatibility. Manual follow-up remains a recorded limitation: normalized WACZ page entries still produced `Archived Page Not Found`; normalized raw WARC/WARC.GZ removed the `_wb_method=HTTP/1.1` symptom but only partially replayed the MSN runtime with privacy modal, black/empty page area, severe Firefox lag, and no visible comments. `source_replay_static_snapshot.py` and the `--add-static-evidence-page` repair CLI option now create a separate derived `STATIC_EVIDENCE_VIEW_READY` no-JavaScript evidence page inside a side-by-side WACZ, preserving the original dynamic page entry and original input archive. This derived static view is a stable review fallback generated from local archived evidence; it is not the original dynamic runtime and does not make comments visually replayable in ReplayWeb.
