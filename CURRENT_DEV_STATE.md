...
- Row 2B is now the first unresolved ordered layer and requires separate explicit approval for a bounded runtime credential-provider abstraction limited to read-only non-secret presence/provenance status, safe diagnostic categories, and fail-closed backend behavior. Row 2C save/clear/migration/reveal/copy UI and all real provider/network testing remain later boundaries.
