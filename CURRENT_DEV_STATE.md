...
- Row 2C1 has no production caller and does not change `main.py`, Access & Keys GUI, settings persistence, provider logic, or the existing YouTube API-key/settings/keyring workflow. It adds no plaintext/settings/env/file fallback, migration, legacy cleanup, reveal/copy behavior, connection testing, provider calls, OAuth, browser access, or network behavior.
- Row 2C2 is now the first unresolved ordered layer and requires separate explicit security approval for masked credential-entry fields, explicit save/clear GUI actions, and any later migration/legacy plaintext cleanup. Real connection/provider/network testing remains a later independently approved boundary.
