...
- Cloud-ASR row 2C2 Save/Clear/status behavior and the secure YouTube credential workflow remain unchanged. The exact next ordered provider boundary is live manual ElevenLabs verification with explicit user permission, real credentials kept out of prompts/logs/tests, preferably a disposable or revocable key, a tiny non-sensitive media sample, no automatic retries, explicit one-call confirmation, and safe capture of only non-secret result metadata. Later user-facing ASR action and Test Connection wiring remain separately approval-gated. The roadmap does not currently define a precise row label for that boundary. OAuth/browser flows, broader provider/API calls, uploads, network behavior, and any future reveal/copy/export behavior remain later independently approved boundaries.
