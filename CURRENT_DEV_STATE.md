...
- The existing `youtube_data_api_key` workflow remains excluded from the row 2C1/2C2 secure-store path. YouTube API-key/settings/keyring/legacy-fallback behavior is unchanged, and migration/legacy plaintext cleanup remains unresolved and separately approval-gated.
- Row 2C2 does not implement provider credential validation, connection tests, provider/API calls, cloud-ASR execution, audio upload, OAuth, browser access, browser-profile access, scraping, downloads, networking, reveal/copy controls, automatic migration, environment-variable writes, settings.json credential writes, or plaintext credential fallback.
- The exact next ordered credential boundary is the remaining separately approved row 2C credential lifecycle work: existing YouTube credential migration, legacy plaintext cleanup, and any cleanup/removal of the legacy YouTube plaintext fallback. Provider credential consumption, connection testing, OAuth/browser flows, provider/API integration, cloud-ASR execution, uploads, and network behavior remain later independently approved boundaries.
