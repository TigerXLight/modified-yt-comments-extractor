...
- ReplayWeb.page / Local Web Archive closeout adds `source_local_web_archive_actions.py` and focused tests for optional ReplayWeb.page viewer detection, official release metadata parsing, safe argument-array Open archive previews, Show files previews, browser/PWA local-file limitation reporting, and WACZ/manifest verification. Official release metadata was checked as ReplayWeb.page App `v2.4.7`; no binary was downloaded or installed, no AGPL viewer code was vendored, and no desktop executable is configured yet. The accepted MSN WACZ verifies as `STRUCTURAL_WACZ_VALID` with ZIP integrity, datapackage profile `data-package`, WACZ version `1.2.0`, one page, 87 index lines, 174 readable WARC records, matching WACZ/manifest hashes, expected source URL present, and comments evidence `25/25` with `16` roots plus `9` replies. ReplayWeb visual status remains `USER_VISUAL_CONFIRMATION_REQUIRED`, while the source-resource Local Web Archive popup now reports real Open/Show/Verify action state without launching viewers or touching live sites.
