...
- Added a GUI-independent, non-secret Access & Keys manager view model with deterministic platform sections, search/filter/selection state, and focused tests; actual `KEYS` sidebar/window wiring, credentials, connection tests, and provider calls remain unimplemented.
