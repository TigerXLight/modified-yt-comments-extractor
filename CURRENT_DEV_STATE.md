...
- Cloud-ASR row 2C2 Save/Clear/status behavior and the secure YouTube credential workflow remain unchanged. The exact next ordered credential boundary is a first real provider implementation and later explicit user-facing Test Connection wiring. The roadmap does not currently define a precise row label for that boundary, so it remains separately approval-gated. Live provider/API calls, cloud-ASR execution, uploads, OAuth/browser flows, network behavior, and any future reveal/copy/export behavior remain later independently approved boundaries.
