...
- Controlled MSN vertical live validation support was added in `66b589f77a23e1e82ba7b774beaeea281b217b6b` via `source_msn_vertical_live_validation.py`. It is limited to the explicitly approved MSN URL, writes review artifacts outside the repo, records source URL/query/fragment/canonical provenance, performs the approved single MSN fetch, performs Wayback availability check only, writes Local Web Archive WARC/WACZ plus an offline evidence bundle, and keeps ArchiveBox as an optional advanced backend. The live run at `C:\Users\fahad\AppData\Local\Temp\ytce_msn_vertical_live_validation_20260808_235152` produced manifest SHA-256 `6d711d840a2fed6973e6f42a1807c2c368fe45987a77ee34582e0c9f73bacc8b`; static MSN HTML was a shell, so article text/visible outline/resource inventory were PARTIAL, comments and screenshots were BLOCKED by missing browser/shadow-DOM/screenshot runtime, representative download was N_A because no direct resource was found, Wayback check was PARTIAL due HTTP 429, and no Wayback submit, archive.today submit, ArchiveBox execution, browser profile/cookies, evidence DB movement, ASR job, or broad/live crawl occurred.
- Controlled MSN rendered-browser validation is now implemented in `source_msn_rendered_browser_validation.py` with Playwright/Chromium and `warcio` as documented optional runtime dependencies. The approved MSN URL was revalidated at `C:\Users\fahad\AppData\Local\Temp\ytce_msn_rendered_browser_validation_dk1_t38t`; manifest SHA-256 `abb0a2d6b6d391aa06daa7018d6a9f05c5f217f7d786c3c2cb97bb60f09eba0f`; WACZ SHA-256 `883076d26a77108f86c4c6732b94997e4399b0bca7f40664c585c9634fb6f886`. Status: article text, page outline, rendered DOM, faithful screenshots, representative article image download, browser-backed WARC, standards-oriented WACZ, offline evidence bundle, and action log are `RENDERED_BROWSER_LIVE_TESTED` / `LOCAL_PACKAGE_STRUCTURALLY_VERIFIED` as applicable. Comments component detection found `social-comment-wc` with open shadow root, and the Android/mobile collector now recursively walks nested open shadows, drives internal scrollports, expands reply groups, incrementally serializes rows, and reconciles same-session provider `items` payloads. The live run captured `16` top-level comments plus `9` replies, exactly matching MSN's declared `25`; comment completeness is `COMMENTS_COMPLETE` / `RENDERED_BROWSER_LIVE_TESTED`, with faithful Android comments screenshot SHA-256 `cb1cc0b7067f7662ea5d2284a5b715a01b8334b3d02b1073a177550914ebb72e`, derived full-thread visual SHA-256 `04ec64fc9585eb76dd36e199e6e92770e599689e7a6acad58acd76469b77001c`, and comments JSON SHA-256 `d970bf0046345006c3c1088c35b148b2ae7a67aaa7605708b9ef067c5be52154`. ReplayWeb.page remains `VIEWER_NOT_CONFIGURED`; no replay visual verification is claimed. Wayback CHECK was rate-limited with HTTP 429 and no submit/save was attempted. No archive.today submit, ArchiveBox/Docker/WSL execution, browser profile/cookies/accounts, second site, Evidence Database work, evidence file movement, ASR job, or provider call occurred.
- ReplayWeb.page / Local Web Archive closeout now includes optional ReplayWeb.page viewer detection, official release metadata parsing, safe argument-array Open archive previews, Show files previews, browser/PWA local-file limitation reporting, WACZ/manifest verification, and a non-destructive offline WACZ replay-compatibility repair path. Commit `c3eb5e9` fixed malformed WARC request-line generation, the old `_wb_method=HTTP/1.1` ReplayWeb symptom, hosted ReplayWeb.page profile handling, sorted/canonical CDXJ readiness, and package-readiness wording as `REPLAYWEB_PAGE_PACKAGE_READY` rather than visual compatibility. Commit `eeba5bb` added static WACZ evidence-view infrastructure, and manual testing confirmed the direct static evidence HTML plus raw static evidence WARC.GZ are usable; however, the derived static WACZ listed both pages while clicking the static URL still returned `Archived Page Not Found`, so WACZ visual/replay success is not claimed. Normalized raw WARC/WARC.GZ still only partially replays the MSN runtime with privacy modal, black/empty page area, severe Firefox lag, and no visible comments. The stable evidence-report fallback is direct `STATIC_EVIDENCE_HTML_READY` output plus raw `STATIC_EVIDENCE_WARC_READY` WARC/WARC.GZ outputs. The first static archived page-view output worked technically but looked too report-like, so the current fallback now improves `static-page-view.html` into a dark MSN/mobile-style, webpage-like reconstruction with relative copied local screenshot assets and `STATIC_PAGE_VIEW_VISUAL_FIDELITY_PARTIAL` status, and adds `static-text-view.html`, `static-article.txt`, `static-comments.txt`, `static-page-text.txt`, and `static-page-text.md` for collapsible local reading/copying. These outputs use only existing local article/comment/screenshot manifest artifacts, without JavaScript, iframes, remote runtime dependencies, MSN recapture, or accepted WACZ/WARC overwrite. The original dynamic ReplayWeb/WACZ limitation remains documented.
...
## Final App Wiring And Audit-Ready Completion - 2026-08-08

- Implementation commits: `131e9fb` app/operator controller surfaces and receipt import review; `1ec8472` app workflow/store/export wiring.
- The Source Evidence execution bridges are now exposed through a real app-facing controller layer in `source_app_operator_controller.py` and a `main.py` wrapper. The controller exposes Source URL/FILES state, operator approval rows, unified execution job preview/local-temp states, local E2E Total Export fixture package creation, database movement preview/temp execution state, manual live-smoke dry-run helpers, archive/media/ArchiveBox launch previews, receipt import/review, and Access/Online ASR preservation summaries.
...
- Commit `480f21e` was audited rather than assumed complete. The repo already had rendered-page/local viewer/WARC/WACZ generation paths through the live viewable capture wrapper, but raw WARC replay verification and WACZ replay verification remain separate manual validation states: generated artifacts now report `WARC_REPLAY_STATUS: NOT_TESTED` / `WACZ_REPLAY_STATUS: NOT_TESTED` unless explicit replay metadata is supplied.
- `msn_source_adapter.py` now exposes the accepted V15 comments screenshot method as a real callable path: `social-comment-wc` open shadow-root traversal, nested open shadow-root recursion, internal comments scroller selection, page/feed scroll locking, reply and clamped-text expansion, final consent/privacy overlay suppression, and one normal `android_comments_all_expanded_SINGLE_INTERNAL_STITCH.png` output. Segment/debug artifacts remain debug-only.
- The accepted V6 Android article screenshot path is now a callable method that uses the article URL without `#comments`, aligns to the headline/body, and clips before sponsored/recommendation boundaries where detected.
- MSN comments/profile JSON and HTML now preserve claim-scoped source-role metadata. A commenter comment/reply is primary/original authored evidence only for the limited authorship claim; it is not automatically primary evidence for incident claims. MSN/The Independent article framing remains secondary/outside perspective by default, and the York `AA292lx3.img` media chain records the claimed Google Street View credit as `PRIMARY_SOURCE_CLAIMED_BUT_UNVERIFIED` with a source-chain gap.

## MSN Source Role Current-Field Tightening - 2026-08-11

- Source-role labels, media source-chain fields, disputed framing/source-author correction notes, manual source notes, closed-loop flags, primary-source status, and source-chain gaps are current manual/export metadata fields. Only fully automated video/media evidence extraction and perceptual hash/fingerprint matching remain deferred.
- York source-role sidecars now represent the MSN republisher layer as `TERTIARY_PROPAGATED_SOURCE`, The Independent police-derived layer as `TERTIARY_PROPAGATED_SOURCE` for incident claims, and the linked North Yorkshire Police page as `SECONDARY_AUTHORITY_SOURCE` with `PRIMARY_SOURCE_LOCATED` for the police statement itself. People mentioned in the article are not treated as primary/original sources without their own direct authored/raw evidence.

## MSN Production Closeout Gate Tightening - 2026-08-11

- The real York production closeout run at `msn_york_adapter_closeout_20260811_065300` remains review-required: live capture completed, but the production V15 comments screenshot path failed to select an internal scroller, comment exports were not imported into closeout, the accepted screenshot names were fallback-promoted, and the required `AA292lx3.img` media item was missing.
