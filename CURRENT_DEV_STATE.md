...
- Cloud-ASR row 2C2 Save/Clear/status behavior and the secure YouTube credential workflow remain unchanged. The exact next ordered credential boundary is explicit connection testing and/or a first real provider implementation. The roadmap does not currently define a precise row label for that boundary, so it remains separately approval-gated. Connection testing must be explicit/user-triggered; provider/API calls, cloud-ASR execution, uploads, OAuth/browser flows, network behavior, and any future reveal/copy/export behavior remain later independently approved boundaries.
