...
- ReplayWeb.page / Local Web Archive closeout now includes optional ReplayWeb.page viewer detection, official release metadata parsing, safe argument-array Open archive previews, Show files previews, browser/PWA local-file limitation reporting, WACZ/manifest verification, and a non-destructive offline WACZ replay-compatibility repair path. Investigation after ReplayWeb.page returned `Archived Page Not Found` exposed a genuine package/index defect in the already-accepted MSN WACZ: the comments/evidence capture remains accepted at `25/25` (`16` roots plus `9` replies), but the WACZ used the pre-1.2 `data-package` profile while declaring `1.2.0`, retained removed legacy metadata, and its `87` CDXJ rows were not byte-wise sorted and did not consistently use canonical lower-case searchable URL keys. The writer/verifier now target WACZ 1.2.0 with profile `wacz`, omit removed `wacz_version`/`mainPageUrl`/`mainPageDate` fields, strip client-only URL fragments from replay entry metadata, and require sorted canonical CDXJ lookup keys. `source_local_web_archive_repair_cli.py` creates a side-by-side repaired WACZ, refuses overwrite/in-place repair, performs no network action, and preserves the original WARC bytes. The original WACZ remains retained evidence but is no longer classified replay-ready; manual ReplayWeb visual confirmation of the repaired copy remains `USER_VISUAL_CONFIRMATION_REQUIRED`.
