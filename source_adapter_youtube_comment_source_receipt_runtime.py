...
RUNTIME_TITLE = 'YouTube Comment Source Receipt'
RUNTIME_STAGE = 'youtube_comment_source_receipt'
...
class YoutubeCommentSourceReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> YoutubeCommentSourceReceiptRecord:
    ...
    return YoutubeCommentSourceReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> YoutubeCommentSourceReceiptRecord:
    return build_record(
        "youtube_comment_source_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "youtube_comment_source_receipt"},
    )


def summarise_record(record: YoutubeCommentSourceReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: YoutubeCommentSourceReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> YoutubeCommentSourceReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: YoutubeCommentSourceReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> YoutubeCommentSourceReceiptRecord:
