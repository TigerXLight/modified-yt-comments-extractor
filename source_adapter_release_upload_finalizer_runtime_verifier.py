...
from source_adapter_release_upload_finalizer_runtime import RUNTIME_STAGE, ReleaseUploadFinalizerRecord


def verify_record(record: ReleaseUploadFinalizerRecord) -> list[str]:
    issues: list[str] = []
...
def verify_records(records: Iterable[ReleaseUploadFinalizerRecord]) -> list[str]:
    issues: list[str] = []
