...
RUNTIME_TITLE = 'KEYS Accounts Secret Boundary Receipt'
RUNTIME_STAGE = 'keys_accounts_secret_boundary_receipt'
...
class KeysAccountsSecretBoundaryReceiptRecord:
    runtime_id: str
...
def build_record(
    ...
    *,
    ...
    status: str = DEFAULT_STATUS,
) -> KeysAccountsSecretBoundaryReceiptRecord:
    ...
    return KeysAccountsSecretBoundaryReceiptRecord(
        runtime_id=clean_id,
        status=clean_status,
...
def build_default_record() -> KeysAccountsSecretBoundaryReceiptRecord:
    return build_record(
        "keys_accounts_secret_boundary_receipt-default",
        input_refs=("source-evidence-roadmap", "operator-approval-gate"),
        output_refs=("redacted-receipt", "total-export-source-evidence"),
        metadata={"runtime_title": RUNTIME_TITLE, "slice": "keys_accounts_secret_boundary_receipt"},
    )


def summarise_record(record: KeysAccountsSecretBoundaryReceiptRecord) -> dict[str, Any]:
    data = record.to_dict()
    return {
...
def render_record_text(record: KeysAccountsSecretBoundaryReceiptRecord) -> str:
    summary = summarise_record(record)
    return "\n".join(f"{key}: {summary[key]}" for key in sorted(summary))


def record_from_dict(payload: Mapping[str, Any]) -> KeysAccountsSecretBoundaryReceiptRecord:
    return build_record(
...
def write_record(path: str | Path, record: KeysAccountsSecretBoundaryReceiptRecord) -> Path:
    target = Path(path)
...
def read_record(path: str | Path) -> KeysAccountsSecretBoundaryReceiptRecord:
